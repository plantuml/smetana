...
#line 34 "graphviz-2.38.0\\lib\\neatogen\\embed_graph.h" 2
	   ...
	   node_t ** nodes)
{
	...
	if (((((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pinned) > 0)) {
	    pt = (((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pos);
	    *xp++ = *pt++;
	    *yp++ = *pt++;
	    if (dim > 2) {
		for (d = 2; d < dim; d++)
...
{
    int i, j, e, rv, count;
    float *Dij = (float*)zmalloc((nG *(nG + 1) / 2)*sizeof(float));
    double **Gm;
    double **Gm_inv;
    ...
    )
{
    ...
    double conj_tol = 1e-3;	/* tolerance of Conjugate Gradient */
	...
	*************************************************/
    ...
    int subspace_dim = ((50)<(n)?(50):(n));	/* overall dimensionality of subspace */
    double **subspace = (double **)gmalloc((subspace_dim)*sizeof(double *));
    double *d_storage = (double*)gmalloc((subspace_dim * n)*sizeof(double));
    ...
    /* compute PHDE: */
    num_centers_local = ((n)<(((2 * subspace_dim)>(50)?(2 * subspace_dim):(50)))?(n):(((2 * subspace_dim)>(50)?(2 * subspace_dim):(50))));
    full_coords = (void *)0;
	...
	*************************************************/
    ...
    CenterIndex = (int*)gmalloc((n)*sizeof(int));
    for (i = 0; i < n; i++) {
	CenterIndex[i] = -1;
    }
    invCenterIndex = (void *)0;
    ...
    Dij = (void *)0;
    dist = (DistType*)gmalloc((n)*sizeof(DistType));
    if (num_centers == 0) {	/* no pivots, skip pivots-to-nodes distance calculation */
	goto after_pivots_selection;
    }

    invCenterIndex = (int*)gmalloc((num_centers)*sizeof(int));

    storage = (DistType*)gmalloc((n * num_centers)*sizeof(DistType));
    Dij = (DistType **)gmalloc((num_centers)*sizeof(DistType *));
	...
	max_dist = 0;
	for (j = 0; j < n; j++) {
	    dist[j] = ((dist[j])<(Dij[i][j])?(dist[j]):(Dij[i][j]));
	    if (dist[j] > max_dist
  ...
  after_pivots_selection:
    ...
    visited_nodes = (int*)gmalloc((n)*sizeof(int));
    distances = (dist_data*)gmalloc((n)*sizeof(dist_data));
    available_space = 0;
    nedges = 0;
    for (i = 0; i < n; i++) {
	if (CenterIndex[i] >= 0) {	/* a pivot node */
	    distances[i].edges = (int*)gmalloc((n - 1)*sizeof(int));
	    distances[i].edist = (DistType*)gmalloc((n - 1)*sizeof(DistType));
	    distances[i].nedges = n - 1;
	    nedges += n - 1;
	    distances[i].free_mem = (!(0));
	    index = CenterIndex[i];
	    for (j = 0; j < i; j++) {
	...
	if (num_neighbors > available_space) {
	    ...
	    storage1 = (int*)gmalloc((available_space)*sizeof(int));
	    storage2 = (DistType*)gmalloc((available_space)*sizeof(DistType));
	    distances[i].free_mem = (!(0));
	} else {
	    distances[i].free_mem = 0;
	}
	...
	*************************************************/
    ...
    lap = (vtx_data*)gmalloc((n)*sizeof(vtx_data));
    edges = (int*)gmalloc((nedges + n)*sizeof(int));
    ewgts = (float*)gmalloc((nedges + n)*sizeof(float));
    for (i = 0; i < n; i++) {
	lap[i].edges = edges;
	lap[i].ewgts = ewgts;
	...
	*************************************************/
    ...
    directions = (double **)gmalloc((dim)*sizeof(double *));
    directions[0] = (double*)gmalloc((dim * subspace_dim)*sizeof(double));
    for (i = 1; i < dim; i++) {
	...
	*************************************************/
    ...
    b = (double*)gmalloc((n)*sizeof(double));
    b_restricted = (double*)gmalloc((subspace_dim)*sizeof(double));
    old_stress = compute_stress1(coords, distances, dim, n, exp);
    for (converged = 0, iterations = 0;
...
{
    int i, j, count;
    float *Dij = (float*)zmalloc((n *(n + 1) / 2)*sizeof(float));

    float *Di = (float*)zmalloc((n)*sizeof(float));
    Queue Q;
...
float *compute_apsp_packed(vtx_data * graph, int n)
{
    ...
    DistType *Di = (DistType*)zmalloc((n)*sizeof(DistType));
    Queue Q;
...
float *compute_apsp_artifical_weights_packed(vtx_data * graph, int n)
{
    ...
    weights = (float*)zmalloc((nedges)*sizeof(float));
    vtx_vec = (int*)zmalloc((n)*sizeof(int));
    for (i = 0; i < n; i++) {
	vtx_vec[i] = 0;
    }
...
(deg_i + deg_j -
 ...
 graph[i].ewgts[j]));
    ...
    free(vtx_vec);
    free(graph[0].ewgts);
    graph[0].ewgts = (void *)0;
    if (old_weights != (void *)0) {
	for (i = 0; i < n; i++) {
    ...
    )
{
    ...
    int i, j, k;
    float **coords = (void *)0;
    float *f_storage = (void *)0;
    float constant_term;
    int count;
    long double degree;
    int lap_length;
    float *lap2 = (void *)0;
    long double *degrees = (void *)0;
    int step;
    float val;
    double old_stress, new_stress;
    boolean converged;
    float **b = (void *)0;
    float *tmp_coords = (void *)0;
    float *dist_accumulator = (void *)0;
    float *lap1 = (void *)0;
    int smart_ini = opts & 0x4;
    int exp = opts & 0x3;
    int len;
	...
	**************************/
    ...
    coords = (float **)zmalloc((dim)*sizeof(float *));
    f_storage = (float*)zmalloc((dim * n)*sizeof(float));
    for (i = 0; i < dim; i++) {
	...
	**************************/
    ...
    count = 0;
    degrees = (long double*)zmalloc((n)*sizeof(long double));
    /* set_vector_val(n, 0, degrees); */
    memset(degrees, 0, n * sizeof(long double));
    for (i = 0; i < n - 1; i++) {
	degree = 0;
	...
	*************************/
    ...
    b = (float **)zmalloc((dim)*sizeof(float *));
    b[0] = (float*)zmalloc((dim * n)*sizeof(float));
    for (k = 1; k < dim; k++) {
	b[k] = b[0] + k * n;
    }

    tmp_coords = (float*)zmalloc((n)*sizeof(float));
    dist_accumulator = (float*)zmalloc((n)*sizeof(float));
    lap1 = (void *)0;
    ...
    lap1 = (float*)zmalloc((lap_length)*sizeof(float));
    ...
    old_stress = 1.7976931348623157e+308;	/* at least one iteration */
	...
	for (count = 0, i = 0; i < n - 1; i++) {
	    ...
	    /* detect overflows */
	    for (j = 0; j < len; j++) {
		if (dist_accumulator[j] >= ((float)3.40282347e+38)
	 ...
	 */
	{
	    ...
	    double change = ((diff) >= 0 ? (diff) : -(diff));
