...
double **new_array(int m, int n, double ival)
{
    ...
    rv = (double **)zmalloc((m)*sizeof(double *));
    mem = (double*)zmalloc((m * n)*sizeof(double));
    for (i = 0; i < m; i++) {
	rv[i] = mem;
	mem = mem + n;
	for (j = 0; j < n; j++)
...
static double ***new_3array(int m, int n, int p, double ival)
{
    ...
    rv = (double ***)zmalloc((m + 1)*sizeof(double **));
    for (i = 0; i < m; i++) {
	rv[i] = (double **)zmalloc((n + 1)*sizeof(double *));
	for (j = 0; j < n; j++) {
	    rv[i][j] = (double*)zmalloc((p)*sizeof(double));
	    for (k = 0; k < p; k++)
		rv[i][j][k] = ival;
	}
	rv[i][j] = (void *)0;	/* NULL terminate so we can clean up */
    }
    rv[i] = (void *)0;
    return rv;
}
...
static int degreeKind(graph_t * g, node_t * n, node_t ** op)
{
	...
	if (deg == 1) {
	    if (((((((((Agobj_t*)(ep))->tag).objtype) == 3? (ep): ((ep)+1))->node) == n) && (((((((Agobj_t*)(ep))->tag).objtype) == 2? (ep): ((ep)-1))->node) == other)) ||	/* ignore multiedge */
		((((((((Agobj_t*)(ep))->tag).objtype) == 3? (ep): ((ep)+1))->node) == other) && (((((((Agobj_t*)(ep))->tag).objtype) == 2? (ep): ((ep)-1))->node) == n)))
		continue;
	    return 2;
	} else {		/* deg == 0 */
	    if (((((((Agobj_t*)(ep))->tag).objtype) == 3? (ep): ((ep)+1))->node) == n)
		other = ((((((Agobj_t*)(ep))->tag).objtype) == 2? (ep): ((ep)-1))->node);
	    else
		other = ((((((Agobj_t*)(ep))->tag).objtype) == 3? (ep): ((ep)+1))->node);
	    *op = other;
	    deg++;
	}
    }
...
static double setEdgeLen(graph_t * G, node_t * np, Agsym_t* lenx, double dfltlen)
{
	...
	(((Agedgeinfo_t*)(((Agobj_t*)(ep))->data))->dist) = len;
	total_len += len;
    }
    return total_len;
}
...
int scan_graph_mode(graph_t * G, int mode)
{
	...
	if ((str = agget(G->root, "Damping")))
	    ...
	    Damping = .99;
	(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist) = (node_t **)zmalloc((nV + 1)*sizeof(node_t *));
	for (i = 0, np = agfstnode(G); np; np = agnxtnode(G, np)) {
	    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i] = np;
	    (((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->id) = i++;
	    (((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->heapindex) = -1;
    ...
    if (str && str[0])
	Initial_dist = ((Epsilon)>(atof(str))?(Epsilon):(atof(str)));
    else
	Initial_dist = total_len / (nE > 0 ? nE : 1) * sqrt(nV) + 1;

    if (!Nop && (mode == 0)) {
	(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist) = new_array(nV, nV, Initial_dist);
	(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->spring) = new_array(nV, nV, 1.0);
	(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t) = new_array(nV, Ndim, 1.0);
	(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t) = new_3array(nV, nV, Ndim, 0.0);
    }
...
{
    return scan_graph_mode(g, 0);
}

void free_scan_graph(graph_t * g)
{
    free((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->neato_nlist));
    if (!Nop) {
	free_array((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->dist));
	free_array((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->spring));
	free_array((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->sum_t));
	free_3array((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->t));
	(((Agraphinfo_t*)(((Agobj_t*)(g))->data))->t) = (void *)0;
    }
}
...
void jitter_d(node_t * np, int nG, int n)
{
    int k;
    for (k = n; k < Ndim; k++)
	(((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pos)[k] = nG * drand48();
}
...
{
    (((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pos)[0] = nG * drand48();
    (((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pos)[1] = nG * drand48();
    if (Ndim > 2)
	jitter3d(np, nG);
}
...
void initial_positions(graph_t * G, int nG)
{
    ...
    for (i = 0; (np = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i]); i++) {
	if (((((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pinned) > 0))
	    continue;
	randompos(np, 1);
    }
}
...
void diffeq_model(graph_t * G, int nG)
{
    ...
    /* init springs */
    K = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->spring);
    D = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist);
    for (i = 0; i < nG; i++) {
	for (j = 0; j < i; j++) {
	    f = 1.0 / (D[i][j] * D[i][j]);
	    if ((e = (agedge(G,(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i],(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[j],(void *)0,0))))
		f = f * (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->factor);
	    K[i][j] = K[j][i] = f;
	}
    }
    ...
    for (i = 0; i < nG; i++)
	for (k = 0; k < Ndim; k++)
	    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[i][k] = 0.0;

    for (i = 0; (vi = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i]); i++) {
	for (j = 0; j < nG; j++) {
	    if (i == j)
		continue;
	    vj = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[j];
	    dist = distvec((((Agnodeinfo_t*)(((Agobj_t*)(vi))->data))->pos), (((Agnodeinfo_t*)(((Agobj_t*)(vj))->data))->pos), del);
	    for (k = 0; k < Ndim; k++) {
		(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[i][j][k] =
		    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->spring)[i][j] * (del[k] -
					  (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[i][j] * del[k] /
					  dist);
		(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[i][k] += (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[i][j][k];
	    }
	}
    }
    if (Verbose) {
...
static double total_e(graph_t * G, int nG)
{
    ...
    for (i = 0; i < nG - 1; i++) {
	ip = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i];
	for (j = i + 1; j < nG; j++) {
	    jp = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[j];
	    for (t0 = 0.0, d = 0; d < Ndim; d++) {
		t1 = ((((Agnodeinfo_t*)(((Agobj_t*)(ip))->data))->pos)[d] - (((Agnodeinfo_t*)(((Agobj_t*)(jp))->data))->pos)[d]);
		t0 += t1 * t1;
	    }
	    e = e + (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->spring)[i][j] *
		(t0 + (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[i][j] * (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[i][j]
		 - 2.0 * (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[i][j] * sqrt(t0));
	}
    }
    return e;
}
...
void solve_model(graph_t * G, int nG)
{
	...
	fprintf(stderr, " %d%s iterations %.2f sec\n",
		(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->move), ((((Agraphinfo_t*)(((Agobj_t*)(G))->data))->move) == MaxIter ? "!" : ""),
		elapsed_sec());
    }
    if ((((Agraphinfo_t*)(((Agobj_t*)(G))->data))->move) == MaxIter)
...
{
    int j, k;
    double del[10], dist, old;
    node_t *vi, *vj;

    vi = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i];
    for (k = 0; k < Ndim; k++)
	(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[i][k] = 0.0;
    for (j = 0; j < nG; j++) {
	if (i == j)
	...
	dist = distvec((((Agnodeinfo_t*)(((Agobj_t*)(vi))->data))->pos), (((Agnodeinfo_t*)(((Agobj_t*)(vj))->data))->pos), del);
	for (k = 0; k < Ndim; k++) {
	    old = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[i][j][k];
	    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[i][j][k] =
		(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->spring)[i][j] * (del[k] -
				      (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[i][j] * del[k] / dist);
	    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[i][k] += (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[i][j][k];
	    old = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[j][i][k];
	    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[j][i][k] = -(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[i][j][k];
	    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[j][k] += ((((Agraphinfo_t*)(((Agobj_t*)(G))->data))->t)[j][i][k] - old);
	}
    }
}
...
void D2E(graph_t * G, int nG, int n, double *M)
{
    int i, l, k;
    node_t *vi, *vn;
    double scale, sq, t[10];
    double **K = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->spring);
    double **D = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist);

    vn = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[n];
    for (l = 0; l < Ndim; l++)
	for (k = 0; k < Ndim; k++)
	    M[(l)*Ndim+(k)] = 0.0;
    for (i = 0; i < nG; i++) {
	...
	sq = 0.0;
	for (k = 0; k < Ndim; k++) {
	    t[k] = (((Agnodeinfo_t*)(((Agobj_t*)(vn))->data))->pos)[k] - (((Agnodeinfo_t*)(((Agobj_t*)(vi))->data))->pos)[k];
	    sq += (t[k] * t[k]);
	}
	scale = 1 / fpow32(sq);
	for (k = 0; k < Ndim; k++) {
	    for (l = 0; l < k; l++)
		M[(l)*Ndim+(k)] += K[n][i] * D[n][i] * t[k] * t[l] * scale;
	    M[(k)*Ndim+(k)] +=
...
node_t *choose_node(graph_t * G, int nG)
{
    ...
    choice = (void *)0;
    for (i = 0; i < nG; i++) {
	np = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[i];
	if ((((Agnodeinfo_t*)(((Agobj_t*)(np))->data))->pinned) > 1)
	    continue;
	for (m = 0.0, k = 0; k < Ndim; k++)
	    m += ((((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[i][k] * (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[i][k]);
...
{
    int i, m;
    static double *a, b[10], c[10];

    m = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->id);
    a = (a? (double*)grealloc(a,(Ndim * Ndim)*sizeof(double)):(double*)gmalloc((Ndim * Ndim)*sizeof(double)));
    D2E(G, nG, m, a);
    for (i = 0; i < Ndim; i++)
	c[i] = -(((Agraphinfo_t*)(((Agobj_t*)(G))->data))->sum_t)[m][i];
    solve(a, b, c, Ndim);
    for (i = 0; i < Ndim; i++) {
	b[i] = (Damping + 2 * (1 - Damping) * drand48()) * b[i];
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[i] += b[i];
    }
    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->move)++;
...
void heapup(node_t * v)
{
    ...
    for (i = (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex); i > 0; i = par) {
	par = (i - 1) / 2;
	u = Heap[par];
	if ((((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->dist) <= (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->dist))
	    break;
	Heap[par] = v;
	(((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex) = par;
	Heap[i] = u;
	(((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->heapindex) = i;
    }
}
...
void heapdown(node_t * v)
{
    ...
    i = (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex);
    while ((left = 2 * i + 1) < Heapsize) {
	right = left + 1;
	if ((right < Heapsize)
	    && ((((Agnodeinfo_t*)(((Agobj_t*)(Heap[right]))->data))->dist) < (((Agnodeinfo_t*)(((Agobj_t*)(Heap[left]))->data))->dist)))
	    c = right;
	else
	    c = left;
	u = Heap[c];
	if ((((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->dist) <= (((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->dist))
	    break;
	Heap[c] = v;
	(((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex) = c;
	Heap[i] = u;
	(((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->heapindex) = i;
	i = c;
    }
...
void neato_enqueue(node_t * v)
{
    ...
    assert((((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex) < 0);
    i = Heapsize++;
    (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex) = i;
    Heap[i] = v;
    if (i > 0)
	heapup(v);
}
...
node_t *neato_dequeue(void)
{
    ...
    v = Heap[i];
    Heap[0] = v;
    (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->heapindex) = 0;
    if (i > 1)
	heapdown(v);
    (((Agnodeinfo_t*)(((Agobj_t*)(rv))->data))->heapindex) = -1;
    return rv;
}
...
void shortest_path(graph_t * G, int nG)
{
    ...
    Heap = (node_t **)zmalloc((nG + 1)*sizeof(node_t *));
    if (Verbose) {
...
void s1(graph_t * G, node_t * node)
{
    ...
    for (t = 0; (v = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->neato_nlist)[t]); t++)
	(((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->dist) = Initial_dist;
    Src = node;
    (((Agnodeinfo_t*)(((Agobj_t*)(Src))->data))->dist) = 0;
    (((Agnodeinfo_t*)(((Agobj_t*)(Src))->data))->hops) = 0;
    neato_enqueue(Src);

    while ((v = neato_dequeue())) {
	if (v != Src)
	    make_spring(G, Src, v, (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->dist));
	for (e = agfstedge(G, v); e; e = agnxtedge(G, e, v)) {
	    if ((u = ((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node)) == v)
		u = ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node);
	    f = (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->dist) + (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->dist);
	    if ((((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->dist) > f) {
		(((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->dist) = f;
		if ((((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->heapindex) >= 0)
		    heapup(u);
		else {
		    (((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->hops) = (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->hops) + 1;
		    neato_enqueue(u);
		}
	    }
	}
...
void make_spring(graph_t * G, node_t * u, node_t * v, double f)
{
    ...
    i = (((Agnodeinfo_t*)(((Agobj_t*)(u))->data))->id);
    j = (((Agnodeinfo_t*)(((Agobj_t*)(v))->data))->id);
    (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[i][j] = (((Agraphinfo_t*)(((Agobj_t*)(G))->data))->dist)[j][i] = f;
}
