...
static int maxcnt = 0;
static Point *tp1 = (void *)0;
static Point *tp2 = (void *)0;
static Point *tp3 = (void *)0;
...
static Point *genRound(Agnode_t * n, int *sidep, float xm, float ym)
{
    ...
    if (sides < 3)
	sides = 20;
    verts = (Point*)gmalloc((sides)*sizeof(Point));
    for (i = 0; i < sides; i++) {
	verts[i].x =
	    ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->width) / 2.0 + xm) * cos(i / (double) sides * 3.14159265358979323846 * 2.0);
	verts[i].y =
	    ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->height) / 2.0 + ym) * sin(i / (double) sides * 3.14159265358979323846 * 2.0);
    }
    *sidep = sides;
    return verts;
}
...
int makeAddPoly(Poly * pp, Agnode_t * n, float xmargin, float ymargin)
{
	...
	Point b;
	sides = 4;
	b.x = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->width) / 2.0 + xmargin;
	b.y = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->height) / 2.0 + ymargin;
	pp->kind = 1;
	verts = (Point*)gmalloc((sides)*sizeof(Point));
	((verts[0]).x=(b.x),(verts[0]).y=(b.y));
	((verts[1]).x=(-b.x),(verts[1]).y=(b.y));
	((verts[2]).x=(-b.x),(verts[2]).y=(-b.y));
	((verts[3]).x=(b.x),(verts[3]).y=(-b.y));
    } else
	switch (shapeOf(n)) {
	case SH_POLY:
	    ...
	    if ((*((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape)->name)==*("box")&&!strcmp((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape)->name,"box")))
		pp->kind = 1;
	    else if ((*((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape)->name)==*("polygon")&&!strcmp((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape)->name,"polygon"))
		...
		if (pp->kind == 1) {
			 ...
			 */
		    verts[0].x = ((poly->vertices[0].x)/(double)72) + xmargin;
		    verts[0].y = ((poly->vertices[0].y)/(double)72) + ymargin;
		    verts[1].x = ((poly->vertices[1].x)/(double)72) - xmargin;
		    verts[1].y = ((poly->vertices[1].y)/(double)72) + ymargin;
		    verts[2].x = ((poly->vertices[2].x)/(double)72) - xmargin;
		    verts[2].y = ((poly->vertices[2].y)/(double)72) - ymargin;
		    verts[3].x = ((poly->vertices[3].x)/(double)72) + xmargin;
		    verts[3].y = ((poly->vertices[3].y)/(double)72) - ymargin;
 
		}
		else {
		    for (i = 0; i < sides; i++) {
                        double h = (sqrt(((((poly->vertices[i].x)) * ((poly->vertices[i].x))) + (((poly->vertices[i].y)) * ((poly->vertices[i].y))))));
		        verts[i].x = poly->vertices[i].x * (1.0 + xmargin/h);
		        verts[i].y = poly->vertices[i].y * (1.0 + ymargin/h);
		        verts[i].x = ((verts[i].x)/(double)72);
		        verts[i].y = ((verts[i].y)/(double)72);
		    }
		}
	    } else
	...
	case SH_RECORD:
	    sides = 4;
	    verts = (Point*)gmalloc((sides)*sizeof(Point));
	    b = ((field_t *) (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape_info))->b;
	...
	default:
	    agerr(AGERR, "makeAddPoly: unknown shape type %s\n",
		  (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape)->name);
	    return 1;
	}
...
int makePoly(Poly * pp, Agnode_t * n, float xmargin, float ymargin)
{
	...
	Point b;
	sides = 4;
	b.x = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->width) / 2.0;
	b.y = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->height) / 2.0;
	pp->kind = 1;
	verts = (Point*)gmalloc((sides)*sizeof(Point));
	((verts[0]).x=(b.x),(verts[0]).y=(b.y));
	((verts[1]).x=(-b.x),(verts[1]).y=(b.y));
	...
	case SH_POLY:
		...
		verts = (Point*)gmalloc((sides)*sizeof(Point));
		for (i = 0; i < sides; i++) {
		    verts[i].x = ((poly->vertices[i].x)/(double)72);
		    verts[i].y = ((poly->vertices[i].y)/(double)72);
		}
	    } else
...
static int inPoly(Point vertex[], int n, Point q)
{
    ...
    if (tp3 == (void *)0)
	tp3 = (Point*)gmalloc((maxcnt)*sizeof(Point));
...
int polyOverlap(Point p, Poly * pp, Point q, Poly * qp)
{
    ...
    if (((pp)->kind & 1) && ((qp)->kind & 1))
	return 1;
    if (((pp)->kind & 2) && ((qp)->kind & 2)) {
	double d =
    ...
    if (tp1 == (void *)0) {
	tp1 = (Point*)gmalloc((maxcnt)*sizeof(Point));
	tp2 = (Point*)gmalloc((maxcnt)*sizeof(Point));
    }
