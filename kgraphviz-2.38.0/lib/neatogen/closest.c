...
{
    s->data = (Pair*)gmalloc((n)*sizeof(Pair));
    s->max_size = n;
    s->top = 0;
}
...
static void heapify(PairHeap * h, int i)
{
	...
	l = (2*(i));
	r = (2*(i)+1);
	if (((l)<h->heapSize) && (((h->data[l]).dist < (h->data[i]).dist) || ((((h->data[l]).dist == (h->data[i]).dist)) && (rand()%2))))
	    largest = l;
	else
	    largest = i;
	if (((r)<h->heapSize) && (((h->data[r]).dist < (h->data[largest]).dist) || ((((h->data[r]).dist == (h->data[largest]).dist)) && (rand()%2))))
	    largest = r;
	if (largest == i)
	    break;

	{Pair temp; temp=h->data[largest]; h->data[largest]=h->data[i]; h->data[i]=temp; };
	i = largest;
    }
}
...
static void initHeap(PairHeap * h, double *place, int *ordering, int n)
{
    ...
    h->data = (Pair*)gmalloc((h->maxSize)*sizeof(Pair));
...
static void insert(PairHeap * h, Pair edge)
{
    ...
    h->heapSize++;
    h->data[i] = edge;
    while (i > 0 && (((h->data[i]).dist < (h->data[((i)/2)]).dist) || ((((h->data[i]).dist == (h->data[((i)/2)]).dist)) && (rand()%2)))) {
	{Pair temp; temp=h->data[i]; h->data[i]=h->data[((i)/2)]; h->data[((i)/2)]=temp; };
	i = ((i)/2);
    }
}
		   ...
		   PairStack * pairs_stack)
{
    ...
    int i;
    PairHeap heap;
    int *left= (int*)gmalloc((n)*sizeof(int));
    int *right= (int*)gmalloc((n)*sizeof(int));
    Pair pair = { 0, 0 }, new_pair;

    /* Order the nodes according to their place */
    int *ordering = (int*)gmalloc((n)*sizeof(int));
    int *inv_ordering = (int*)gmalloc((n)*sizeof(int));
	...
	{ if (pairs_stack->top>=pairs_stack->max_size) { pairs_stack->max_size *= 2; pairs_stack->data = (Pair*) realloc(pairs_stack->data, pairs_stack->max_size*sizeof(Pair)); } pairs_stack->data[pairs_stack->top++] = pair; };
...
construct_graph(int n, PairStack * edges_stack, vtx_data ** New_graph)
{
    ...
    int *degrees = (int*)gmalloc((n)*sizeof(int));
    int top = edges_stack->top;
    int new_nedges = 2 * top + n;
    Pair pair;
    int *edges = (int*)gmalloc((new_nedges)*sizeof(int));
    float *weights = (float*)gmalloc((new_nedges)*sizeof(float));

    for (i = 0; i < n; i++) {
	degrees[i] = 1;		/* save place for the self loop */
    }
    for (i = 0; i < top; i++) {
	pair = (edges_stack->data[i]);
	degrees[pair.left]++;
	degrees[pair.right]++;
    }
    ...
    *New_graph = new_graph = (vtx_data*)gmalloc((n)*sizeof(vtx_data));
    for (i = 0; i < n; i++) {
	...
	*weights = 0;		/* self loop weight for Lap */
    ...
    while (((edges_stack->top==0) ? 0 : (edges_stack->top--, pair = edges_stack->data[edges_stack->top], (!(0))))) {
