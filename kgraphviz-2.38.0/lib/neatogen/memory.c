...
void freeinit(Freelist * fl, int size)
{
    ...
    fl->head = 0;
    fl->nodesize = ((size)%(sizeof(Freenode)) == 0 ? (size) : (sizeof(Freenode))%(size) == 0 ? (sizeof(Freenode)) : size*(sizeof(Freenode)/gcd(size,sizeof(Freenode))));
    if (fl->blocklist != 0) {
	Freeblock *bp, *np;
...
void *getfree(Freelist * fl)
{
	...
	mem = (Freeblock*)gmalloc(sizeof(Freeblock));
