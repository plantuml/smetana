...
static void sgnarea(vertex *l, vertex *m, int i[])
{
    ...
    a = l->pos.x;
    b = l->pos.y;
    c = (((l)==((l)->poly->finish))?((l)->poly->start):((l)+1))->pos.x - a;
    d = (((l)==((l)->poly->finish))?((l)->poly->start):((l)+1))->pos.y - b;
    e = m->pos.x - a;
    f = m->pos.y - b;
    g = (((m)==((m)->poly->finish))?((m)->poly->start):((m)+1))->pos.x - a;
    h = (((m)==((m)->poly->finish))?((m)->poly->start):((m)+1))->pos.y - b;
    t = (c * f) - (d * e);
    i[0] = ((t == 0) ? 0 : (t > 0 ? 1 : -1));
    t = (c * h) - (d * g);
    i[1] = ((t == 0) ? 0 : (t > 0 ? 1 : -1));
...
static int online(vertex *l, vertex *m, int i)
{
    pointf a, b, c;
    a = l->pos;
    b = (((l)==((l)->poly->finish))?((l)->poly->start):((l)+1))->pos;
    c = (i == 0) ? m->pos : (((m)==((m)->poly->finish))?((m)->poly->start):((m)+1))->pos;
...
static int intpoint(vertex *l, vertex *m, double *x, double *y, int cond)
{
	...
	return (0);
    ls = l->pos;
    le = (((l)==((l)->poly->finish))?((l)->poly->start):((l)+1))->pos;
    ms = m->pos;
    me = (((m)==((m)->poly->finish))?((m)->poly->start):((m)+1))->pos;
...
putSeg (int i, vertex* v)
{
	...
	i, v->pos.x, v->pos.y, (((v)==((v)->poly->finish))?((v)->poly->start):((v)+1))->pos.x, (((v)==((v)->poly->finish))?((v)->poly->start):((v)+1))->pos.y);
}
...
realIntersect (vertex *firstv, vertex *secondv, pointf p)
{
    ...
    vft = firstv->pos;
    avft = (((firstv)==((firstv)->poly->finish))?((firstv)->poly->start):((firstv)+1))->pos;
    vsd = secondv->pos;
    avsd = (((secondv)==((secondv)->poly->finish))?((secondv)->poly->start):((secondv)+1))->pos;
	...
	((vsd.x == avsd.x) &&
	 !(((vsd).x == (p).x) && ((vsd).y == (p).y)) && !(((avsd).x == (p).x) && ((avsd).y == (p).y)))) 
    {
	if (Verbose > 1) {
		  ...
		  intersection* ilist, data *input)
{
	...
	if (i[2] > 0)
	    return 0;
	if (!intpoint
	    (l, m, &x, &y, (i[2] < 0) ? 3 : online(m, l, ((i[0]) >= 0 ? (i[0]) : -(i[0])))))
	    return 0;
    }

    else if (!intpoint(l, m, &x, &y, (i[0] == i[1]) ?
		       2 * ((online(l, m, 0))>(
 online(l, m, 1))?(online(l, m, 0)):(
 online(l, m, 1))) : online(l, m, ((i[0]) >= 0 ? (i[0]) : -(i[0])))))
	return 0;
	  ...
	  data *input, intersection ilist[])
{
    ...
    pvertex = (vertex **)gmalloc((input->nvertices)*sizeof(vertex *));
...
/* walk through the vertices in order of increasing x coordinate	*/
    for (i = 0; i < input->nvertices; i++) {
	pt1 = pvertex[i];
	templ = pt2 = (((pvertex[i])==((pvertex[i])->poly->start))?((pvertex[i])->poly->finish):((pvertex[i])-1));
	for (k = 0; k < 2; k++) {	/* each vertex has 2 edges */
		...
		new = (active_edge*)gmalloc(sizeof(active_edge));
		if (all.number == 0) {
		    all.first = new;
		    new->last = 0;
		} /* insert */
	    ...
	    pt2 = (((pvertex[i])==((pvertex[i])->poly->finish))?((pvertex[i])->poly->start):((pvertex[i])+1));
...
findInside(Ppoly_t ** polys, int n_polys, polygon* polygon_list)
{
	...
	for (j = i+1; j < n_polys; j++) {
	    p2 = polys[j];
	    if ((((polygon_list[i].bb.LL.x <= polygon_list[j].bb.UR.x) && (polygon_list[i].bb.LL.x >= polygon_list[j].bb.LL.x) && (polygon_list[i].bb.LL.y <= polygon_list[j].bb.UR.y) && (polygon_list[i].bb.LL.y >= polygon_list[j].bb.LL.y)) && ((polygon_list[i].bb.UR.x <= polygon_list[j].bb.UR.x) && (polygon_list[i].bb.UR.x >= polygon_list[j].bb.LL.x) && (polygon_list[i].bb.UR.y <= polygon_list[j].bb.UR.y) && (polygon_list[i].bb.UR.y >= polygon_list[j].bb.LL.y)))) {
		if (in_poly(*p2, pt)) return 1;
	    }
	    else if ((((polygon_list[j].bb.LL.x <= polygon_list[i].bb.UR.x) && (polygon_list[j].bb.LL.x >= polygon_list[i].bb.LL.x) && (polygon_list[j].bb.LL.y <= polygon_list[i].bb.UR.y) && (polygon_list[j].bb.LL.y >= polygon_list[i].bb.LL.y)) && ((polygon_list[j].bb.UR.x <= polygon_list[i].bb.UR.x) && (polygon_list[j].bb.UR.x >= polygon_list[i].bb.LL.x) && (polygon_list[j].bb.UR.y <= polygon_list[i].bb.UR.y) && (polygon_list[j].bb.UR.y >= polygon_list[i].bb.LL.y)))) {
...
int Plegal_arrangement(Ppoly_t ** polys, int n_polys)
{
    ...
    polygon *polygon_list;
    data input;
    intersection ilist[10000];
    boxf bb;
    double x, y;

    polygon_list = (polygon*)gmalloc((n_polys)*sizeof(polygon));

    for (i = nverts = 0; i < n_polys; i++)
	nverts += polys[i]->pn;

    vertex_list = (vertex*)gmalloc((nverts)*sizeof(vertex));
	...
	for (j = 0; j < polys[i]->pn; j++) {
	    x = polys[i]->ps[j].x;
	    y = polys[i]->ps[j].y;
	    bb.LL.x = ((bb.LL.x)<(x)?(bb.LL.x):(x));
	    bb.LL.y = ((bb.LL.y)<(y)?(bb.LL.y):(y));
	    bb.UR.x = ((bb.UR.x)>(x)?(bb.UR.x):(x));
	    bb.UR.y = ((bb.UR.y)>(y)?(bb.UR.y):(y));
