...
static DistType **compute_apsp_dijkstra(vtx_data * graph, int n)
{
    ...
    storage = (DistType*)gmalloc((n * n)*sizeof(DistType));
    dij = (DistType **)gmalloc((n)*sizeof(DistType *));
    for (i = 0; i < n; i++)
	dij[i] = storage + i * n;
...
static DistType **compute_apsp_simple(vtx_data * graph, int n)
{
    ...
    int i;
    DistType *storage = (int*)gmalloc((n * n)*sizeof(int));
    DistType **dij;
    Queue Q;
...
void compute_new_weights(vtx_data * graph, int n)
{
    ...
    int nedges = 0;
    float *weights;
    int *vtx_vec = (int*)gmalloc((n)*sizeof(int));
    int deg_i, deg_j, neighbor;

    for (i = 0; i < n; i++) {
	nedges += graph[i].nedges;
    }
    weights = (float*)gmalloc((nedges)*sizeof(float));
