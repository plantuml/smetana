...
v_data *UG_graph(double *x, double *y, int n, int accurate_computation)
{
    ...
    if (n == 2) {
	int *edges = (int*)gmalloc((4)*sizeof(int));
	delaunay = (v_data*)gmalloc((n)*sizeof(v_data));
	delaunay[0].ewgts = (void *)0;
	delaunay[0].edges = edges;
	delaunay[0].nedges = 2;
	delaunay[0].edges[0] = 0;
	delaunay[0].edges[1] = 1;
	delaunay[1].edges = edges + 2;
	delaunay[1].ewgts = (void *)0;
	delaunay[1].nedges = 2;
	delaunay[1].edges[0] = 1;
	delaunay[1].edges[1] = 0;
	return delaunay;
    } else if (n == 1) {
	int *edges = (int*)gmalloc((1)*sizeof(int));
	delaunay = (v_data*)gmalloc((n)*sizeof(v_data));
	delaunay[0].ewgts = (void *)0;
	delaunay[0].edges = edges;
	delaunay[0].nedges = 1;
