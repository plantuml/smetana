...
PCA_alloc(DistType ** coords, int dim, int n, double **new_coords,
	  int new_dim)
{
    double **DD = (void *)0;		/* dim*dim matrix: coords*coords^T */
    double sum;
    int i, j, k;
    double **eigs = (void *)0;
    double *evals = (void *)0;
    ...
    eigs = (double **)gmalloc((new_dim)*sizeof(double *));
    for (i = 0; i < new_dim; i++)
	eigs[i] = (double*)gmalloc((dim)*sizeof(double));
    evals = (double*)gmalloc((new_dim)*sizeof(double));

    DD = (double **)gmalloc((dim)*sizeof(double *));
    storage_ptr = (double*)gmalloc((dim * dim)*sizeof(double));
    for (i = 0; i < dim; i++) {
	DD[i] = storage_ptr;
	storage_ptr += dim;
    }
    ...
    power_iteration(DD, dim, new_dim, eigs, evals, (!(0)));
...
iterativePCA_1D(double **coords, int dim, int n, double *new_direction)
{
    ...
    /* Compute direction */
    return power_iteration(mat, dim, 1, &new_direction, &eval, (!(0)));
/* ?? When is mat freed? */
}
