...
static void heapify(heap * h, int i, int index[], Word dist[])
{
	...
	l = (2*(i));
	r = (2*(i)+1);
	if (((l)<h->heapSize) && (dist[h->data[l]]<dist[h->data[i]]))
	    largest = l;
	else
	    largest = i;
	if (((r)<h->heapSize) && (dist[h->data[r]]<dist[h->data[largest]]))
	    largest = r;

	if (largest == i)
	    break;

	{int temp; temp=h->data[largest]; h->data[largest]=h->data[i]; h->data[i]=temp; index[h->data[largest]]=largest; index[h->data[i]]=i; };
	i = largest;
    }
}
...
initHeap(heap * h, int startVertex, int index[], Word dist[], int n)
{
    ...
    if (n == 1) h->data = (void *)0;
    else h->data = (int*)gmalloc((n - 1)*sizeof(int));
    h->heapSize = n - 1;
	    ...
	    Word dist[])
{
    ...
    i = placeInHeap;
    while (i > 0 && dist[h->data[((i)/2)]] > newDist) {	/* can write here: greaterPriority(i,parent(i),dist) */
	{h->data[i]=h->data[((i)/2)]; index[h->data[i]]=i;};
	i = ((i)/2);
    }
...
void dijkstra(int vertex, vtx_data * graph, int n, DistType * dist)
{
    ...
    DistType closestDist, prevClosestDist = ((int)(~(unsigned)0 >> 1));
    static int *index;
    ...
    for (i = 0; i < n; i++)
	dist[i] = (DistType) (double)((int)(~(unsigned)0 >> 1));
    dist[vertex] = 0;
	...
	if (closestDist == (double)((int)(~(unsigned)0 >> 1)))
	    break;
    ...
    for (i = 0; i < n; i++)
	if (dist[i] == (double)((int)(~(unsigned)0 >> 1)))	/* 'i' is not connected to 'vertex' */
 ...
 /* MON-EFFICIENT implementation, see below. */
{
    ...
    int i;
    static boolean *node_in_neighborhood = (void *)0;
    static int size = 0;
    static int *index;
    Queue Q;
    heap H;
	...
	for (i = size; i < n; i++) {
	    node_in_neighborhood[i] = 0;
	}
	size = n;
    }
    for (i = 0; i < num_visited_nodes; i++) {
	node_in_neighborhood[visited_nodes[i]] = (!(0));
    }
...
initHeap_f(heap * h, int startVertex, int index[], float dist[], int n)
{
    int i, count;
    int j;			/* We cannot use an unsigned value in this loop */
    h->data = (int*)gmalloc((n - 1)*sizeof(int));
    h->heapSize = n - 1;
...
void dijkstra_f(int vertex, vtx_data * graph, int n, float *dist)
{
    ...
    index = (int*)gmalloc((n)*sizeof(int));

    /* initial distances with edge weights: */
    for (i = 0; i < n; i++)
	dist[i] = ((float)3.40282347e+38);
    dist[vertex] = 0;
    for (i = 1; i < graph[vertex].nedges; i++)
	...
	closestDist = dist[closestVertex];
	if (closestDist == ((float)3.40282347e+38))
	    break;
	for (i = 1; i < graph[closestVertex].nedges; i++) {
	    neighbor = graph[closestVertex].edges[i];
