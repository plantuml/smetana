...
*/
    ...
    int i, j;
    int node;
    DistType *storage = (DistType*)gmalloc((n * dim)*sizeof(DistType));
    DistType **coords = *Coords;
    DistType *dist = (DistType*)gmalloc((n)*sizeof(DistType));	/* this vector stores  the distances of
    ...
    *Coords = coords = (DistType **)gmalloc((dim)*sizeof(DistType *));
    for (i = 0; i < dim; i++)
	...
	max_dist = 0;
	for (j = 0; j < n; j++) {
	    dist[j] = ((dist[j])<(coords[i][j])?(dist[j]):(coords[i][j]));
	    if (dist[j] > max_dist) {
		node = j;
		max_dist = dist[j];
	    }
