#line 1 "graphviz-2.38.0\\lib\\cgraph\\cmpnd.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\cgraph\\cmpnd.c" 2
				   ...
				   Agnode_t * node_being_saved)
{
	...
	(Agcmpedge_t *) agbindrec(e, Descriptor_id, sizeof(*edgerec),
				  (0));
    if (node_being_saved == ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node))
	i = 0;
    else
	i = 1;
...
static void splice(Agobj_t * obj, void *arg)
{
    ...
    opp = ((((((Agobj_t*)(e))->tag).objtype)==3)?((e)-1):((e)+1));
    spl = arg;
    target = spl->target;
...
int agsplice(Agedge_t * e, Agnode_t * target)
{
    ...
    root = agroot(g);
    agapply(root, (Agobj_t *) e, splice, &splice_arg, (!(0)));
    return 0;
}
...
int agassociate(Agnode_t * n, Agraph_t * sub)
{
    ...
    if (agsubnode(sub, n, (0)))
	return -1;		/* avoid cycles */
    noderec = agbindrec(n, Descriptor_id, sizeof(*noderec), (0));
    graphrec = agbindrec(sub, Descriptor_id, sizeof(*graphrec), (0));
				...
				Agraph_t * subg)
{
    ...
    if ((g != subg) && (n = agsubnode(g, (Agnode_t *) node, (0)))) {
	(*(((Dt_t*)(g->n_dict))->searchf))((g->n_dict),(void*)(n),0000002);

	graphrec = agbindrec(g, Descriptor_id, sizeof(*graphrec), (0));
	if ((d = graphrec->hidden_node_set) == ((Dict_t *)0)) {
	    /* use name disc. to permit search for hidden node by name */
	    d = graphrec->hidden_node_set
		= agdtopen(g, &Ag_node_name_disc, Dttree);
	}
	(*(((Dt_t*)(d))->searchf))((d),(void*)(n),0000001);
...
int aghide(Agnode_t * cmpnode)
{
    ...
    if (agcmpgraph_of(cmpnode) == ((Agraph_t*)0))
	return -1;
    noderec = (Agcmpnode_t *) aggetrec(cmpnode, Descriptor_id, (0));

    subg = noderec->subg;
    root = agroot(g);

    /* make sure caller hasn't put a node "inside itself" */
    if ((n = agsubnode(subg, cmpnode, (0))))
	agdelnode(n);

    /* remap edges by splicing and saving previous endpt */
    for (n = agfstnode(subg); n; n = agnxtnode(n)) {
	rootn = agsubnode(root, n, (0));
				...
				Agraph_t * subg)
{
    ...
    if ((g != subg)
	&& ((n = agsubnode(g, (Agnode_t *) node, (0))) == ((Agnode_t*)0))) {
	graphrec = (Agcmpgraph_t *) aggetrec(g, Descriptor_id, (0));
	if (graphrec
	    &&
	    ((n = (Agnode_t *) (*(((Dt_t*)(graphrec->hidden_node_set))->searchf))((graphrec->hidden_node_set),(void*)(node),0000004))))
	    (*(((Dt_t*)(g->n_dict))->searchf))((g->n_dict),(void*)(n),0000001);
...
int agexpose(Agnode_t * cmpnode)
{
    ...
    noderec = (Agcmpnode_t *) aggetrec(cmpnode, Descriptor_id, (0));
    if ((noderec == ((Agcmpnode_t *)0) || (!(noderec->collapsed))))
	return -1;
    ...
    root = agroot(g);
    rootcmp = agsubnode(root, cmpnode, (0));
    for (e = agfstedge(rootcmp); e; e = f) {
	f = agnxtedge(e, rootcmp);
	if ((edgerec = (Agcmpedge_t *) aggetrec(e, Descriptor_id, (0)))) {
	    /* anything interesting on either stack? */
	    for (i = 0; i < 2; i++) {
		stk = &(edgerec->stack[i]);
		sav = stacktop(stk);
		if (sav.to && (((((Agobj_t*)(sav.to))->tag).id) == ((((Agobj_t*)(cmpnode))->tag).id))) {
		    if (e->node != sav.to)
			e = ((((((Agobj_t*)(e))->tag).objtype)==3)?((e)-1):((e)+1));
		    agsplice(e, sav.from);
		    stackpop(stk);
		    continue;
		}
...
{
    Agcmpnode_t *noderec;
    noderec = (Agcmpnode_t *) aggetrec(n, Descriptor_id, (0));
...
Agnode_t *agfindhidden(Agraph_t * g, char *name)
{
    ...
    if (graphrec) {
	key.name = name;
	return (Agnode_t *) (*(((Dt_t*)(graphrec->hidden_node_set))->searchf))((graphrec->hidden_node_set),(void*)(&key),0000004);
    } else
	return ((Agnode_t*)0);
}
