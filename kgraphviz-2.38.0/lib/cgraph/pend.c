#line 1 "graphviz-2.38.0\\lib\\cgraph\\pend.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\cgraph\\pend.c" 2
...
static Dtdisc_t Disc = {
    ((int)(&(((pending_cb_t*)0)->key))),	/* sort by 'key' */
...
static pending_cb_t *lookup(Dict_t * dict, Agobj_t * obj)
{
    ...
    key.key = genkey(obj);
    rv = (pending_cb_t *) (*(((Dt_t*)(dict))->searchf))((dict),(void*)(&key),0000004);
    return rv;
}
			    ...
			    Agsym_t * optsym)
{
    ...
    (*(((Dt_t*)(dict))->searchf))((dict),(void*)(handle),0000001);
    return handle;
}
...
static void purge(Dict_t * dict, Agobj_t * obj)
{
	...
	(*(((Dt_t*)(dict))->searchf))((dict),(void*)(handle),0000002);
    }
}
...
static void cb(Dict_t * dict, int callback_kind)
{
    ...
    if (dict)
	while ((pcb = (pending_cb_t *) (*(((Dt_t*)(dict))->searchf))((dict),(void*)(0),0000200))) {
	    g = pcb->g;
	    stack = g->clos->cb;
	    ...
	    (*(((Dt_t*)(dict))->searchf))((dict),(void*)(pcb),0000002);
	}
}
...
static void agrelease_callbacks(Agraph_t * g)
{
	...
	g->clos->callbacks_enabled = (!(0));
	pending =
	    (pendingset_t *) agbindrec(g, DRName, sizeof(pendingset_t),
				       (0));
	/* this destroys objects in the opposite of their order of creation */
	cb(pending->ins.g, 100);
	cb(pending->ins.n, 100);
	cb(pending->ins.e, 100);

	cb(pending->mod.g, 101);
	cb(pending->mod.n, 101);
	cb(pending->mod.e, 101);

	cb(pending->del.e, 102);
	cb(pending->del.n, 102);
	cb(pending->del.g, 102);
    }
}
