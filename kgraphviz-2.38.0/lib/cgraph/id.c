#line 1 "graphviz-2.38.0\\lib\\cgraph\\id.c" 1
/* $Id$ $Revision$ */
...
#line 17 "graphviz-2.38.0\\lib\\cgraph\\id.c" 2
...
int agallocid(Agraph_t * g, int objtype, unsigned long request)
{
    return ((g)->clos->disc.id)->alloc(((g)->clos->state.id), objtype, request);
}

void agfreeid(Agraph_t * g, int objtype, unsigned long id)
{
    (void) aginternalmapdelete(g, objtype, id);
    (((g)->clos->disc.id)->free) (((g)->clos->state.id), objtype, id);
}
