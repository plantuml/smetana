#line 1 "graphviz-2.38.0\\lib\\sparse\\IntStack.c" 1
/* $Id$Revision: */
...
#line 16 "graphviz-2.38.0\\lib\\sparse\\IntStack.c" 2
...
#line 17 "graphviz-2.38.0\\lib\\sparse\\IntStack.c" 2

IntStack IntStack_new(void){
  IntStack s;
  int max_len = 1<<5;

  s = gmalloc(sizeof(struct IntStack_struct));
  s->max_len = max_len;
  s->last = -1;
  s->stack = gmalloc(sizeof(int)*max_len);
  return s;
}
...
static IntStack IntStack_realloc(IntStack s){
  ...
  max_len = max_len + ((10)>(0.2*max_len)?(10):(0.2*max_len));
  s->max_len = max_len;
  s->stack = grealloc(s->stack, sizeof(int)*max_len);
  if (!s->stack) return (void *)0;
  return s;
}
