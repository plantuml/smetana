#line 1 "graphviz-2.38.0\\lib\\sparse\\vector.c" 1
/* $Id$Revision: */
...
#line 15 "graphviz-2.38.0\\lib\\sparse\\vector.c" 2
...
#line 16 "graphviz-2.38.0\\lib\\sparse\\vector.c" 2
...
Vector Vector_add(Vector v, void *stuff){
  if (v->len + 1 >= v->maxlen){
    v->maxlen = v->maxlen + (((int) .2*(v->maxlen))>(10)?((int) .2*(v->maxlen)):(10));
...
StringVector StringVector_part(StringVector v, int n, int *selected_list){
  ...
  char *s, *s2;
  int i;
  u = StringVector_new(1, (!(0)));
  for (i = 0; i < n; i++){
    s = *(StringVector_get(v, selected_list[i]));
    s2 = gmalloc(sizeof(char)*(strlen(s)+1));
    strcpy(s2, s);
