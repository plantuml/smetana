#line 1 "graphviz-2.38.0\\lib\\sparse\\general.c" 1
/* $Id$Revision: */
...
#line 16 "graphviz-2.38.0\\lib\\sparse\\general.c" 2
#line 1 "graphviz-2.38.0\\lib\\sparse\\general.c" 1
...
void vector_take(int n, double *v, int m, int *p, double **u){
  ...
  if (!*u) *u = gmalloc(sizeof(double)*m);
...
void vector_ordering(int n, double *v, int **p, int ascending){
  ...
  */
  ...
  if (!*p) *p = gmalloc(sizeof(int)*n);
  u = gmalloc(sizeof(double)*2*n);
  ...
  if (ascending){
    ...
    qsort(u, n, sizeof(double)*2, comp_descend);
  }
...
void vector_sort_real(int n, double *v, int ascending){
  if (ascending){
    qsort(v, n, sizeof(double), comp_ascend);
  } else {
    qsort(v, n, sizeof(double), comp_descend);
  }
}
void vector_sort_int(int n, int *v, int ascending){
  if (ascending){
...
double distance_cropped(double *x, int dim, int i, int j){
  ...
  dist = sqrt(dist);
  return ((dist)>(1.e-15)?(dist):(1.e-15));
}
...
void scale_to_box(double xmin, double ymin, double xmax, double ymax, int n, int dim, double *x){
  ...
  for (i = 0; i < n; i++){
    for (k = 0; k < dim; k++) {
      min[k] = ((x[i*dim+k])<(min[k])?(x[i*dim+k]):(min[k]));
      max[k] = ((x[i*dim+k])>(max[k])?(x[i*dim+k]):(max[k]));
    }
  }
  ...
  if (max[1] - min[1] != 0) {
    ratio = ((ratio)<((ymax-ymin)/(max[1] - min[1]))?(ratio):((ymax-ymin)/(max[1] - min[1])));
  }
...
int validQ_int_string(char *to_convert, int *v){
  ...
  if (val > ((int)(~(unsigned)0 >> 1)) || val < (-((int)(~(unsigned)0 >> 1)) - 1)) return 0;
  *v = (int) val;
  return 1;
}
