#line 1 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 1
/* $Id$Revision: */
...
#line 31 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
#line 32 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
#line 33 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
#line 34 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
#line 35 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
#line 36 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
#line 37 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
typedef struct {
    int d;
    Agedge_t* e;
} epair_t;
...
{
    pointf p;
    p.x = (((cp->bb.LL.x)+(cp->bb.UR.x))/2.0);
    p.y = (((cp->bb.LL.y)+(cp->bb.UR.y))/2.0);
    return p;
}
...
sidePt (snode* ptr, cell* cp)
{
	...
	if (ptr->isVert) {
	    pt.x = cp->bb.LL.x;
	    pt.y = (((cp->bb.LL.y)+(cp->bb.UR.y))/2.0);
	}
	else {
	    pt.x = (((cp->bb.LL.x)+(cp->bb.UR.x))/2.0);
	    pt.y = cp->bb.LL.y;
	}
    }
    else {
...
convertSPtoRoute (sgraph* g, snode* fst, snode* lst)
{
    ...
    for (ptr = fst; ptr; ptr = (ptr)->n_dad) sz++;
    rte.n = 0;
    rte.segs = (segment*)zmalloc((sz-2)*sizeof(segment));  /* at most sz-2 segments */

    seg.prev = seg.next = 0;
    ptr = prev = (fst)->n_dad;
    next = (ptr)->n_dad;
    if ((ptr->cells[0]->flags & 1))
	cp = ptr->cells[1];
    else
	cp = ptr->cells[0];
    bp1 = sidePt (ptr, cp);
    while ((next)->n_dad!=NULL) {
	ncp = cellOf (prev, next);
	updateWts (g, ncp, (ptr)->n_edge);

        /* add seg if path bends or at end */
	if ((ptr->isVert != next->isVert) || ((next)->n_dad == lst)) {
	    if (ptr->isVert != next->isVert)
		bp2 = midPt (ncp);
	    else
		bp2 = sidePt(next, ncp);
	    if (ptr->isVert) {   /* horizontal segment */
		if (ptr == (fst)->n_dad) l1 = B_NODE;
...
{
    chanItem* subd = (*(((Dt_t*)(chdict))->searchf))((chdict),(void*)(&j),0001000);

    if (!subd) {
	subd = (chanItem*)zmalloc(sizeof(chanItem));
	subd->v = j;
	subd->chans = dtopen (&chanDisc, Dtoset);
	(*(((Dt_t*)(chdict))->searchf))((chdict),(void*)(subd),0000001);
    }
    (*(((Dt_t*)(subd->chans))->searchf))((subd->chans),(void*)(cp),0000001);
}
...
extractHChans (maze* mp)
{
	...
	chp = (channel*)zmalloc(sizeof(channel));
	chp->cp = cp;
	chp->p.p1 = cp->bb.LL.x;
...
{
    seg->ind_no = chan->cnt++;
    chan->seg_list = (chan->seg_list? (segment**)grealloc(chan->seg_list,(chan->cnt)*sizeof(segment*)):(segment**)gmalloc((chan->cnt)*sizeof(segment*)));
...
{
  channel* cp;
  chanItem* chani = (*(((Dt_t*)(chans))->searchf))((chans),(void*)(&seg->comm_coord),0001000);
  assert (chani);
  cp = (*(((Dt_t*)(chani->chans))->searchf))((chani->chans),(void*)(&seg->p),0001000);
  assert (cp);
  return cp;
}
...
#line 474 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
...
dumpChanG (channel* cp, int v)
{
  ...
  for (k=0;k<cp->cnt;k++) {
    ...
    fputs (" ->\n", stderr);
    for (ip = (intitem*)(*(((Dt_t*)(adj))->searchf))((adj),(void*)(0),0000200); ip; ip = (intitem*)(*(((Dt_t*)(adj))->searchf))((adj),(void*)(ip),0000010)) {
      fputs ("     ", stderr);
...
assignTrackNo (Dt_t* chans)
{
    ...
    for (l1 = dtflatten (chans); l1; l1 = (((Dtlink_t*)(l1))->right)) {
	lp = ((chanItem*)l1)->chans;
	for (l2 = dtflatten (lp); l2; l2 = (((Dtlink_t*)(l2))->right)) {
	    cp = (channel*)l2;
	    if (cp->cnt) {

    if (odb_flags & 8) dumpChanG (cp, ((chanItem*)l1)->v);
...
attachOrthoEdges (Agraph_t* g, maze* mp, int n_edges, route* route_list, splineInfo *sinfo, epair_t es[], int doLbls)
{
	...
	e = es[irte].e;
	p1 = addPoints((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->coord), (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->tail_port).p);
	q1 = addPoints((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->coord), (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->head_port).p);

	rte = route_list[irte];
	npts = 1 + 3*rte.n;
	if (npts > splsz) {
		if (ispline) free (ispline);
		ispline = (pointf*)gmalloc((npts)*sizeof(pointf));
		splsz = npts;
	}
	...
	clip_and_install(e, ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node), ispline, npts, sinfo);
	if (doLbls && (lbl = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->label)) && !lbl->set)
...
edgeLen (Agedge_t* e)
{
    pointf p = (((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->coord);
    pointf q = (((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->coord);
...
orthoEdges (Agraph_t* g, int doLbls)
{
    ...
    snode* sn;
    snode* dn;
    epair_t* es = (epair_t*)gmalloc((agnedges(g))*sizeof(epair_t));
    cell* start;
    cell* dest;
    PointSet* ps;
    textlabel_t* lbl;
	...
	if (s && (*s != '\0')) {
		...
		switch (c) {
		case 'c' :
		    odb_flags |= 8;     // emit channel graph 
		    break;
		case 'i' :
		    odb_flags |= (2|16);  // emit search graphs
		    break;
		case 'm' :
		    odb_flags |= 1;      // emit maze
		    break;
		case 'r' :
		    odb_flags |= 4;     // emit routes in maze
		    break;
		case 's' :
		    odb_flags |= 2;    // emit search graph 
		    break;
		}
	    }
	}
    ...
    if (odb_flags & 2) emitSearchGraph (stderr, sg);
    ...
    for (n = agfstnode (g); n; n = agnxtnode(g, n)) {
	    ...
	    if ((Nop == 2) && (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)) continue;
	    if (Concentrate) {
		int ti = ((((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->tag).seq);
		int hi = ((((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->tag).seq);
		if (ti <= hi) {
    ...
    route_list = (route*)zmalloc((n_edges)*sizeof(route));
	...
	if ((i > 0) && (odb_flags & 16)) emitSearchGraph (stderr, sg);

	e = es[i].e;
        start = ((cell*)(((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->alg));
        dest = ((cell*)(((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->alg));
    ...
    if (odb_flags & 4) emitGraph (stderr, mp, n_edges, route_list, es);
...
#line 1238 "graphviz-2.38.0\\lib\\ortho\\ortho.c" 2
/* #include <values.h> */
...
{
    int i, x, y;
    boxf n = ((cell*)(((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->alg))->bb;
    ...
    bb.LL.x = ((bb.LL.x)<(1*x)?(bb.LL.x):(1*x));
    bb.LL.y = ((bb.LL.y)<(1*y)?(bb.LL.y):(1*y));
    bb.UR.x = ((bb.UR.x)>(1*x)?(bb.UR.x):(1*x));
    bb.UR.y = ((bb.UR.y)>(1*y)?(bb.UR.y):(1*y));
    ...
    n = ((cell*)(((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->alg))->bb;
    if (seg->isVert) {
	x = vtrack(seg, m);
	y = (n.UR.y + n.LL.y)/2;
    }
...
emitGraph (FILE* fp, maze* mp, int n_edges, route* route_list, epair_t es[])
{
    ...
    absbb.LL.x = absbb.LL.y = 1.7976931348623157e+308;
    absbb.UR.x = absbb.UR.y = -1.7976931348623157e+308;
    ...
    for (i = 0; i < mp->ncells; i++) {
      ...
      absbb.LL.x = ((absbb.LL.x)<(bb.LL.x)?(absbb.LL.x):(bb.LL.x));
      absbb.LL.y = ((absbb.LL.y)<(bb.LL.y)?(absbb.LL.y):(bb.LL.y));
      absbb.UR.x = ((absbb.UR.x)>(bb.UR.x)?(absbb.UR.x):(bb.UR.x));
      absbb.UR.y = ((absbb.UR.y)>(bb.UR.y)?(absbb.UR.y):(bb.UR.y));
    }

    bbox.LL.x = absbb.LL.x + 10;
    bbox.LL.y = absbb.LL.y + 10;
    bbox.UR.x = absbb.UR.x + 10;
    bbox.UR.y = absbb.UR.y + 10;
