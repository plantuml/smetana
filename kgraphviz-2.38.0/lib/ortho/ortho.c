...
orthoEdges (Agraph_t* g, int doLbls)
{
	...
	e = es[i].e;
        start = ((cell*)ND_alg(agtail(e)));
        dest = ((cell*)ND_alg(aghead(e)));
...
{
    int i, x, y;
    boxf n = ((cell*)ND_alg(agtail(e)))->bb;
    ...
    bb.LL.x = MIN(bb.LL.x, 1*x);
    bb.LL.y = MIN(bb.LL.y, 1*y);
    bb.UR.x = MAX(bb.UR.x, 1*x);
    bb.UR.y = MAX(bb.UR.y, 1*y);
