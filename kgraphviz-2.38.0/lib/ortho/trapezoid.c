#line 1 "graphviz-2.38.0\\lib\\ortho\\trapezoid.c" 1
/* $Id$Revision: */
...
#line 23 "graphviz-2.38.0\\lib\\ortho\\trapezoid.c" 2
...
#line 24 "graphviz-2.38.0\\lib\\ortho\\trapezoid.c" 2
...
#line 25 "graphviz-2.38.0\\lib\\ortho\\trapezoid.c" 2
...
#line 26 "graphviz-2.38.0\\lib\\ortho\\trapezoid.c" 2
...
init_query_structure(int segnum, segment_t* seg, trap_t* tr, qnode_t* qs)
{
  ...
  tr[t4].hi.y = (double) (1<<30);
  tr[t4].hi.x = (double) (1<<30);
  tr[t3].lo.y = (double) -1* (1<<30);
  tr[t3].lo.x = (double) -1* (1<<30);
...
is_left_of (int segnum, segment_t* seg, pointf *v)
{
  ...
  if ((((&s->v1)->y > (&s->v0)->y + 1.0e-7) ? (!(0)) : (((&s->v1)->y < (&s->v0)->y - 1.0e-7) ? (0) : ((&s->v1)->x > (&s->v0)->x)))) /* seg. going upwards */
    {
      if ((fabs(s->v1.y - v->y) <= 1.0e-7))
	{
	  if (v->x < s->v1.x)
	    area = 1.0;
	  else
      ...
      else if ((fabs(s->v0.y - v->y) <= 1.0e-7))
	{
	  if (v->x < s->v0.x)
	    area = 1.0;
	  else
...
locate_endpoint (pointf *v, pointf *vo, int r, segment_t* seg, qnode_t* qs)
{
  ...
  switch (rptr->nodetype) {
    ...
    case 2:
      if ((((v)->y > (&rptr->yval)->y + 1.0e-7) ? (!(0)) : (((v)->y < (&rptr->yval)->y - 1.0e-7) ? (0) : ((v)->x > (&rptr->yval)->x)))) /* above */
	return locate_endpoint(v, vo, rptr->right, seg, qs);
      else if (((fabs((v)->y - (&rptr->yval)->y) <= 1.0e-7) && (fabs((v)->x - (&rptr->yval)->x) <= 1.0e-7))) /* the point is already */
	{			          /* inserted. */
	  if ((((vo)->y > (&rptr->yval)->y + 1.0e-7) ? (!(0)) : (((vo)->y < (&rptr->yval)->y - 1.0e-7) ? (0) : ((vo)->x > (&rptr->yval)->x)))) /* above */
	    return locate_endpoint(v, vo, rptr->right, seg, qs);
	  else 
    ...
    case 1:
      if (((fabs((v)->y - (&seg[rptr->segnum].v0)->y) <= 1.0e-7) && (fabs((v)->x - (&seg[rptr->segnum].v0)->x) <= 1.0e-7)) || 
	       ((fabs((v)->y - (&seg[rptr->segnum].v1)->y) <= 1.0e-7) && (fabs((v)->x - (&seg[rptr->segnum].v1)->x) <= 1.0e-7)))
	{
	  if ((fabs(v->y - vo->y) <= 1.0e-7)) /* horizontal segment */
	    {
	      if (vo->x < v->x)
...
add_segment (int segnum, segment_t* seg, trap_t* tr, qnode_t* qs)
{
  ...
  s = seg[segnum];
  if ((((&s.v1)->y > (&s.v0)->y + 1.0e-7) ? (!(0)) : (((&s.v1)->y < (&s.v0)->y - 1.0e-7) ? (0) : ((&s.v1)->x > (&s.v0)->x)))) /* Get higher vertex in v0 */
    {
      int tmp;
      tpt = s.v0;
      s.v0 = s.v1;
  ...
  if ((is_swapped) ? !inserted(segnum, seg, 2) :
       !inserted(segnum, seg, 1))     /* insert v0 in the tree */
    {
      int tmp_d;
  ...
  if ((is_swapped) ? !inserted(segnum, seg, 1) :
       !inserted(segnum, seg, 2))     /* insert v1 in the tree */
    {
      int tmp_d;
  ...
  while ((t > 0) && 
				...
				/* traverse from top to bot */
    {
      ...
      if (t == tfirst)
	tfirstr = tn;
      if (((fabs((&tr[t].lo)->y - (&tr[tlast].lo)->y) <= 1.0e-7) && (fabs((&tr[t].lo)->x - (&tr[tlast].lo)->x) <= 1.0e-7)))
	tlastr = tn;
	...
	{			/* Only one trapezoid below */
	  ...
	  if ((fabs(tr[t].lo.y - tr[tlast].lo.y) <= 1.0e-7) && 
	      (fabs(tr[t].lo.x - tr[tlast].lo.x) <= 1.0e-7) && tribot)
      ...
      else
	{
	  ...
	  i_d0 = i_d1 = (0);
	  if ((fabs(tr[t].lo.y - s.v0.y) <= 1.0e-7))
	    {
	      if (tr[t].lo.x > s.v0.x)
		i_d0 = (!(0));
	      else
  ...
  tfirstl = tfirst; 
  tlastl = tlast;
  merge_trapezoids(segnum, tfirstl, tlastl, 1, tr, qs);
  merge_trapezoids(segnum, tfirstr, tlastr, 2, tr, qs);

  seg[segnum].is_inserted = (!(0));
  return 0;
}
  ...
  trap_t* tr)
{
    ...
    QSIZE = 2*ntraps;
    TRSIZE = ntraps;
    qs = (qnode_t*)zmalloc((2*ntraps)*sizeof(qnode_t));
    q_idx = tr_idx = 1;
