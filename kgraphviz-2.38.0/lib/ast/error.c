#line 1 "graphviz-2.38.0\\lib\\ast\\error.c" 1
/* $Id$ $Revision$ */
...
#line 22 "graphviz-2.38.0\\lib\\ast\\error.c" 2
...
#line 23 "graphviz-2.38.0\\lib\\ast\\error.c" 2
#line 1 "graphviz-2.38.0\\lib\\ast\\error.c" 1
#line 1 "graphviz-2.38.0\\lib\\ast\\error.c" 1

Error_info_t _err_info;

void setErrorLine (int line) { _err_info.line = line; }
void setErrorFileLine (char* src, int line) {
    _err_info.file = src;
    _err_info.line = line;
}
void setErrorId (char* id) { _err_info.id = id; }
void setErrorErrors (int errors) { _err_info.errors = errors; }
int  getErrorErrors () { return _err_info.errors; }
void setTraceLevel (int i) { _err_info.trace = i; }

void _err_msgv(const char *id, int level, va_list ap)
{
    char *s;
    int flags;

    if (level < _err_info.trace) return;
    if (level < 0)
	flags = 0;
    else {
	flags = level & ~0x00ff;
    ...
    if (level && ((s = _err_info.id) || (s = (char *) id))) {
	if (flags & 0x0800)
	...
	if (_err_info.line) {
	    if (_err_info.file && *_err_info.file)
		sfprintf(sfstderr, "\"%s\", ", _err_info.file);
	    sfprintf(sfstderr, "line %d: ", _err_info.line);
	}
    }
    s = va_arg(ap, char *);
...
void _err_msg(int level, ...)
{
    va_list ap;

    va_start(ap, level);
    _err_msgv(0, level, ap);
    va_end(ap);
}
