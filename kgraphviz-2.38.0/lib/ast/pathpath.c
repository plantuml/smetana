#line 1 "graphviz-2.38.0\\lib\\ast\\pathpath.c" 1
/* $Id$ $Revision$ */
...
#line 30 "graphviz-2.38.0\\lib\\ast\\pathpath.c" 2
#line 1 "graphviz-2.38.0\\lib\\ast\\pathpath.c" 1
...
#line 32 "graphviz-2.38.0\\lib\\ast\\pathpath.c" 2
...
char *pathpath(register char *path, const char *p, const char *a, int mode)
{
	...
	if ((!cmd || *cmd) &&
	    (strchr(s, '/') ||
	      ...
	      *s++ == '=' && strchr(s, '/') && !(*(s)==*("/bin/")&&!strncmp(s,"/bin/",5)) &&
	      !(*(s)==*("/usr/bin/")&&!strncmp(s,"/usr/bin/",9))) ||
	     (*x && !access(x, 0) && (s = getenv("PWD")) && *s == '/')
	    )
	    ) {
	    if (!cmd)
		cmd = strdup(s);
