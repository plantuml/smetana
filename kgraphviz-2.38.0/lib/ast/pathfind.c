#line 1 "graphviz-2.38.0\\lib\\ast\\pathfind.c" 1
/* $Id$ $Revision$ */
...
#line 24 "graphviz-2.38.0\\lib\\ast\\pathfind.c" 2
...
#line 25 "graphviz-2.38.0\\lib\\ast\\pathfind.c" 2
...
int pathinclude(const char *dir)
{
    ...
    if (dir && *dir && !(*(dir)==*(".")&&!strcmp(dir,"."))) {
	if (!(dp = ((0)?(Dir_t*)realloc((char*)(0),sizeof(Dir_t)*(1)+(strlen(dir))):(Dir_t*)malloc(sizeof(Dir_t)*(1)+(strlen(dir))))))
	    return -1;
	strcpy(dp->dir, dir);
	if (state.tail)
