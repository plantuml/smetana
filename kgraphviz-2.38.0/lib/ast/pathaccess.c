#line 1 "graphviz-2.38.0\\lib\\ast\\pathaccess.c" 1
/* $Id$ $Revision$ */
...
#line 27 "graphviz-2.38.0\\lib\\ast\\pathaccess.c" 2
...
#line 28 "graphviz-2.38.0\\lib\\ast\\pathaccess.c" 2
		 ...
		 const char *a, const char *b, register int mode)
{
	...
	if (!access(path, m)) {
	    if ((mode & 010)
		&& (stat(path, &st) || (((st.st_mode) & 0170000) == 0040000)))
		continue;
	    if (*path == '/' || !(mode & 020))
		return (path);
