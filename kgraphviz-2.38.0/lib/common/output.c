...
static void set_record_rects(node_t * n, field_t * f, agxbuf * xb)
{
	...
	sprintf(buf, "%.5g,%.5g,%.5g,%.5g ",
		...
		(Y_invert ? (Y_off - (f->b.LL.y + ND_coord(n).y)) : (f->b.LL.y + ND_coord(n).y)),
		f->b.UR.x + ND_coord(n).x,
		(Y_invert ? (Y_off - (f->b.UR.y + ND_coord(n).y)) : (f->b.UR.y + ND_coord(n).y)));
	agxbput(xb, buf);
    }
...
static void rec_attach_bb(graph_t * g, Agsym_t* bbsym, Agsym_t* lpsym, Agsym_t* lwsym, Agsym_t* lhsym)
{
    ...
    sprintf(buf, "%.5g,%.5g,%.5g,%.5g", GD_bb(g).LL.x, (Y_invert ? (Y_off - (GD_bb(g).LL.y)) : (GD_bb(g).LL.y)),
	    GD_bb(g).UR.x, (Y_invert ? (Y_off - (GD_bb(g).UR.y)) : (GD_bb(g).UR.y)));
    agxset(g, bbsym, buf);
...
void attach_attrs_and_arrows(graph_t* g, int* sp, int* ep)
{
	...
	if (dim3) {
	    ...
	    sprintf(buf, "%.5g,%.5g,%.5g", ND_coord(n).x, (Y_invert ? (Y_off - (ND_coord(n).y)) : (ND_coord(n).y)), 72*(ND_pos(n)[2]));
	    agxbput (&xb, buf);
	    for (k = 3; k < GD_odim(g); k++) {
		sprintf(buf, ",%.5g", 72*(ND_pos(n)[k]));
		agxbput (&xb, buf);
	    }
	    agset(n, "pos", (((((&xb)->ptr >= (&xb)->eptr) ? agxbmore(&xb,1) : 0), (int)(*(&xb)->ptr++ = ((unsigned char)'\0'))),(char*)((&xb)->ptr = (&xb)->buf)));
	} else {
	    sprintf(buf, "%.5g,%.5g", ND_coord(n).x, (Y_invert ? (Y_off - (ND_coord(n).y)) : (ND_coord(n).y)));
	    agset(n, "pos", buf);
	}
	sprintf(buf, "%.5g", ((ND_ht(n))/(double)72));
	agxset(n, N_height, buf);
	sprintf(buf, "%.5g", ((ND_lw(n) + ND_rw(n))/(double)72));
	...
	if (strcmp(ND_shape(n)->name, "record") == 0) {
		...
		for (i = 0; i < sides; i++) {
			...
			sprintf(buf, "%.5g %.5g",
				...
				(Y_invert ? (YF_off - (ND_height(n) / 2.0 * sin(i /(double) sides * M_PI * 2.0))) : (ND_height(n) / 2.0 * sin(i /(double) sides * M_PI * 2.0))));
		    agxbput(&xb, buf);
		}
	...
	if (State >= 1) {
		...
		for (i = 0; i < ED_spl(e)->size; i++) {
			...
			sprintf(buf, "s,%.5g,%.5g ",
				...
				(Y_invert ? (Y_off - (ED_spl(e)->list[i].sp.y)) : (ED_spl(e)->list[i].sp.y)));
			agxbput(&xb, buf);
		    }
			...
			sprintf(buf, "e,%.5g,%.5g ",
				...
				(Y_invert ? (Y_off - (ED_spl(e)->list[i].ep.y)) : (ED_spl(e)->list[i].ep.y)));
			agxbput(&xb, buf);
		    }
