#line 1 "graphviz-2.38.0\\lib\\common\\routespl.c" 1
/* $Id$ $Revision$ */
...
#line 18 "graphviz-2.38.0\\lib\\common\\routespl.c" 2
...
#line 19 "graphviz-2.38.0\\lib\\common\\routespl.c" 2
#line 1 "graphviz-2.38.0\\lib\\common\\routespl.c" 1
    ...
    int polyline)
{
	...
	if (poly.pn > edgen) {
	    edges = (edges? (Pedge_t*)grealloc(edges,(poly.pn)*sizeof(Pedge_t)):(Pedge_t*)gmalloc((poly.pn)*sizeof(Pedge_t)));
	    edgen = poly.pn;
	}
	for (i = 0; i < poly.pn; i++) {
	    edges[i].a = poly.ps[i];
...
routesplinesinit()
{
    ...
    if (!(ps = (pointf*)gmalloc((300)*sizeof(pointf)))) {
...
static pointf *_routesplines(path * pp, int *npoints, int polyline)
{
    ...
    for (realedge = (edge_t *) pp->data;
	 ...
	 realedge && (((Agedgeinfo_t*)(((Agobj_t*)(realedge))->data))->edge_type) != 0;
	 realedge = (((Agedgeinfo_t*)(((Agobj_t*)(realedge))->data))->to_orig));
    if (!realedge) {
	...
	polypoints = (polypoints? (Ppoint_t*)grealloc(polypoints,(boxn * 8)*sizeof(Ppoint_t)):(Ppoint_t*)gmalloc((boxn * 8)*sizeof(Ppoint_t)));
	polypointn = boxn * 8;
    }
    ...
    if (((((((Agobj_t*)(realedge))->tag).objtype) == 3? (realedge): ((realedge)+1))->node) != ((((((Agobj_t*)(realedge))->tag).objtype) == 2? (realedge): ((realedge)-1))->node)) {
	...
	agerr(AGERR, "in routesplines, edge is a loop at %s\n", agnameof(((((((Agobj_t*)(realedge))->tag).objtype) == 2? (realedge): ((realedge)-1))->node)));
	return NULL;
    }
    ...
    for (bi = 0; bi < boxn; bi++)
	boxes[bi].LL.x = ((int)(~(unsigned)0 >> 1)), boxes[bi].UR.x = (-((int)(~(unsigned)0 >> 1)) - 1);
    ...
    for (bi = 0; bi < boxn; bi++) {
	boxes[bi].LL.x = ((int)(~(unsigned)0 >> 1));
	boxes[bi].UR.x = (-((int)(~(unsigned)0 >> 1)) - 1);
    }
    unbounded = (!(0));
	 ...
	 * values have been changed since initialization - ok */
	    if ((boxes[bi].LL.x == ((int)(~(unsigned)0 >> 1))) || (boxes[bi].UR.x == (-((int)(~(unsigned)0 >> 1)) - 1))) {
		delta *= 2; /* try again with a finer interval */
		if (delta > ((int)(~(unsigned)0 >> 1))/boxn) /* in limitBoxes, boxn*delta must fit in an int, so give up */
		    loopcnt = 15;
		break;
	    }
	}
	 ...
	 */
	Ppolyline_t polyspl;
	agerr(AGWARN, "Unable to reclaim box space in spline routing for edge \"%s\" -> \"%s\". Something is probably seriously wrong.\n", agnameof(((((((Agobj_t*)(realedge))->tag).objtype) == 3? (realedge): ((realedge)+1))->node)), agnameof(((((((Agobj_t*)(realedge))->tag).objtype) == 2? (realedge): ((realedge)-1))->node)));
...
static int overlap(int i0, int i1, int j0, int j1)
{
    ...
    if ((j0 <= i1) && (i1 <= j1))
	return (i1 - j0);
    return ((i1 - i0)<(j1 - j0)?(i1 - i0):(j1 - j0));
}
...
static int checkpath(int boxn, boxf* boxes, path* thepath)
{
    ...
    i = 0;
    for (bi = 0; bi < boxn; bi++) {
	if (((boxes[bi].LL.y - boxes[bi].UR.y) >= 0 ? (boxes[bi].LL.y - boxes[bi].UR.y) : -(boxes[bi].LL.y - boxes[bi].UR.y)) < .01)
	    continue;
	if (((boxes[bi].LL.x - boxes[bi].UR.x) >= 0 ? (boxes[bi].LL.x - boxes[bi].UR.x) : -(boxes[bi].LL.x - boxes[bi].UR.x)) < .01)
	    continue;
	if (i != bi)
	    boxes[i] = boxes[bi];
	i++;
...
static int mkspacep(int size)
{
	...
	ps = ((pointf*)grealloc(ps,(newmax)*sizeof(pointf)));
	if (!ps) {
...
static pointf get_centroid(Agraph_t *g)
{
    ...
    sum.x = ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->bb).LL.x + (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->bb).UR.x) / 2.0;
    sum.y = ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->bb).LL.y + (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->bb).UR.y) / 2.0;
    return sum;

    if (save == g) return sum;
    save = g;
    for (n = agfstnode(g); n; n = agnxtnode(g,n)) {
        sum.x += (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[0];
        sum.y += (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[1];
        cnt++;
    }
    sum.x = sum.x / cnt;
    sum.y = sum.y / cnt;
...
makeStraightEdge(graph_t * g, edge_t * e, int et, splineInfo* sinfo)
{
    ...
    e_cnt = 1;
    e0 = e;
    while ((e0 = (((Agedgeinfo_t*)(((Agobj_t*)(e0))->data))->to_virt))) e_cnt++;

    if (e_cnt <= 20)
	edges = elist;
    else
	edges = (edge_t**)zmalloc((e_cnt)*sizeof(edge_t*));
    e0 = e;
    for (i = 0; i < e_cnt; i++) {
	edges[i] = e0;
...
makeStraightEdges(graph_t * g, edge_t** edges, int e_cnt, int et, splineInfo* sinfo)
{
    ...
    p = dumb[1] = dumb[0] = add_pointf((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord), (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->tail_port).p);
    q = dumb[2] = dumb[3] = add_pointf((((Agnodeinfo_t*)(((Agobj_t*)(head))->data))->coord), (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->head_port).p);
    if ((e_cnt == 1) || Concentrate) {
	if (curved) bend(dumb,get_centroid(g));
	clip_and_install(e, ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node), dumb, 4, sinfo);
	addEdgeLabels(g, e, p, q);
	return;
    }

    e0 = e;
    if (((((((((dumb[0])).x -((dumb[3])).x)) * ((((dumb[0])).x -((dumb[3])).x))) + (((((dumb[0])).y -((dumb[3])).y)) * ((((dumb[0])).y -((dumb[3])).y))))) < ((.001) * (.001)))) {
	/* degenerate case */
	dumb[1] = dumb[0];
	dumb[2] = dumb[3];
	del.x = 0;
	...
	xstep = (((Agraphinfo_t*)(((Agobj_t*)(g->root))->data))->nodesep);
	dx = xstep * (e_cnt - 1) / 2;
    ...
    for (i = 0; i < e_cnt; i++) {
	e0 = edges[i];
	if (((((((Agobj_t*)(e0))->tag).objtype) == 2? (e0): ((e0)-1))->node) == head) {
	    p = dumb[0];
	    q = dumb[3];
	    for (j = 0; j < 4; j++) {
		dumber[j] = dumb[j];
	...
	if (et == (3 << 1)) {
	    ...
	    clip_and_install(e0, ((((((Agobj_t*)(e0))->tag).objtype) == 2? (e0): ((e0)-1))->node), spl.ps, spl.pn, sinfo);
	}
	else
	    clip_and_install(e0, ((((((Agobj_t*)(e0))->tag).objtype) == 2? (e0): ((e0)-1))->node), dumber, 4, sinfo);
