...
static polygon_t p_doublecircle = { NOT(0), 2, 1, 0., 0., 0. };
...
static polygon_t p_Msquare = { NOT(0), 1, 4, 0., 0., 0., (1 << 3) };
static polygon_t p_Mcircle =
    { NOT(0), 1, 1, 0., 0., 0., (1 << 3) | (1 << 4) };
...
static void poly_gencode(GVJ_t * job, node_t * n)
{
    ...
    xsize = (ND_lw(n) + ND_rw(n)) / (ROUND((ND_width(n))*72));
    ysize = ND_ht(n) / (ROUND((ND_height(n))*72));

    style = stylenode(job, n);
    clrs[0] = NULL;

    if (ND_gui_state(n) & (1<<0)) {
    ...
    pfilled = !ND_shape(n)->usershape || (*(ND_shape(n)->name)==*("custom")&&!strcmp(ND_shape(n)->name,"custom"));
...
static void point_init(node_t * n)
{
    ...
    ND_height(n) = ND_width(n) = ((sz)/(double)72);
    ND_shape_info(n) = (void *) poly;
}
...
static field_t *parse_reclbl(node_t * n, int LR, int flag, char *text)
{
	...
	case '{':
		...
		return parse_error(rv, tmpport);
	    mode = 4;
	    if (!(rv->fld[fi++] = parse_reclbl(n, NOT(LR), 0, text)))
		return parse_error(rv, tmpport);
	    break;
	case '}':
	case '|':
...
static void record_init(node_t * n)
{
    ...
    if (!(info = parse_reclbl(n, flip, NOT(0), textbuf))) {
...
static void record_gencode(GVJ_t * job, node_t * n)
{
    ...
    if ((*(ND_shape(n)->name)==*("Mrecord")&&!strcmp(ND_shape(n)->name,"Mrecord")))
	style |= (1 << 2);
...
static pointf star_size (pointf sz0)
{
    ...
    rx = sz0.x/(2*cos((M_PI/10.0)));
    ry = sz0.y/(sin((M_PI/10.0)) + sin((3*(M_PI/10.0))));
    r0 = MAX(rx,ry);
    r = (r0*sin((2*(2*(M_PI/10.0))))*cos((2*(M_PI/10.0))))/(cos((M_PI/10.0))*cos((2*(2*(M_PI/10.0)))));

    sz.x = 2*r*cos((M_PI/10.0));
    sz.y = r*(1 + sin((3*(M_PI/10.0))));
    return sz;
}
...
static void star_vertices (pointf* vertices, pointf* bb)
{
    int i;
    pointf sz = *bb;
    double offset, a, aspect = (1 + sin((3*(M_PI/10.0))))/(2*cos((M_PI/10.0)));
    double r, r0, theta = (M_PI/10.0);
    ...
    r = sz.x/(2*cos((M_PI/10.0)));
    r0 = (r*cos((M_PI/10.0))*cos((2*(2*(M_PI/10.0)))))/(sin((2*(2*(M_PI/10.0))))*cos((2*(M_PI/10.0))));
    
    /* offset is the y shift of circle center from bb center */
    offset = (r*(1 - sin((3*(M_PI/10.0)))))/2;
