...
getObjId (GVJ_t* job, void* obj, agxbuf* xb)
{
    ...
    char* gid = (((Agraphinfo_t*)(((Agobj_t*)(root))->data))->drawing)->id;
    long idnum;
    char* pfx;
	...
	break;
    case 1:
        idnum = ((((Agobj_t*)((Agnode_t*)obj))->tag).seq);
	pfx = "node";
	break;
    case 2:
        idnum = ((((Agobj_t*)((Agedge_t*)obj))->tag).seq);
	pfx = "edge";
	break;
    }
...
wedgedEllipse (GVJ_t* job, pointf * pf, char* clrs)
{
	...
	if (s[1].color == NULL) 
	    ...
	    angle1 = angle0 + 2*3.14159265358979323846*(s->t);
...
{
    return (p->sides == 4 && (((p->orientation>=0)?(int)(p->orientation + .5):(int)(p->orientation - .5)) % 90) == 0
...
{
    double theta = 0.0;
    double deltheta = 2 * 3.14159265358979323846 / np;
    int i;
    pointf *ps;
...
static int parse_layers(GVC_t *gvc, graph_t * g, char *p)
{
    ...
    for (tok = strtok(gvc->layers, gvc->layerDelims); tok;
        ...
        if (ntok > sz) {
            sz += 128;
            gvc->layerIDs = (gvc->layerIDs? (char **)grealloc(gvc->layerIDs,(sz)*sizeof(char *)):(char **)gmalloc((sz)*sizeof(char *)));
        }
        gvc->layerIDs[ntok] = tok;
    }
    if (ntok) {
        gvc->layerIDs = ((char **)grealloc(gvc->layerIDs,(ntok + 2)*sizeof(char *)));        /* shrink to minimum size */
        gvc->layerIDs[0] = NULL;
...
static void init_job_pagination(GVJ_t * job, graph_t *g)
{
	...
	imageSize.x = ((imageSize.x)<(pageSize.x)?(imageSize.x):(pageSize.x));
	imageSize.y = ((imageSize.y)<(pageSize.y)?(imageSize.y):(pageSize.y));
    } else {
...
//fprintf(stderr,"page=%g,%g dpi=%g,%g zoom=%g\n", pageSize.x, pageSize.y, job->dpi.x, job->dpi.y, job->zoom);
    job->width = (((pageSize.x + 2*margin.x) * job->dpi.x / 72>=0)?(int)((pageSize.x + 2*margin.x) * job->dpi.x / 72 + .5):(int)((pageSize.x + 2*margin.x) * job->dpi.x / 72 - .5));
    job->height = (((pageSize.y + 2*margin.y) * job->dpi.y / 72>=0)?(int)((pageSize.y + 2*margin.y) * job->dpi.y / 72 + .5):(int)((pageSize.y + 2*margin.y) * job->dpi.y / 72 - .5));
    ...
    if ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->centered) {
	if (pageSize.x > imageSize.x)
    ...
    job->pageBoundingBox.LL.x = ((job->canvasBox.LL.x * job->dpi.x / 72>=0)?(int)(job->canvasBox.LL.x * job->dpi.x / 72 + .5):(int)(job->canvasBox.LL.x * job->dpi.x / 72 - .5));
    job->pageBoundingBox.LL.y = ((job->canvasBox.LL.y * job->dpi.y / 72>=0)?(int)(job->canvasBox.LL.y * job->dpi.y / 72 + .5):(int)(job->canvasBox.LL.y * job->dpi.y / 72 - .5));
    job->pageBoundingBox.UR.x = ((job->canvasBox.UR.x * job->dpi.x / 72>=0)?(int)(job->canvasBox.UR.x * job->dpi.x / 72 + .5):(int)(job->canvasBox.UR.x * job->dpi.x / 72 - .5));
    job->pageBoundingBox.UR.y = ((job->canvasBox.UR.y * job->dpi.y / 72>=0)?(int)(job->canvasBox.UR.y * job->dpi.y / 72 + .5):(int)(job->canvasBox.UR.y * job->dpi.y / 72 - .5));
    if (job->rotation) {
...
static boolean write_edge_test(Agraph_t * g, Agedge_t * e)
{
	...
	sg = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->clust)[c];
	if (agcontains(sg, e))
	    return 0;
    }
    return (!(0));
...
copyPts (pointf* pts, int* ptsize, xdot_point* inpts, int numpts)
{
    ...
    if (numpts > sz) {
	sz = ((2*sz)>(numpts)?(2*sz):(numpts));
	pts = ((pointf*)grealloc(pts,(sz)*sizeof(pointf)));
	*ptsize = sz;
    }
...
static void emit_xdot (GVJ_t * job, xdot* xd)
{
	...
	case xd_grad_fill_color : 
		...
		if (op->op.u.grad_color.type == xd_radial) {
			...
			angle = (int)(180.0*acos((p->x0 - p->x1)/p->r0)/3.14159265358979323846);
		    ...
		    clr1 = p->stops[1].color;
		    frac = p->stops[1].frac;
		    angle = (int)(180.0*atan2(p->y1-p->y0,p->x1-p->x0)/3.14159265358979323846);
        	    gvrender_set_fillcolor(job, clr0);
		    gvrender_set_gradient_vals(job, clr1, angle, frac);
		    filled = 2;
		}
...
static void emit_background(GVJ_t * job, graph_t *g)
{
    ...
    if ((xd = (xdot*)(((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->xdots))
	emit_xdot (job, xd);
}
...
static void setup_page(GVJ_t * job, graph_t * g)
{
    ...
    if (job->common->viewNum == 0)
        ...
        ((job->boundingBox).LL.x = (((job->boundingBox).LL.x)<((job->pageBoundingBox).LL.x)?((job->boundingBox).LL.x):((job->pageBoundingBox).LL.x)), (job->boundingBox).LL.y = (((job->boundingBox).LL.y)<((job->pageBoundingBox).LL.y)?((job->boundingBox).LL.y):((job->pageBoundingBox).LL.y)), (job->boundingBox).UR.x = (((job->boundingBox).UR.x)>((job->pageBoundingBox).UR.x)?((job->boundingBox).UR.x):((job->pageBoundingBox).UR.x)), (job->boundingBox).UR.y = (((job->boundingBox).UR.y)>((job->pageBoundingBox).UR.y)?((job->boundingBox).UR.y):((job->pageBoundingBox).UR.y)));
...
static boolean edge_in_layer(GVJ_t *job, graph_t * g, edge_t * e)
{
	...
	return 0;
    for (cnt = 0; cnt < 2; cnt++) {
	pn = late_string(cnt < 1 ? ((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node) : ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node), N_layer, "");
...
static boolean clust_in_layer(GVJ_t *job, graph_t * sg)
{
    ...
    if (job->numLayers <= 1)
	return (!(0));
    pg = late_string(sg, agattr(sg, 0, "layer", 0), "");
...
{
    return boxf_overlap((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->bb), b);
}
...
static void emit_begin_node(GVJ_t * job, node_t * n)
{
	...
	if ((((Agraphinfo_t*)(((Agobj_t*)(agraphof(n)))->data))->odim) >=3)
            obj->z = (((((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[2])*72>=0)?(int)(((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[2])*72 + .5):(int)(((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[2])*72 - .5)));
	else
            obj->z = 0.0;
    }
    initObjMapData (job, (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->label), n);
    if ((flags & ((1<<16) | (1<<22)))
        ...
        if (poly && !rect && (flags & (1<<19))) {
            ...
            if (poly->peripheries == 0 && !filled) {
                ...
                (p[0].x = coord.x - (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->lw), p[0].y = coord.y - (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht) / 2.0, p[1].x = coord.x + (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->lw), p[1].y = coord.y + (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht) / 2.0);
            }
            /* circle or ellipse */
            ...
            nump = 2;
            p = (pointf*)zmalloc((nump)*sizeof(pointf));
            p[0].x = coord.x - (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->lw);
            p[0].y = coord.y - ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht) / 2);
            p[1].x = coord.x + (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->rw);
            p[1].y = coord.y + ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht) / 2);
        }
        if (! (flags & (1<<13)))
...
static void emit_node(GVJ_t * job, node_t * n)
{
    ...
    if ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape) 				     /* node has a shape */
	    && node_in_layer(job, agraphof(n), n)    /* and is in layer */
	    && node_in_box(n, job->clip)             /* and is in page/view */
	    && ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->state) != gvc->common.viewNum)) /* and not already drawn */
    {
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->state) = gvc->common.viewNum; 	     /* mark node as drawn */
	...
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape)->fns->codefn(job, n);
	if ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->xlabel) && (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->xlabel)->set)
	    emit_label(job, EMIT_NLABEL, (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->xlabel));
	emit_end_node(job);
    }
}
...
{
    double d = (sqrt((((((((pts[0])).x -((pts[1])).x)) * ((((pts[0])).x -((pts[1])).x))) + (((((pts[0])).y -((pts[1])).y)) * ((((pts[0])).y -((pts[1])).y)))))));
    d += (sqrt((((((((pts[1])).x -((pts[2])).x)) * ((((pts[1])).x -((pts[2])).x))) + (((((pts[1])).y -((pts[2])).y)) * ((((pts[1])).y -((pts[2])).y)))))));
    d += (sqrt((((((((pts[2])).x -((pts[3])).x)) * ((((pts[2])).x -((pts[3])).x))) + (((((pts[2])).y -((pts[3])).y)) * ((((pts[2])).y -((pts[3])).y)))))));
    return d;
}
...
{ 
    ...
    if (rv > 1) {
	Agraph_t* g = agraphof(((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node));
	...
	left = 1;
	bz = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->list[i];
	first = 1;
	for (s = segs->segs; s->color; s++) {
	...
	if (((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->size>1) && (bz.sflag||bz.eflag) && styles) 
	    gvrender_set_style(job, styles);
    }
    free (segs);
    return 0;
...
taperfun (edge_t* e)
{
    ...
    return (agisdirected(agraphof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node))) ? forfunc : nonefunc);
}
...
static void emit_edge_graphics(GVJ_t * job, edge_t * e, char** styles)
{
	...
	if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->gui_state) & (1<<0)) {
	...
	else if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->gui_state) & (1<<1)) {
	...
	else if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->gui_state) & (1<<3)) {
	...
	else if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->gui_state) & (1<<2)) {
	...
	if (tapered) {
	    ...
	    bz = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->list[0];
	    stp = taper (&bz, taperfun (e), penwidth, 0, 0);
	    gvrender_polygon(job, stp->vertices, stp->nvertices, (!(0)));
	    free_stroke (stp);
	...
	/* if more than one color - then generate parallel beziers, one per color */
	    ...
	    tmpspl.size = offspl.size = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->size;
		...
		if (!color[0])
		    color = "black";
		if (color != lastcolor) {
	            if (! ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->gui_state) & ((1<<0) | (1<<1)))) {
		        gvrender_set_pencolor(job, color);
		        gvrender_set_fillcolor(job, color);
		    }
		    lastcolor = color;
...
static boolean edge_in_box(edge_t *e, boxf b)
{
    ...
    spl = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl);
    if (spl && boxf_overlap(spl->bb, b))
        return (!(0));

    lp = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->label);
    if (lp && overlap_label(lp, b))
        return (!(0));

    lp = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->xlabel);
...
static void emit_begin_edge(GVJ_t * job, edge_t * e, char** styles)
{
     ...
     */
    if (styles && (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)) gvrender_set_style(job, styles);
	...
	if ((((Agraphinfo_t*)(((Agobj_t*)(agraphof(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node))))->data))->odim) >=3) {
            obj->tail_z = (((((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->pos)[2])*72>=0)?(int)(((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->pos)[2])*72 + .5):(int)(((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 3?(e):((e)+1))->node)))->data))->pos)[2])*72 - .5)));
            obj->head_z = (((((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->pos)[2])*72>=0)?(int)(((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->pos)[2])*72 + .5):(int)(((((Agnodeinfo_t*)(((Agobj_t*)(((((((Agobj_t*)(e))->tag).objtype) == 2?(e):((e)-1))->node)))->data))->pos)[2])*72 - .5)));
	} else {
    ...
    if (flags & (1<<15)) {
	if ((lab = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->label)))
	    obj->label = lab->text;
	obj->taillabel = obj->headlabel = obj->xlabel = obj->label;
	if ((tlab = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->xlabel)))
	    obj->xlabel = tlab->text;
	if ((tlab = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->tail_label)))
	    obj->taillabel = tlab->text;
	if ((hlab = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->head_label)))
    ...
    if (flags & ((1<<16) | (1<<22))) {
	if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl) && (obj->url || obj->tooltip) && (flags & (1<<19))) {
	    int ns;
	    splines *spl;
	    double w2 = ((job->obj->penwidth/2.0)>(2.0)?(job->obj->penwidth/2.0):(2.0));
...
static void emit_end_edge(GVJ_t * job)
{
	...
	bz = (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->list[(((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->size - 1];
    ...
    emit_edge_label(job, (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->label), EMIT_ELABEL,
	obj->explicit_labeltooltip, 
	obj->labelurl, obj->labeltooltip, obj->labeltarget, obj->id, 
	((mapbool(late_string(e, E_decorate, "false")) && (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)) ? (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl) : 0));
    emit_edge_label(job, (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->xlabel), EMIT_ELABEL,
	obj->explicit_labeltooltip, 
	obj->labelurl, obj->labeltooltip, obj->labeltarget, obj->id, 
	((mapbool(late_string(e, E_decorate, "false")) && (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)) ? (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl) : 0));
    emit_edge_label(job, (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->head_label), EMIT_HLABEL, 
	obj->explicit_headtooltip,
	obj->headurl, obj->headtooltip, obj->headtarget, obj->id,
	0);
    emit_edge_label(job, (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->tail_label), EMIT_TLABEL, 
...
static void emit_edge(GVJ_t * job, edge_t * e)
{
    ...
    if (edge_in_box(e, job->clip) && edge_in_layer(job, agraphof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node)), e) ) {

	s = malloc(strlen(agnameof(((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node))) + 2 + strlen(agnameof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node))) + 1);
	strcpy(s,agnameof(((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node)));
	if (agisdirected(agraphof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node))))

	    strcat(s,"->");
	else
	    strcat(s,"--");
	strcat(s,agnameof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node)));
	gvrender_comment(job, s);
	free(s);
...
{
    GVC_t *gvc = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->gvc);
    exdot_op* op;
    int i;
    double fontsize = 0.0;
    char* fontname = NULL;
    ...
    pointf sz;
    boxf bb0;
    boxf bb = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->bb);
    xdot* xd = (xdot*)(((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->xdots;
...
static void init_gvc(GVC_t * gvc, graph_t * g)
{
    ...
    gvc->pageSize = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->page;
    if (((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->page.x > 0.001) && ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->page.y > 0.001))
        gvc->graph_sets_pageSize = (!(0));

    /* rotation */
    if ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->landscape)
	gvc->rotation = 90;
    else 
	gvc->rotation = 0;
    ...
    /* bounding box */
    gvc->bb = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->bb);

    /* clusters have peripheries */
    G_peripheries = (agattr(g,0,"peripheries",NULL));
    G_penwidth = (agattr(g,0,"penwidth",NULL));
...
static void init_job_dpi(GVJ_t *job, graph_t *g)
{
    ...
    if ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->dpi != 0) {
        job->dpi.x = job->dpi.y = (double)((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->dpi);
    }
...
static void init_job_viewport(GVJ_t * job, graph_t * g)
{
    ...
    Z = 1.0;
    if ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->size.x > 0.001 && (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->size.y > 0.001) { /* graph size was given by user... */
	size = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->size;
	if (sz.x == 0) sz.x = size.x;
	if (sz.y == 0) sz.y = size.y;
	if ((size.x < sz.x) || (size.y < sz.y) /* drawing is too big (in either axis) ... */
	    || (((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)->filled) /* or ratio=filled requested and ... */
		&& (size.x > sz.x) && (size.y > sz.y))) /* drawing is too small (in both axes) ... */
	    Z = ((size.x/sz.x)<(size.y/sz.y)?(size.x/sz.x):(size.y/sz.y));
    }
	...
	if (rv == 4) {
	    ...
	    if (n) {
		x = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).x;
		y = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).y;
	    }
	}
	else {
...
static void emit_view(GVJ_t * job, graph_t * g, int flags)
{
	...
	for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	    emit_node(job, n);
	    for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
		emit_node(job, ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node));
		emit_edge(job, e);
	    }
	}
    }
...
static void emit_begin_graph(GVJ_t * job, graph_t * g)
{
    ...
    initObjMapData (job, (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->label), g);
...
static void emit_page(GVJ_t * job, graph_t * g)
{
    ...
    if ((flags & (1<<15)) && ((lab = (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->label))))
    ...
    if ((((Agraphinfo_t*)(((Agobj_t*)(g))->data))->label))
	emit_label(job, EMIT_GLABEL, (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->label));
    if (!(flags & (1<<2)) && (obj->url || obj->explicit_tooltip))
	gvrender_end_anchor(job);
...
void emit_graph(GVJ_t * job, graph_t * g)
{
    ...
    s = late_string(g, agattr(g, 0, "comment", 0), "");
    gvrender_comment(job, s);
    ...
    /* reset node state */
    for (n = agfstnode(g); n; n = agnxtnode(g, n))
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->state) = 0;
    /* iterate layers */
...
static void emit_begin_cluster(GVJ_t * job, Agraph_t * sg)
{
    ...
    initObjMapData (job, (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->label), sg);
...
void emit_clusters(GVJ_t * job, Agraph_t * g, int flags)
{
	...
	if (doAnchor && !(flags & (1<<2))) {
	    emit_map_rect(job, (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->bb));
	    gvrender_begin_anchor(job, obj->url, obj->tooltip, obj->target, obj->id);
	}
	filled = 0;
	istyle = 0;
	...
	if (istyle & (1 << 2)) {
		...
		AF[0] = (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->bb).LL;
		AF[2] = (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->bb).UR;
		AF[1].x = AF[2].x;
		AF[1].y = AF[0].y;
		AF[3].x = AF[0].x;
		AF[3].y = AF[2].y;
	    ...
	    gvrender_box(job, (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->bb), 0);
	}
	else {
	    if (late_int(sg, G_peripheries, 1, 0)) {
    		gvrender_set_pencolor(job, pencolor);
		gvrender_box(job, (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->bb), filled);
	    }
	    else if (filled) {
        	gvrender_set_pencolor(job, "transparent");
		gvrender_box(job, (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->bb), filled);
	...
	free (clrs[0]);
	if ((lab = (((Agraphinfo_t*)(((Agobj_t*)(sg))->data))->label)))
...
static boxf bezier_bb(bezier bz)
{
	...
	p.x = ( p1.x + p2.x ) / 2;
	p.y = ( p1.y + p2.y ) / 2;
        ((bb).LL.x = (((bb).LL.x)<((p).x)?((bb).LL.x):((p).x)), (bb).LL.y = (((bb).LL.y)<((p).y)?((bb).LL.y):((p).y)), (bb).UR.x = (((bb).UR.x)>((p).x)?((bb).UR.x):((p).x)), (bb).UR.y = (((bb).UR.y)>((p).y)?((bb).UR.y):((p).y)));
...
static void init_splines_bb(splines *spl)
{
    ...
    for (i = 0; i < spl->size; i++) {
        if (i > 0) {
            bz = spl->list[i];
            b = bezier_bb(bz);
            ((bb).LL.x = (((bb).LL.x)<((b).LL.x)?((bb).LL.x):((b).LL.x)), (bb).LL.y = (((bb).LL.y)<((b).LL.y)?((bb).LL.y):((b).LL.y)), (bb).UR.x = (((bb).UR.x)>((b).UR.x)?((bb).UR.x):((b).UR.x)), (bb).UR.y = (((bb).UR.y)>((b).UR.y)?((bb).UR.y):((b).UR.y)));
        }
        if (bz.sflag) {
...
static void init_bb_node(graph_t *g, node_t *n)
{
    ...
    (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->bb).LL.x = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).x - (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->lw);
    (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->bb).LL.y = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).y - (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht) / 2.;
    (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->bb).UR.x = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).x + (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->rw);
    (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->bb).UR.y = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).y + (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht) / 2.;
...
int gvRenderJobs (GVC_t * gvc, graph_t * g)
{
    ...
    if (!(agbindrec(g, "Agraphinfo_t", 0, (!(0))) && (((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing))) {
	...
	if (!(((Agraphinfo_t*)(((Agobj_t*)(g))->data))->drawing)) {
