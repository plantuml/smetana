#line 1 "graphviz-2.38.0\\lib\\common\\psusershape.c" 1
/* $Id$ $Revision$ */
...
#line 20 "graphviz-2.38.0\\lib\\common\\psusershape.c" 2
...
#line 21 "graphviz-2.38.0\\lib\\common\\psusershape.c" 2
...
static usershape_t *user_init(const char *str)
{
    ...
    us = (*(((Dt_t*)(EPSF_contents))->searchf))((EPSF_contents),(void*)(str),0001000);
    if (us)
	return us;
    ...
    /* try to find size */
    saw_bb = must_inline = 0;
    while (fgets(line, sizeof(line), fp)) {
	if (sscanf
	    (line, "%%%%BoundingBox: %d %d %d %d", &lx, &ly, &ux, &uy) == 4) {
	    saw_bb = (!(0));
	}
	if ((line[0] != '%') && strstr(line,"read")) must_inline = (!(0));
	if (saw_bb && must_inline) break;
    }

    if (saw_bb) {
	us = (usershape_t*)gmalloc(sizeof(usershape_t));
	us->x = lx;
	us->y = ly;
	us->w = ux - lx;
	us->y = uy - ly;
	...
	contents = us->data = (char*)gmalloc((statbuf.st_size + 1)*sizeof(char));
	fseek(fp, 0, SEEK_SET);
	rc = fread(contents, statbuf.st_size, 1, fp);
	contents[statbuf.st_size] = '\0';
	(*(((Dt_t*)(EPSF_contents))->searchf))((EPSF_contents),(void*)(us),0000001);
...
void epsf_init(node_t * n)
{
	...
	dx = us->w;
	dy = us->h;
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->width) = ((dx)/(double)72);
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->height) = ((dy)/(double)72);
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape_info) = desc = (epsf_t*)zmalloc(sizeof(epsf_t));
...
void epsf_free(node_t * n)
{
    ...
    if ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape_info))
	free((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->shape_info));
}
...
void epsf_define(GVJ_t *job)
{
    ...
    if (!EPSF_contents)
	return;
    for (us = (*(((Dt_t*)(EPSF_contents))->searchf))((EPSF_contents),(void*)(0),0000200); us; us = (*(((Dt_t*)(EPSF_contents))->searchf))((EPSF_contents),(void*)(us),0000010)) {
	if (us->must_inline)
	    continue;
...
char *ps_string(char *ins, int chset)
{
    ...
    rc = ((((&xb)->ptr >= (&xb)->eptr) ? agxbmore(&xb,1) : 0), (int)(*(&xb)->ptr++ = ((unsigned char)'(')));
    s = base;
    while (*s) {
        if ((*s == '(') || (*s == ')') || (*s == '\\'))
            rc = ((((&xb)->ptr >= (&xb)->eptr) ? agxbmore(&xb,1) : 0), (int)(*(&xb)->ptr++ = ((unsigned char)'\\')));
        rc = ((((&xb)->ptr >= (&xb)->eptr) ? agxbmore(&xb,1) : 0), (int)(*(&xb)->ptr++ = ((unsigned char)*s++)));
    }
    ((((&xb)->ptr >= (&xb)->eptr) ? agxbmore(&xb,1) : 0), (int)(*(&xb)->ptr++ = ((unsigned char)')')));
    if (base != ins) free (base);
    s = (((((&xb)->ptr >= (&xb)->eptr) ? agxbmore(&xb,1) : 0), (int)(*(&xb)->ptr++ = ((unsigned char)'\0'))),(char*)((&xb)->ptr = (&xb)->buf));
    return s;
}
