...
static void emit_html_img(GVJ_t * job, htmlimg_t * cp, htmlenv_t * env)
{
    ...
    assert(cp->src);
    assert(cp->src[0]);
    gvrender_usershape(job, cp->src, A, 4, NOT(0), scale);
}
...
void makeGraphs(htmltbl_t * tbl, graph_t * rowg, graph_t * colg)
{
	...
	agbindrec(t, "Agnodeinfo_t", sizeof(Agnodeinfo_t), NOT(0));
...
void sizeArray(htmltbl_t * tbl)
{
    ...
    /* Only need GD_nlist */
    agbindrec(rowg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	// graph custom data
    agbindrec(colg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	// graph custom data
