#line 1 "graphviz-2.38.0\\lib\\common\\taper.c" 1
...
#line 25 "graphviz-2.38.0\\lib\\common\\taper.c" 2
...
#line 26 "graphviz-2.38.0\\lib\\common\\taper.c" 2
...
#line 27 "graphviz-2.38.0\\lib\\common\\taper.c" 2
...
#line 28 "graphviz-2.38.0\\lib\\common\\taper.c" 2
...
#line 29 "graphviz-2.38.0\\lib\\common\\taper.c" 2
...
static void addto (stroke_t* p, double x, double y)
{
	...
	p->flags =+ 2000;
	p->vertices = ((pointf*)grealloc(p->vertices,(p->flags)*sizeof(pointf)));
    }
    pt.x = x;
    pt.y = y;
...
static void arcn (stroke_t* p, double x, double y, double r, double a1, double a2)
{
    ...
    if (r == 0) return;
    while (a2 > a1) a2 -= 2*3.14159265358979323846;
    theta = a1 - a2; 
...
static double myatan (double y, double x)
{
	...
	v = atan2 (y, x);
	if (v >= 0) return v;
	else return (v + 2*3.14159265358979323846);
    }
}
...
newArr (void)
{
    vararr_t* arr = (vararr_t*)zmalloc(sizeof(vararr_t));

    arr->cnt = 0;
    arr->sz = 2000;
    arr->pts = (pathpoint*)zmalloc((2000)*sizeof(pathpoint));
...
insertArr (vararr_t* arr, pointf p, double l)
{
    if (arr->cnt >= arr->sz) {
	arr->sz *= 2;
	arr->pts = ((pathpoint*)grealloc(arr->pts,(arr->sz)*sizeof(pathpoint)));
    }
...
{
    if (arr->sz > arr->cnt)
	arr->pts = ((pathpoint*)grealloc(arr->pts,(arr->cnt)*sizeof(pathpoint)));
}
...
static vararr_t* pathtolines (bezier* bez, double initwid)
{
	...
	for (step = 1; step <= 20; step++) {
	    p1 = Bezier(V, 3, (double) step / 20, (void *)0, (void *)0);
	    seglen = l2dist(p0, p1);
...
static void drawbevel(double x, double y, double lineout, int forward, double dir, double dir2, int linejoin, stroke_t* p)
{
    ...
    if (linejoin == 1) {
	a = a1 - a2;
	if (a <= (3.14159265358979323846*(0.1)/180.0)) a += (3.14159265358979323846*(360)/180.0);
	if (a < (3.14159265358979323846*(180)/180.0)) {
	    a1 = a + a2;
...
stroke_t* taper (bezier* bez, radfunc_t radfunc, double initwid, int linejoin, int linecap)
{
	...
	linerad = radfunc(dist, linelen, initwid);
 	...
 	if ((i == 0) || (i == pathcount-1)) {
	    lineout = linerad;
	    if (i == 0) {
		direction = ndir + (3.14159265358979323846*(90)/180.0);
		if (linecap == 2) {
		    x -= cos(ndir)*lineout;
		    y -= sin(ndir)*lineout;
		}
	    } else {
		direction = ldir - (3.14159265358979323846*(90)/180.0);
		if (linecap == 2) {
		    x -= cos(ldir)*lineout;
		    y -= sin(ldir)*lineout;
		}
	    ...
	    theta = ndir-ldir;
	    if (theta < 0) {
		theta += (3.14159265358979323846*(360)/180.0);
	    }
	    phi = (3.14159265358979323846*(90)/180.0)-(theta/2);
	    ...
	    direction = ndir+(3.14159265358979323846*(90)/180.0)+phi;
	    if ((0 != linejoin) || (lineout > currentmiterlimit * linerad)) {
		bevel = (!(0));
		lineout = linerad;
		direction = mymod(ldir-(3.14159265358979323846*(90)/180.0),(3.14159265358979323846*(360)/180.0));
		direction_2 = mymod(ndir+(3.14159265358979323846*(90)/180.0),(3.14159265358979323846*(360)/180.0));
		if (i == pathcount-1) {
		    bevel = 0;
		}
	    } else {
	...
	pathpoints[i].dir2 = direction_2;
	 ...
	 /* draw line */
    p = (stroke_t*)zmalloc(sizeof(stroke_t));
	 /* side 1 */
    for (i = 0; i < pathcount; i++) {
	cur_point = pathpoints[i];
	x = cur_point.x;
	...
	if (bevel) {
	    drawbevel (x, y, lineout, (!(0)), direction, direction_2, linejoin, p);
	}
    }
	 /* end circle as needed */
    if (linecap == 1) {
	arcn (p, x,y,lineout,direction,direction+(3.14159265358979323846*(180)/180.0));
    } else {
	direction += (3.14159265358979323846*(180)/180.0);
	addto(p,x+cos(direction)*lineout,y+sin(direction)*lineout);
    }
	 /* side 2 */
	...
	x = cur_point.x;
	y = cur_point.y;
	direction = cur_point.dir + (3.14159265358979323846*(180)/180.0);
	lineout = cur_point.lout;
	bevel = cur_point.bevel;
	direction_2 = cur_point.dir2 + (3.14159265358979323846*(180)/180.0);
