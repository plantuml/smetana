#line 1 "graphviz-2.38.0\\lib\\common\\arrows.c" 1
/* $Id$ $Revision$ */
...
#line 17 "graphviz-2.38.0\\lib\\common\\arrows.c" 2
...
static arrowname_t Arrowmods[] = {
    ...
    {"e", (1<<(4+0))},	/* o  - needed for "ediamond" */
    {"half", (1<<(4+2))},	/* l  - needed for "halfopen" */
    {(char *) 0, (0)}
};
...
void arrow_flags(Agedge_t * e, int *sflag, int *eflag)
{
    ...
    *eflag = agisdirected(agraphof(e)) ? 1 : (0);
    ...
    if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->conc_opp_flag)) {
	edge_t *f;
	int s0, e0;
	/* pick up arrowhead of opposing edge */
	f = (agedge(agraphof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node)),((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node),((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node),NULL,0));
	arrow_flags(f, &s0, &e0);
	*eflag = *eflag | s0;
	*sflag = *sflag | e0;
    }
...
{
    return ((((((p).x -(inside_context->a.p[0]).x)) * (((p).x -(inside_context->a.p[0]).x))) + ((((p).y -(inside_context->a.p[0]).y)) * (((p).y -(inside_context->a.p[0]).y))))) <= inside_context->a.r[0];
}
		 ...
		 int endp, bezier * spl, int eflag)
{
    ...
    if (endp > startp && ((((((ps[endp]).x -(ps[endp + 3]).x)) * (((ps[endp]).x -(ps[endp + 3]).x))) + ((((ps[endp]).y -(ps[endp + 3]).y)) * (((ps[endp]).y -(ps[endp + 3]).y))))) < elen2) {
	endp -= 3;
    }
    sp[3] = ps[endp];
    sp[2] = ps[endp + 1];
    ...
    bezier_clip(&inside_context, inside, sp, (!(0)));
		   ...
		   int endp, bezier * spl, int sflag)
{
    ...
    if (endp > startp && ((((((ps[startp]).x -(ps[startp + 3]).x)) * (((ps[startp]).x -(ps[startp + 3]).x))) + ((((ps[startp]).y -(ps[startp + 3]).y)) * (((ps[startp]).y -(ps[startp + 3]).y))))) < slen2) {
	startp += 3;
    }
    sp[0] = ps[startp + 3];
    sp[1] = ps[startp + 2];
...
static void arrow_type_normal(GVJ_t * job, pointf p, pointf u, double arrowsize, double penwidth, int flag)
{
    ...
    if (flag & (1<<(4+2)))
	gvrender_polygon(job, a, 3, !(flag & (1<<(4+0))));
    else if (flag & (1<<(4+3)))
	gvrender_polygon(job, &a[2], 3, !(flag & (1<<(4+0))));
    else
	gvrender_polygon(job, &a[1], 3, !(flag & (1<<(4+0))));
}
...
static void arrow_type_crow(GVJ_t * job, pointf p, pointf u, double arrowsize, double penwidth, int flag)
{
    ...
    arrowwidth = 0.45;
    if (penwidth > (4 * arrowsize) && (flag & (1<<(4+1))))
        arrowwidth *= penwidth / (4 * arrowsize);

    shaftwidth = 0;
    if (penwidth > 1 && (flag & (1<<(4+1))))
...
static void arrow_type_box(GVJ_t * job, pointf p, pointf u, double arrowsize, double penwidth, int flag)
{
    ...
    gvrender_polygon(job, a, 4, !(flag & (1<<(4+0))));
    a[0] = m;
    a[1] = q;
    gvrender_polyline(job, a, 2);
}
...
static void arrow_type_dot(GVJ_t * job, pointf p, pointf u, double arrowsize, double penwidth, int flag)
{
    ...
    AF[1].x = p.x + u.x / 2. + r;
    AF[1].y = p.y + u.y / 2. + r;
    gvrender_ellipse(job, AF, 2, !(flag & (1<<(4+0))));
}
...
static void arrow_type_curve(GVJ_t* job, pointf p, pointf u, double arrowsize, double penwidth, int flag)
{
    ...
    gvrender_beziercurve(job, AF, sizeof(AF) / sizeof(pointf), 0, 0, 0);
}
...
boxf arrow_bb(pointf p, pointf u, double arrowsize, int flag)
{
    ...
    s = 10. * arrowsize / (sqrt(u.x * u.x + u.y * u.y) + .0001);
    u.x += (u.x >= 0.0) ? .0001 : -.0001;
    u.y += (u.y >= 0.0) ? .0001 : -.0001;
    u.x *= s;
    u.y *= s;
    ...
    /* compute a right bb */
    bb.UR.x = ((ax)>(((bx)>(((cx)>(dx)?(cx):(dx)))?(bx):(((cx)>(dx)?(cx):(dx)))))?(ax):(((bx)>(((cx)>(dx)?(cx):(dx)))?(bx):(((cx)>(dx)?(cx):(dx))))));
    bb.UR.y = ((ay)>(((by)>(((cy)>(dy)?(cy):(dy)))?(by):(((cy)>(dy)?(cy):(dy)))))?(ay):(((by)>(((cy)>(dy)?(cy):(dy)))?(by):(((cy)>(dy)?(cy):(dy))))));
    bb.LL.x = ((ax)<(((bx)<(((cx)<(dx)?(cx):(dx)))?(bx):(((cx)<(dx)?(cx):(dx)))))?(ax):(((bx)<(((cx)<(dx)?(cx):(dx)))?(bx):(((cx)<(dx)?(cx):(dx))))));
    bb.LL.y = ((ay)<(((by)<(((cy)<(dy)?(cy):(dy)))?(by):(((cy)<(dy)?(cy):(dy)))))?(ay):(((by)<(((cy)<(dy)?(cy):(dy)))?(by):(((cy)<(dy)?(cy):(dy))))));
...
void arrow_gen(GVJ_t * job, emit_state_t emit_state, pointf p, pointf u, double arrowsize, double penwidth, int flag)
{
    ...
    u.y -= p.y;
    /* the EPSILONs are to keep this stable as length of u approaches 0.0 */
    s = 10. / (sqrt(u.x * u.x + u.y * u.y) + .0001);
    u.x += (u.x >= 0.0) ? .0001 : -.0001;
    u.y += (u.y >= 0.0) ? .0001 : -.0001;
    u.x *= s;
    u.y *= s;
