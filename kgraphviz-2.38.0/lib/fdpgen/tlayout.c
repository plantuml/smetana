...
void fdp_initParams(graph_t * g)
{
    ...
    (parms.maxIters) = late_int(g, agattr(g,AGRAPH, "maxiter", NULL), 600, 0);
    (fdp_parms->K) = (parms.K) = late_double(g, agattr(g,AGRAPH, "K", NULL), 0.3, 0.0);
    if ((fdp_parms->T0) == -1.0) {
	(parms.T0) = late_double(g, agattr(g,AGRAPH, "T0", NULL), -1.0, 0.0);
    } else
	(parms.T0) = (fdp_parms->T0);
    (parms.seed) = 1;
    (parms.smode) = setSeed (g, INIT_RANDOM, &(parms.seed));
    if ((parms.smode) == INIT_SELF) {
	agerr(AGWARN, "fdp does not support start=self - ignoring\n");
	(parms.seed) = INIT_RANDOM;
    }
...
doRep(node_t * p, node_t * q, double xdelta, double ydelta, double dist2)
{
    ...
    if ((!(((dndata*)(ND_alg(p)))->dn) && !ND_clust(p)) && (!(((dndata*)(ND_alg(q)))->dn) && !ND_clust(q)))
	force *= 10.0;
    (((dndata*)(ND_alg(q)))->disp)[0] += xdelta * force;
    (((dndata*)(ND_alg(q)))->disp)[1] += ydelta * force;
    (((dndata*)(ND_alg(p)))->disp)[0] -= xdelta * force;
    (((dndata*)(ND_alg(p)))->disp)[1] -= ydelta * force;
}
...
static void updatePos(Agraph_t * g, double temp, bport_t * pp)
{
	...
	if (ND_pinned(n) & 2)
	    continue;
	dx = (((dndata*)(ND_alg(n)))->disp)[0];
	dy = (((dndata*)(ND_alg(n)))->disp)[1];
	len2 = dx * dx + dy * dy;
	...
	if (pp) {
	    ...
	    if ((!(((dndata*)(ND_alg(n)))->dn) && !ND_clust(n))) {
		ND_pos(n)[0] = x / d;
		ND_pos(n)[1] = y / d;
	    } else if (d >= 1.0) {
		ND_pos(n)[0] = 0.95 * x / d;
...
static void gAdjust(Agraph_t * g, double temp, bport_t * pp, Grid * grid)
{
    ...
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	(((dndata*)(ND_alg(n)))->disp)[0] = (((dndata*)(ND_alg(n)))->disp)[1] = 0;
	addGrid(grid, ((int)floor((ND_pos(n))[0] / (parms.Cell))), ((int)floor((ND_pos(n))[1] / (parms.Cell))),
		n);
    }
...
{
    int nG = agnnodes(g) - (((gdata*)(GD_alg(g)))->nports);
    double size;
    Agnode_t *np;
...
/* fprintf (stderr, "initPos %s ctr (%g,%g) Wd %g Ht %g\n", g->name, ctr.x, ctr.y, Wd, Ht); */
	while (pp->e) {		/* position ports on ellipse */
	    np = pp->n;
	    ND_pos(np)[0] = (parms.Wd) * cos(pp->alpha) + ctr.x;
	    ND_pos(np)[1] = (parms.Ht) * sin(pp->alpha) + ctr.y;
	    ND_pinned(np) = 1;
/* fprintf (stderr, "%s pt (%g,%g) %g\n", np->name, ND_pos(np)[0], ND_pos(np)[1], pp->alpha); */
	    pp++;
	}
	for (np = agfstnode(g); np; np = agnxtnode(g, np)) {
	    if ((!(((dndata*)(ND_alg(np)))->dn) && !ND_clust(np)))
		continue;
	    if (ND_pinned(np)) {
		ND_pos(np)[0] -= ctr.x;
		ND_pos(np)[1] -= ctr.y;
...
/* fprintf (stderr, "%s %d (%g,%g)\n", np->name, cnt, ND_pos(np)[0], ND_pos(np)[1]); */
		    ...
		    ND_pos(np)[0] = radius * (parms.Wd) * cos(angle);
		    ND_pos(np)[1] = radius * (parms.Ht) * sin(angle);
/* fprintf (stderr, "%s 0 (%g,%g)\n", np->name, ND_pos(np)[0], ND_pos(np)[1]); */
	...
	if (n_pos) {		/* If positioned nodes */
		...
		if (ND_pinned(np)) {
		    ...
		    ND_pos(np)[0] = (parms.Wd) * (2.0 * drand48() - 1.0);
		    ND_pos(np)[1] = (parms.Ht) * (2.0 * drand48() - 1.0);
		}
	    }
...
void dumpstat(graph_t * g)
{
    ...
    edge_t *ep;
    for (np = agfstnode(g); np; np = agnxtnode(g, np)) {
	dx = (((dndata*)(ND_alg(np)))->disp)[0];
	dy = (((dndata*)(ND_alg(np)))->disp)[1];
	l = dx * dx + dy * dy;
	if (l > max2)
	    max2 = l;
	fprintf(stderr, "%s: (%f,%f) (%f,%f)\n", agnameof(np),
		ND_pos(np)[0], ND_pos(np)[1], (((dndata*)(ND_alg(np)))->disp)[0], (((dndata*)(ND_alg(np)))->disp)[1]);
    }
...
void fdp_tLayout(graph_t * g, xparams * xpms)
{
    int i;
    int reset;
    bport_t *pp = (((gdata*)(GD_alg(g)))->ports);
    double temp;
    Grid *grid;
    pointf ctr;
    Agnode_t *n;
