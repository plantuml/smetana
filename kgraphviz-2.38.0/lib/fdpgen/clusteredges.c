...
static objlist *objectList(edge_t * ep, expand_t* pm)
{
    node_t *h = aghead(ep);
    node_t *t = agtail(ep);
    graph_t *hg = (ND_clust(h));
    graph_t *tg = (ND_clust(t));
    int hlevel;
    int tlevel;
    ...
    if ((ND_clustnode(h))) {
	hex = hg;
	hg = (((gdata*)(GD_alg(hg)))->parent);
    } else
	hex = h;
    if ((ND_clustnode(t))) {
	tex = tg;
	tg = (((gdata*)(GD_alg(tg)))->parent);
    } else
	tex = t;

    hlevel = (((gdata*)(GD_alg(hg)))->level);
    tlevel = (((gdata*)(GD_alg(tg)))->level);
    if (hlevel > tlevel) {
