	...
	layout_info* infop)
{
	...
	bb.LL.x = 0;
	bb.LL.y = 0;
	bb.UR.x = late_int(rg, G_width, (ROUND((0.75)*72)), 3);
	bb.UR.y = late_int(rg, G_height, (ROUND((0.5)*72)), 3);
	isEmpty = 1;
    }
    ...
    pt.x = -bb.LL.x + margin;
    pt.y = -bb.LL.y + margin + GD_border(rg)[0].y;
    bb.LL.x = 0;
    bb.LL.y = 0;
    bb.UR.x += pt.x + margin;
    bb.UR.y += pt.y + margin + GD_border(rg)[2].y;
...
static node_t *mkDeriveNode(graph_t * dg, char *name)
{
    ...
    dn = agnode(dg, name,1);
    agbindrec(dn, "Agnodeinfo_t", sizeof(Agnodeinfo_t), NOT(0));	//node custom data
    ND_alg(dn) = (void *) (dndata*)zmalloc(sizeof(dndata));	/* free in freeDeriveNode */
    ND_pos(dn) = (double*)gmalloc((GD_ndim(dg))*sizeof(double));
...
static void freeDerivedGraph(graph_t * g, graph_t ** cc)
{
    ...
    if ((((gdata*)(GD_alg(g)))->ports))
	free((((gdata*)(GD_alg(g)))->ports));
    freeGData(g);
...
static void evalPositions(graph_t * g, graph_t* rootg)
{
	...
	subg = GD_clust(g)[i];
	if (g != rootg) {
	    sbb = (((gdata*)(GD_alg(subg)))->bb);
	    sbb.LL.x += bb.LL.x;
	    sbb.LL.y += bb.LL.y;
	    sbb.UR.x += bb.LL.x;
	    sbb.UR.y += bb.LL.y;
	    (((gdata*)(GD_alg(subg)))->bb) = sbb;
	}
...
static graph_t *deriveGraph(graph_t * g, layout_info * infop)
{
    ...
    agbindrec(dg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));
    GD_alg(dg) = (void *) (gdata*)zmalloc(sizeof(gdata));	/* freed in freeDeriveGraph */
    ...
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	if (!(ND_next(n))) {
	    if ((ND_clust(n)) && ((ND_clust(n)) != (((gdata*)(GD_alg(g)))->parent))) {
	...
	for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
	    ...
	    agbindrec(de, "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));
	    ED_dist(de) = ED_dist(e);
	    ED_factor(de) = ED_factor(e);
	    /* fprintf (stderr, "edge %s -- %s\n", tl->name, hd->name); */
	    (((dndata*)(ND_alg(hd)))->wdeg)++;
	    (((dndata*)(ND_alg(tl)))->wdeg)++;
	    if ((ED_to_virt(de) == 0)) {
		(((dndata*)(ND_alg(hd)))->deg)++;
		(((dndata*)(ND_alg(tl)))->deg)++;
	    }
	    addEdge(de, e);
	}
    }
    ...
    /* transform ports */
    if ((pp = (((gdata*)(GD_alg(g)))->ports))) {
	bport_t *pq;
	node_t *m;
	int sz = (((gdata*)(GD_alg(g)))->nports);

	/* freed in freeDeriveGraph */
	(((gdata*)(GD_alg(dg)))->ports) = pq = (bport_t*)zmalloc((sz + 1)*sizeof(bport_t));
	sz = 0;
	while (pp->e) {
	    m = (ND_next(pp->n));
		...
		addEdge(de, pp->e);
		(((dndata*)(ND_alg(dn)))->wdeg)++;
		(((dndata*)(ND_alg(m)))->wdeg)++;
		(((dndata*)(ND_alg(dn)))->deg)++;	/* ports are unique, so this will be the first and */
		(((dndata*)(ND_alg(m)))->deg)++;	/* only time the edge is touched. */
		pq->n = dn;
		pq->alpha = pp->alpha;
		pq->e = de;
		pq++;
...
genPorts(node_t * n, erec * er, bport_t * pp, int idx, double bnd)
{
	...
	el = *ep;
	pp[i].e = el;
	pp[i].n = ((ND_next(agtail(el))) == n ? agtail(el) : aghead(el));
	pp[i].alpha = angle;
	i += inc;
	angle += delta;
    }
...
static graph_t *expandCluster(node_t * n, graph_t * cg)
{
	...
	(((gdata*)(GD_alg(sg)))->ports) = pp;
	(((gdata*)(GD_alg(sg)))->nports) = sz;
	free(es);
    }
    return sg;
}
...
setClustNodes(graph_t* root)
{
	...
	p = (ND_clust(n));
	bb = (((gdata*)(GD_alg(p)))->bb);  /* bbox in parent cluster's coordinates */
	w = bb.UR.x - bb.LL.x;
	h = bb.UR.y - bb.LL.y;
	ctr.x = w / 2.0;
	ctr.y = h / 2.0;
...
void layout(graph_t * g, layout_info * infop)
{
	...
	for (n = agfstnode(cg); n; n = agnxtnode(cg, n)) {
		...
		layout(sg, infop);
		/* bb.LL == origin */
		ND_width(n) = (((gdata*)(GD_alg(sg)))->bb).UR.x;
		ND_height(n) = (((gdata*)(GD_alg(sg)))->bb).UR.y;
		pt.x = 72 * (((gdata*)(GD_alg(sg)))->bb).UR.x;
		pt.y = 72 * (((gdata*)(GD_alg(sg)))->bb).UR.y;
		ND_rw(n) = ND_lw(n) = pt.x/2;
		ND_ht(n) = pt.y;
	    } else if ((!(((dndata*)(ND_alg(n)))->dn) && !ND_clust(n)))
	...
	if ((sg = ND_clust(dn))) {
	    (((gdata*)(GD_alg(sg)))->bb).LL.x = ND_pos(dn)[0] - ND_width(dn) / 2;
	    (((gdata*)(GD_alg(sg)))->bb).LL.y = ND_pos(dn)[1] - ND_height(dn) / 2;
	    (((gdata*)(GD_alg(sg)))->bb).UR.x = (((gdata*)(GD_alg(sg)))->bb).LL.x + ND_width(dn);
	    (((gdata*)(GD_alg(sg)))->bb).UR.y = (((gdata*)(GD_alg(sg)))->bb).LL.y + ND_height(dn);
	} else if ((n = (((dndata*)(ND_alg(dn)))->dn))) {
	    ND_pos(n)[0] = ND_pos(dn)[0];
	    ND_pos(n)[1] = ND_pos(dn)[1];
	}
    }
    (((gdata*)(GD_alg(g)))->bb) = (((gdata*)(GD_alg(dg)))->bb);
...
static void setBB(graph_t * g)
{
    ...
    bb.LL.x = 72 * (((gdata*)(GD_alg(g)))->bb).LL.x;
    bb.LL.y = 72 * (((gdata*)(GD_alg(g)))->bb).LL.y;
    bb.UR.x = 72 * (((gdata*)(GD_alg(g)))->bb).UR.x;
    bb.UR.y = 72 * (((gdata*)(GD_alg(g)))->bb).UR.y;
    GD_bb(g) = bb;
    for (i = 1; i <= GD_n_cluster(g); i++) {
	setBB(GD_clust(g)[i]);
    }
...
mkClusters (graph_t * g, clist_t* pclist, graph_t* parent)
{
	...
	if (!strncmp(agnameof(subg), "cluster", 7)) {
	    ...
	    GD_alg(subg) = (void *) (gdata*)zmalloc(sizeof(gdata));	/* freed in cleanup_subgs */
	    GD_ndim(subg) = GD_ndim(parent);
	    (((gdata*)(GD_alg(subg)))->level) = (((gdata*)(GD_alg(parent)))->level) + 1;
	    (((gdata*)(GD_alg(subg)))->parent) = parent;
...
{
    setEdgeType (g, (1 << 1));
    GD_alg(g) = (void *) (gdata*)zmalloc(sizeof(gdata));	/* freed in cleanup_graph */
    GD_ndim(g) = late_int(g, agattr(g,AGRAPH, "dim", NULL), 2, 2);
    Ndim = GD_ndim(g) = MIN(GD_ndim(g), 10);
...
void fdp_layout(graph_t * g)
{
    ...
    if ((GD_flags(g) & (7 << 1)) != (0 << 1)) fdpSplines (g); 
