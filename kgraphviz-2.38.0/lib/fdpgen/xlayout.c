...
{
    double w = (X_marg.doAdd ? (ND_width(n)/2.0 + X_marg.x): ND_width(n)*X_marg.x/2.0);
    double h = (X_marg.doAdd ? (ND_height(n)/2.0 + X_marg.y): ND_height(n)*X_marg.y/2.0);
    return sqrt(w * w + h * h);
}
...
static int overlap(node_t * p, node_t * q)
{
    ...
    if (ydelta < 0)
	ydelta = -ydelta;
    ret = ((xdelta <= ((X_marg.doAdd ? (ND_width(p)/2.0 + X_marg.x): ND_width(p)*X_marg.x/2.0) + (X_marg.doAdd ? (ND_width(q)/2.0 + X_marg.x): ND_width(q)*X_marg.x/2.0))) && (ydelta <= ((X_marg.doAdd ? (ND_height(p)/2.0 + X_marg.y): ND_height(p)*X_marg.y/2.0) + (X_marg.doAdd ? (ND_height(q)/2.0 + X_marg.y): ND_height(q)*X_marg.y/2.0))));
    return ret;
...
/* if (V && ov)  */
	...
	if (ND_pinned(n) == 3)
	    continue;
	disp[0] = (((dndata*)(ND_alg(n)))->disp)[0];
	disp[1] = (((dndata*)(ND_alg(n)))->disp)[1];
