#line 1 "graphviz-2.38.0\\lib\\expr\\exexpr.c" 1
...
#line 22 "graphviz-2.38.0\\lib\\expr\\exexpr.c" 2
...
exexpr(Expr_t* ex, const char* name, Exid_t* sym, int type)
{
	...
	{
		if (!sym)
			sym = name ? (Exid_t*)(*(((Dt_t*)(ex->symbols))->searchf))((ex->symbols),(void*)(name),0001000) : &ex->main;
		if (sym && sym->lex == 293 && sym->value)
		{
			if (type != 258)
				return excast(ex, sym->value->data.procedure.body, type, 0, 0);
