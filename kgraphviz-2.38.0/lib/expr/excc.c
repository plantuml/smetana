#line 1 "graphviz-2.38.0\\lib\\expr\\excc.c" 1
...
struct Excc_s				/* excc() state			*/
{
	...
	char* id; int lastop; int tmp; Exccdisc_t* ccdisc;
...
#line 28 "graphviz-2.38.0\\lib\\expr\\excc.c" 2
#line 1 "graphviz-2.38.0\\lib\\expr\\excc.c" 1
...
print(Excc_t* cc, Exnode_t* expr)
{
	...
	{
		sfprintf(cc->ccdisc->text, "sfprintf(%s, \"%s", expr->data.print.descriptor->op == 271 && expr->data.print.descriptor->data.constant.value.integer == 2 ? "sfstderr" : "sfstdout", fmtesq(x->format, quote));
		while ((x = x->next))
		...
		for (x = expr->data.print.args; x; x = x->next)
		{
			if (x->arg)
			{
				for (i = 0; i < (sizeof(x->param)/sizeof(x->param[0])) && x->param[i]; i++)
				{
					sfprintf(cc->ccdisc->text, ", (");
					gen(cc, x->param[i]);
					sfprintf(cc->ccdisc->text, ")");
...
gen(Excc_t* cc, register Exnode_t* expr)
{
	...
	case 300:
   	...
   	case 302:
   	case 303:
		s = (expr->op == 280 ? "gsub(" : expr->op == 302 ? "sub(" : "substr(");
	...
	case 323:
		...
		return;
	case 308:
		sfprintf(cc->ccdisc->text, "(%s)(", extype(259));
		gen(cc, x);
		sfprintf(cc->ccdisc->text, ")");
		return;
	case 310:
		sfprintf(cc->ccdisc->text, "(%s)(", extype(262));
		gen(cc, x);
		sfprintf(cc->ccdisc->text, ")");
		return;
	case 314:
...
exccopen(Expr_t* expr, Exccdisc_t* disc)
{
	...
	if (!(id = disc->id))
		id = "";
	if (!(cc = ((0)?(Excc_t*)realloc((char*)(0),sizeof(Excc_t)*(1)+(strlen(id) + 2)):(Excc_t*)calloc(1,sizeof(Excc_t)*(1)+(strlen(id) + 2)))))
		return 0;
	cc->expr = expr;
	cc->disc = expr->disc;
	cc->id = (char*)(cc + 1);
	cc->ccdisc = disc;
	if (!(disc->flags & 0x8000))
	{
...
excc(Excc_t* cc, const char* name, Exid_t* sym, int type)
{
	...
	if (!cc)
		return -1;
	if (!sym)
		sym = name ? (Exid_t*)(*(((Dt_t*)(cc->expr->symbols))->searchf))((cc->expr->symbols),(void*)(name),0001000) : &cc->expr->main;
	if (sym && sym->lex == 293 && sym->value)
	{
		t = extype(type);
...
exdump(Expr_t* expr, Exnode_t* node, Sfio_t* sp)
{
	...
	if (node)
		...
		for (sym = (Exid_t*)(*(((Dt_t*)(expr->symbols))->searchf))((expr->symbols),(void*)(0),0000200); sym; sym = (Exid_t*)(*(((Dt_t*)(expr->symbols))->searchf))((expr->symbols),(void*)(sym),0000010))
			if (sym->lex == 293 && sym->value)
			{
				sfprintf(sp, "%s:\n", sym->name);
				gen(cc, sym->value->data.procedure.body);
			}
