#line 1 "graphviz-2.38.0\\lib\\expr\\exopen.c" 1
/* $Id$ $Revision$ */
...
#line 26 "graphviz-2.38.0\\lib\\expr\\exopen.c" 2
#line 1 "graphviz-2.38.0\\lib\\expr\\exopen.c" 1
...
exopen(register Exdisc_t* disc)
{
	...
	if (!(program = ((0)?(Expr_t*)realloc((char*)(0),sizeof(Expr_t)*(1)+(0)):(Expr_t*)calloc(1,sizeof(Expr_t)*(1)+(0)))))
		return 0;
	program->symdisc.key = offsetof(Exid_t, name);
	debug = getenv("VMDEBUG") != 0;
	if (!(program->symbols = dtopen(&program->symdisc, Dtset)) ||
	    !(program->tmp = sfnew((Sfio_t*)0,(char*)0,-1,-1,0000002|0000004)) ||
	    !(program->vm = (debug ? vmopen(Vmdcsbrk, Vmdebug, 0000004|0000010) : vmopen(Vmdcheap, Vmbest, 0))) ||
	    !(program->ve = (debug ? vmopen(Vmdcsbrk, Vmdebug, 0000004|0000010) : vmopen(Vmdcheap, Vmbest, 0))))
	{
		exclose(program, 1);
		return 0;
	}
	...
	program->disc = disc;
	((program)->linep=(program)->line,(program)->linewrap=0);
	program->file[0] = sfstdin;
	program->file[1] = sfstdout;
	program->file[2] = sfstderr;
	strcpy(program->main.name, "main");
	program->main.lex = 293;
	program->main.index = 293;
	(*(((Dt_t*)(program->symbols))->searchf))((program->symbols),(void*)(&program->main),0000001);
	if (!(disc->flags & (1<<4)))
		for (sym = exbuiltin; *sym->name; sym++)
			(*(((Dt_t*)(program->symbols))->searchf))((program->symbols),(void*)(sym),0000001);
	if ((sym = disc->symbols))
		for (; *sym->name; sym++)
			(*(((Dt_t*)(program->symbols))->searchf))((program->symbols),(void*)(sym),0000001);
	return program;
