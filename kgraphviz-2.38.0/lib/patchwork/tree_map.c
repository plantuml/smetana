#line 1 "graphviz-2.38.0\\lib\\patchwork\\tree_map.c" 1
/* $Id$Revision: */
...
#line 16 "graphviz-2.38.0\\lib\\patchwork\\tree_map.c" 2
...
#line 17 "graphviz-2.38.0\\lib\\patchwork\\tree_map.c" 2
...
static void squarify(int n, double *area, rectangle *recs, int nadded, double maxarea, double minarea, double totalarea,
   ...
   */
  double w = ((fillrec.size[0])<(fillrec.size[1])?(fillrec.size[0]):(fillrec.size[1]));
  int i;
  ...
  if (nadded == 0){
    ...
    asp = ((area[0]/(w*w))>((w*w)/area[0])?(area[0]/(w*w)):((w*w)/area[0]));
    totalarea = area[0];
    ...
    if (nadded < n){
      newmaxarea = ((maxarea)>(area[nadded])?(maxarea):(area[nadded]));
      newminarea = ((minarea)<(area[nadded])?(minarea):(area[nadded]));
      s = totalarea + area[nadded];
      h = s/w;
      maxw = newmaxarea/h;
      minw = newminarea/h;
      newasp = ((h/minw)>(maxw/h)?(h/minw):(maxw/h));/* same as MAX{s^2/(w^2*newminarea), (w^2*newmaxarea)/(s^2)}*/
    }
...
rectangle* tree_map(int n, double *area, rectangle fillrec){
  ...
  recs = (rectangle*)zmalloc((n)*sizeof(rectangle));
  squarify(n, area, recs, nadded, maxarea, minarea, totalarea, asp, fillrec);
  return recs;
}
