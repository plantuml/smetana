#line 1 "graphviz-2.38.0\\lib\\patchwork\\patchwork.c" 1
/* $Id$Revision: */
...
#line 18 "graphviz-2.38.0\\lib\\patchwork\\patchwork.c" 2
#line 1 "graphviz-2.38.0\\lib\\patchwork\\tree_map.h" 1
/* $Id$Revision: */
/* vim:set shiftwidth=4 ts=8: */
...
#line 17 "graphviz-2.38.0\\lib\\patchwork\\tree_map.h" 2
...
#line 19 "graphviz-2.38.0\\lib\\patchwork\\patchwork.c" 2
#line 1 "graphviz-2.38.0\\lib\\common\\render.h" 1
...
#line 20 "graphviz-2.38.0\\lib\\patchwork\\patchwork.c" 2
...
{
    double area = late_double (obj, ap, 1.0, 0);
    if (area == 0) area = 1.0;
    area *= 1000.0;
    return area;
}
...
{
    treenode_t *p = (treenode_t*)zmalloc(sizeof(treenode_t));
...
static treenode_t *mkTree (Agraph_t * g, attrsym_t* gp, attrsym_t* ap, attrsym_t* mp)
{
	...
	n_children++;
	area += cp->area;
	if(!first) first=cp; if(prev) prev->rightsib=cp; prev=cp;;
    }

    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	if ((((rdata*)((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->alg)))->parent))
	    continue;
	cp = mkTreeNode (n, ap);
	n_children++;
	area += cp->area;
	if(!first) first=cp; if(prev) prev->rightsib=cp; prev=cp;;
	(((rdata*)((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->alg)))->parent) = g;
    }
...
static void layoutTree(treenode_t * tree)
{
    ...
    nc = tree->n_children;
    nodes = (treenode_t**)zmalloc((nc)*sizeof(treenode_t*));
    cp = tree->leftchild;
    for (i = 0; i < nc; i++) {
	nodes[i] = cp;
	cp = cp->rightsib;
    ...
    areas_sorted = (double*)zmalloc((nc)*sizeof(double));
    for (i = 0; i < nc; i++) {
...
static void finishNode(node_t * n)
{
	...
	if (*str == '\0') {
	    sprintf (buf, "%.03f", (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht)*0.7); 
...
static void walkTree(treenode_t * tree)
{
	...
	r.UR.x = r.LL.x + wd;
	r.UR.y = r.LL.y + ht;
	(((Agraphinfo_t*)(((Agobj_t*)(tree->u.subg))->data))->bb) = r;
    }
    else {
	rr = tree->r;
	center.x = rr.x[0];
	...
	n = tree->u.n;
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord) = center;
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->width) = ((rr.size[0])/(double)72);
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->height) = ((rr.size[1])/(double)72);
	gv_nodesize(n, ((((Agraphinfo_t*)(((Agobj_t*)(agraphof(n)))->data))->rankdir & 0x3) & 1));
	finishNode(n);
	if (Verbose)
	    fprintf(stderr,"%s coord %.5g %.5g ht %f width %f\n",
		agnameof(n), (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).x, (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->coord).y, (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->ht), ((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->lw)+(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->rw)));
    }
}
...
{
    treenode_t* root;
    attrsym_t * ap = (agattr(g,1,"area",(void *)0));
    attrsym_t * gp = (agattr(g,0,"area",(void *)0));
    attrsym_t * mp = (agattr(g,0,"inset",(void *)0));
    double total;
