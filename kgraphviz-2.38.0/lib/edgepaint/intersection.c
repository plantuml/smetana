#line 1 "graphviz-2.38.0\\lib\\edgepaint\\intersection.c" 1
/*************************************************************************
...
#line 12 "graphviz-2.38.0\\lib\\edgepaint\\intersection.c" 2
...
double point_line_distance(double *p, double *q, double *r){
  ...
  if (b <= 1.0e-16) return dist(dim, p, q);
  t = t/b;
 ...
 */
  ...
  snorm = sqrt(snorm);
  b = cross(r, s);
  line_dist_close = (line_segments_distance(p1, p2, q1, q2)  <= close*((rnorm)>(snorm)?(rnorm):(snorm)));
  if (((b) >= 0 ? (b) : -(b)) <= epsilon*snorm*rnorm){/* parallel */
  ...
  if ((t >= 0 && t <= 1 && u >= 0 && u <= 1) /* they intersect */
    ...
    double rs = 0;
    if (rnorm*snorm < 1.0e-16) return 0;
    for (i = 0; i < dim; i++){
      rs += r[i]*s[i];
    }
    res = rs/(rnorm*snorm);
