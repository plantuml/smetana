#line 1 "graphviz-2.38.0\\lib\\mingle\\ink.c" 1
/*************************************************************************
...
#line 15 "graphviz-2.38.0\\lib\\mingle\\ink.c" 2
...
#line 16 "graphviz-2.38.0\\lib\\mingle\\ink.c" 2
...
#line 17 "graphviz-2.38.0\\lib\\mingle\\ink.c" 2
...
#line 18 "graphviz-2.38.0\\lib\\mingle\\ink.c" 2
...
#line 19 "graphviz-2.38.0\\lib\\mingle\\ink.c" 2
...
#line 20 "graphviz-2.38.0\\lib\\mingle\\ink.c" 2
...
static double sumLengths_avoid_bad_angle(point_t* points, int npoints, point_t end, point_t meeting, double angle_param) 
{
  ...
  for (i=0; i<npoints; i++) {
    ...
    sum += len;
    cos_theta = (diff_x0*diff_x + diff_y0*diff_y)/(((len*len0))>(0.00001)?((len*len0)):(0.00001));
    cos_max = ((cos_max)>(cos_theta)?(cos_max):(cos_theta));
  }
...
static double bestInk(point_t* points, int npoints, point_t begin, point_t end, double prec, point_t *meet, double angle_param)
{
  ...
  } while (fabs(value1-value4)/(((value1)<(value4)?(value1):(value4))+1e-10)>prec && dotPoint(diff, diff) > 1.e-20);
...
static double project_to_line(point_t pt, point_t left, point_t right, double angle){
  ...
  */
  ...
  if (angle <=0 || angle >= 3.14159265358979323846) return 2;/* return outside of the interval which should be handled as a sign of infeasible turning angle */
  alpha = angle;

  assert(alpha > 0 && alpha < 3.14159265358979323846);
  b = subPoint(right, left);
  a = subPoint(pt, left);
  bnorm = ((1.e-10)>(dotPoint(b, b))?(1.e-10):(dotPoint(b, b)));
  ccord = dotPoint(b, a)/bnorm;
  dnorm = dotPoint(a,a)/bnorm - ccord*ccord;
  if (alpha == 3.14159265358979323846/2){
    return ccord;
  } else {
    //    assert(dnorm >= MIN(-1.e-5, -1.e-5*bnorm));
    return ccord + sqrt(((0)>(dnorm)?(0):(dnorm)))/tan(alpha);
  }
}
...
double ink(pedge* edges, int numEdges, int *pick, double *ink0, point_t *meet1, point_t *meet2, double angle_param, double angle)
{
  ...
  pedge e;
  double *x;
  point_t* sources = (point_t*)zmalloc((numEdges)*sizeof(point_t));
  point_t* targets = (point_t*)zmalloc((numEdges)*sizeof(point_t));
  double inkUsed;
  //double eps = 1.0e-2;
  double eps = 1.0e-2;
  ...
  *ink0 = 0;
  ...
  if (pick) vector_sort_int(numEdges, pick, (!(0)));
  ...
  for (i = 0; i < numEdges; i++) {
    ...
    if (i == 0){
      ...
      cbegin = ((cbegin)>(project_to_line(sources[i], begin, end, angle))?(cbegin):(project_to_line(sources[i], begin, end, angle)));
      cend = ((cend)>(project_to_line(targets[i], end, begin, angle))?(cend):(project_to_line(targets[i], end, begin, angle)));
    }
  }
  ...
  if (angle > 0 && angle < 3.14159265358979323846){
    ...
    cbegin = ((0)>(cbegin)?(0):(cbegin));/* make sure the new adjusted point is with in [begin,end] internal */
    diff = subPoint(end, begin);
    begin = addPoint(begin, scalePoint(diff, cbegin));
    
    cend = ((0)>(cend)?(0):(cend));/* make sure the new adjusted point is with in [end,begin] internal */
