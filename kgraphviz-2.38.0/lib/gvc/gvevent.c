...
static void gv_graph_state(GVJ_t *job, graph_t *g)
{
    ...
    a = (agattr(g,AGRAPH,s_href,NULL));
    if (!a)
	a = (agattr(g,AGRAPH,s_URL,NULL));
    if (a)
...
static void gv_node_state(GVJ_t *job, node_t *n)
{
    ...
    a = (agattr(agraphof(n),AGNODE,s_href,NULL));
    if (!a)
        a = (agattr(agraphof(n),AGNODE,s_URL,NULL));
    if (a)
...
static void gv_edge_state(GVJ_t *job, edge_t *e)
{
    ...
    a = (agattr(agraphof(aghead(e)),AGEDGE,s_href,NULL));
    if (!a)
	a = (agattr(agraphof(aghead(e)),AGEDGE,s_URL,NULL));
    if (a)
...
static void gvevent_leave_obj(GVJ_t * job)
{
    ...
    if (obj) {
        ...
        case AGRAPH:
	    GD_gui_state((graph_t*)obj) &= ~(1<<0);
	    break;
        case AGNODE:
	    ND_gui_state((node_t*)obj) &= ~(1<<0);
	    break;
        case AGEDGE:
	    ED_gui_state((edge_t*)obj) &= ~(1<<0);
	    break;
        }
    }
...
static void gvevent_enter_obj(GVJ_t * job)
{
    ...
    if (obj) {
	    ...
	    g = (graph_t*)obj;
	    GD_gui_state(g) |= (1<<0);
	    a = (agattr(g,AGRAPH,s_tooltip,NULL));
	    if (a)
	    ...
	    e = (edge_t*)obj;
	    ED_gui_state(e) |= (1<<0);
	    a = (agattr(agraphof(aghead(e)),AGEDGE,s_tooltip,NULL));
	    if (a)
...
static void gvevent_select_current_obj(GVJ_t * job)
{
    ...
    if (obj) {
        switch (agobjkind(obj)) {
        case AGRAPH:
	    GD_gui_state((graph_t*)obj) |= (1<<2);
	    GD_gui_state((graph_t*)obj) &= ~(1<<1);
	    break;
        case AGNODE:
	    ND_gui_state((node_t*)obj) |= (1<<2);
	    ND_gui_state((node_t*)obj) &= ~(1<<1);
	    break;
        case AGEDGE:
	    ED_gui_state((edge_t*)obj) |= (1<<2);
	    ED_gui_state((edge_t*)obj) &= ~(1<<1);
	    break;
        }
    }
...
static void gvevent_read (GVJ_t * job, const char *filename, const char *layout)
{
    ...
    aginit (g, AGRAPH, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));
    aginit (g, AGNODE, "Agnodeinfo_t", sizeof(Agnodeinfo_t), NOT(0));
    aginit (g, AGEDGE, "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));
    gvc->g = g;
    GD_gvc(g) = gvc;
