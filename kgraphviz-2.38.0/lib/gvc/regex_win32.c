#line 1 "graphviz-2.38.0\\lib\\gvc\\regex_win32.c" 1
/* #define DEBUG */
...
/* #include "regex.h" */
#line 1 "graphviz-2.38.0\\lib\\gvc\\regex_win32.h" 1
/* LGPLd GNU regex for Native WIN32 */
...
/*
  ...
  End:
*/
#line 150 "graphviz-2.38.0\\lib\\gvc\\regex_win32.c" 2
            ...
            switch (c)
                ...
                case '*':
...

                    {
                        ...
                        while ((unsigned long) (b - bufp->buffer + (3)) > bufp->allocated) do { unsigned char *old_buffer = bufp->buffer; if (bufp->allocated == (1L << 16)) return REG_ESIZE; bufp->allocated <<= 1; if (bufp->allocated > (1L << 16)) bufp->allocated = (1L << 16); bufp->buffer = (unsigned char *) realloc ((bufp->buffer), (bufp->allocated)); if (bufp->buffer == (void *)0) return REG_ESPACE; if (old_buffer != bufp->buffer) { b = (b - old_buffer) + bufp->buffer; begalt = (begalt - old_buffer) + bufp->buffer; if (fixup_alt_jump) fixup_alt_jump = (fixup_alt_jump - old_buffer) + bufp->buffer; if (laststart) laststart = (laststart - old_buffer) + bufp->buffer; if (pending_exact) pending_exact = (pending_exact - old_buffer) + bufp->buffer; } } while (0);
                        insert_op1 (keep_string_p ? on_failure_keep_string_jump
 : on_failure_jump, 
 laststart, (int) ((b + 3) - (
 laststart) - 3), b);
                        pending_exact = 0;
                        ...
                        handle_interval:
                            {
                                ...
                                { if (p != pend) { do {if (p == pend) return REG_EEND; c = (unsigned char) *p++; if (translate) c = (unsigned char) translate[c]; } while (0); while ((1 && isdigit (c))) { if (lower_bound < 0) lower_bound = 0; lower_bound = lower_bound * 10 + c - '0'; if (p == pend) break; do {if (p == pend) return REG_EEND; c = (unsigned char) *p++; if (translate) c = (unsigned char) translate[c]; } while (0); } } };

                                if (c == ',')
                                    {
                                        { if (p != pend) { do {if (p == pend) return REG_EEND; c = (unsigned char) *p++; if (translate) c = (unsigned char) translate[c]; } while (0); while ((1 && isdigit (c))) { if (upper_bound < 0) upper_bound = 0; upper_bound = upper_bound * 10 + c - '0'; if (p == pend) break; do {if (p == pend) return REG_EEND; c = (unsigned char) *p++; if (translate) c = (unsigned char) translate[c]; } while (0); } } };
                                ...
                                /* Otherwise, we have a nontrivial interval.  When
                                   ...
                                   `upper_bound' is 1, though.)  */
                                    ...
                                    { /* If the upper bound is > 1, we need to insert
                                        ...
                                        insert_op2 (succeed_n, laststart, (int) ((
 b + 5 +(upper_bound > 1) * 5) - (laststart) - 3), 
 lower_bound, b);
                                        b += 5;
                                        ...
                                        if (upper_bound > 1)
                                            { /* More than one repetition is allowed, so
                                                ...
                                                store_op2 (jump_n, b, (int) ((laststart + 5) - (b) - 3), 
 upper_bound - 1);
                                                b += 5;
...
re_compile_fastmap(struct re_pattern_buffer *bufp)
{
    ...
    do { fail_stack.stack = (fail_stack_elt_t *) alloca (5 * sizeof (fail_stack_elt_t)); if (fail_stack.stack == (void *)0) return -2; fail_stack.size = 5; fail_stack.avail = 0; } while (0);
    bzero (fastmap, 1 << 8);  /* Assume nothing's valid.  */
    ...
    while (1)
        {
            ...
            switch (((re_opcode_t) *p++))
                ...
                case dummy_failure_jump:
                    do { do { (j) = *(p) & 0377; (j) += ((((unsigned char) (*((p) + 1))) ^ 128) - 128) << 8; } while (0); (p) += 2; } while (0);
                    p += j;
                    if (j > 0)
                        continue;
                ...
                if (succeed_n_p)
                    {
                        do { do { (k) = *(p) & 0377; (k) += ((((unsigned char) (*((p) + 1))) ^ 128) - 128) << 8; } while (0); (p) += 2; } while (0); /* Skip the n.  */
                        succeed_n_p = 0;
                    }
                ...
                case succeed_n:
                    ...
                    do { do { (k) = *(p) & 0377; (k) += ((((unsigned char) (*((p) + 1))) ^ 128) - 128) << 8; } while (0); (p) += 2; } while (0);
                    if (k == 0)
                        {
                            p -= 4;
                            succeed_n_p = 1;  /* Spaghetti code alert.  */
    ...
    for (;;)
        {
            ...
            switch (((re_opcode_t) *p++))
                ...
                case charset_not:
                    {
                        ...
                        while (d == dend) { if (dend == end_match_2) goto fail; d = string2; dend = end_match_2; };
                        c = (translate ? (char) translate[(unsigned char) (*d)] : (*d)); /* The character to match.  */
                    ...
                    if ((!((reg_info[*p]).bits.matched_something)
                        ...
                        && (p + 2) < pend)
                        {
                            ...
                            switch ((re_opcode_t) *p1++)
                                ...
                                case jump:
                                case dummy_failure_jump:
                                    do { do { (mcnt) = *(p1) & 0377; (mcnt) += ((((unsigned char) (*((p1) + 1))) ^ 128) - 128) << 8; } while (0); (p1) += 2; } while (0);
                                    if (is_a_jump_n)
                                        p1 += 2;
                                    break;
                ...
                case on_failure_keep_string_jump:
                    ...
                    do { do { (mcnt) = *(p) & 0377; (mcnt) += ((((unsigned char) (*((p) + 1))) ^ 128) - 128) << 8; } while (0); (p) += 2; } while (0);
                ...
                case pop_failure_jump:
                    {
                        ...
                        ;
                        {  s_reg_t this_reg; const unsigned char *string_temp; ; ; ; ; ; ; ; string_temp = fail_stack.stack[--fail_stack.avail].pointer; if (string_temp != (void *)0) sdummy = (const char *) string_temp; ; ; ; pdummy = (unsigned char *) fail_stack.stack[--fail_stack.avail].pointer; ; ; dummy_high_reg = (active_reg_t) fail_stack.stack[--fail_stack.avail].integer; ; 
 dummy_low_reg = (active_reg_t) fail_stack.stack[--fail_stack.avail].integer; ; if (1) for (this_reg = dummy_high_reg; this_reg >= 
 dummy_low_reg; this_reg--) { ; reg_info_dummy[this_reg].word = fail_stack.stack[--fail_stack.avail]; ; reg_dummy[this_reg] = (const char *) fail_stack.stack[--fail_stack.avail].pointer; ; 
 reg_dummy[this_reg] = (const char *) fail_stack.stack[--fail_stack.avail].pointer; ; } else { for (this_reg = highest_active_reg; this_reg > dummy_high_reg; this_reg--) { reg_info_dummy[this_reg].word.integer = 0; reg_dummy[this_reg] = 0; 
 reg_dummy[this_reg] = 0; } highest_active_reg = dummy_high_reg; } set_regs_matched_done = 0; ; };
                    }
                    /* Note fall through.  */
                ...
                case jump:
                    do { do { (mcnt) = *(p) & 0377; (mcnt) += ((((unsigned char) (*((p) + 1))) ^ 128) - 128) << 8; } while (0); (p) += 2; } while (0);  /* Get the amount to jump.  */
                    ;
                    p += mcnt;                          /* Do the jump.  */
                ...
                case succeed_n:
                    do { (mcnt) = *(p + 2) & 0377; (mcnt) += ((((unsigned char) (*((p + 2) + 1))) ^ 128) - 128) << 8; } while (0);
                    ;
        ...
        fail:
            if (!(fail_stack.avail == 0))
                { /* A restart point is known.  Restore to that state.  */
                    ;
                    {  s_reg_t this_reg; const unsigned char *string_temp; ; ; ; ; ; ; ; string_temp = fail_stack.stack[--fail_stack.avail].pointer; if (string_temp != (void *)0) d = (const char *) string_temp; ; ; ; p = (unsigned char *) fail_stack.stack[--fail_stack.avail].pointer; ; ; highest_active_reg = (active_reg_t) fail_stack.stack[--fail_stack.avail].integer; ; 
 lowest_active_reg = (active_reg_t) fail_stack.stack[--fail_stack.avail].integer; ; if (1) for (this_reg = highest_active_reg; this_reg >= 
 lowest_active_reg; this_reg--) { ; reg_info[this_reg].word = fail_stack.stack[--fail_stack.avail]; ; regend[this_reg] = (const char *) fail_stack.stack[--fail_stack.avail].pointer; ; 
 regstart[this_reg] = (const char *) fail_stack.stack[--fail_stack.avail].pointer; ; } else { for (this_reg = highest_active_reg; this_reg > highest_active_reg; this_reg--) { reg_info[this_reg].word.integer = 0; regend[this_reg] = 0; 
 regstart[this_reg] = 0; } highest_active_reg = highest_active_reg; } set_regs_matched_done = 0; ; };
                    ...
                    if (mcnt >= 0)
                        {
                            ...
                            do { (mcnt) = *(p1 - 2) & 0377; (mcnt) += ((((unsigned char) (*((p1 - 2) + 1))) ^ 128) - 128) << 8; } while (0);
