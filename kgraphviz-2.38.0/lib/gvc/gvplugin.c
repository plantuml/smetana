...
char **gvPluginList(GVC_t * gvc, char *kind, int *sz, const char *str)
{
    ...
    for (pnext = plugin; *pnext; pnext = &((*pnext)->next)) {
        ...
        if ((p = strchr(q, ':')))
            *p++ = '\0';
        if (!typestr_last || strcasecmp(typestr_last, q) != 0) {
            list = ((char **)grealloc(list,(cnt + 1)*sizeof(char *)));
            list[cnt++] = q;
        }
        typestr_last = q;
    }
...
Agraph_t *gvplugin_graph(GVC_t * gvc)
{
    ...
    a = (agattr(g,0,"rankdir",NULL));
    agxset(g, a, "LR");

    a = (agattr(g,0,"ranksep",NULL));
    agxset(g, a, "2.5");

    a = (agattr(g,0,"label",NULL));
    agxset(g, a, "Plugins");
        ...
        for (api = 0; api < (sizeof(api_names)/sizeof(api_names[0])); api++) {
			...
			lq = q;
			    ...
			    lq = "tiff\\ntif";
                        ...
                        strcpy(buf2, q);
                        n = agnode(ssg, bufa, 1);
                        a = (agattr(g,1,"label",NULL));
                        agxset(n, a, lq);
                        a = (agattr(g,1,"width",NULL));
                        agxset(n, a, "1.0");
                        a = (agattr(g,1,"shape",NULL));
			if (api == API_device) {
                            agxset(n, a, "box");
                            device_n = n;
			}
