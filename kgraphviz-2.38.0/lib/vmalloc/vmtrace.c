#line 1 "graphviz-2.38.0\\lib\\vmalloc\\vmtrace.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\vmalloc\\vmtrace.c" 2
...

{
    ...
    *s-- = '\0';
    ...
    if (type == 0) {		/* base-16 */
	register char *digit = "0123456789abcdef";
	do {
	    *s-- = digit[v & 0xf];
	    v >>= 4;
	} while (v);
...

{
    ...
    bufp = trstrcpy(bufp, tritoa(oldaddr ? ((Vmulong_t)((char*)(oldaddr) - (char*)0) ) : 0L, 0), ':');
    bufp = trstrcpy(bufp, tritoa(newaddr ? ((Vmulong_t)((char*)(newaddr) - (char*)0) ) : 0L, 0), ':');
    bufp = trstrcpy(bufp, tritoa((Vmulong_t) size, 1), ':');
    bufp = trstrcpy(bufp, tritoa((Vmulong_t) align, 1), ':');
    bufp = trstrcpy(bufp, tritoa(((Vmulong_t)((char*)(vm) - (char*)0) ), 0), ':');
    if (type & 0000100)
	bufp = trstrcpy(bufp, "best", ':');
    else if (type & 0000400)
	bufp = trstrcpy(bufp, "last", ':');
    else if (type & 0000200)
	bufp = trstrcpy(bufp, "pool", ':');
    else if (type & 0002000)
	bufp = trstrcpy(bufp, "profile", ':');
    else if (type & 0001000)
	bufp = trstrcpy(bufp, "debug", ':');
    else
	bufp = trstrcpy(bufp, "busy", ':');
    if (file && file[0] && line > 0
	&& (bufp + strlen(file) + 32) < endbuf) {
...

{
    ...
    (_Vmextern.vm_strcpy) = trstrcpy;
    (_Vmextern.vm_itoa) = tritoa;
    (_Vmextern.vm_trace) = trtrace;
...

{
    ...
    if (Trfile < 0
	|| !(vd->mode & (0000100 | 0001000 | 0002000)))
	return -1;
	...
	for (b = ((Block_t*)(((Vmuchar_t*)(seg)) + (((sizeof(struct _align_s))&((sizeof(struct _align_s))-1)) ? (((((sizeof(Seg_t))) + (((sizeof(struct _align_s)))-1)) / ((sizeof(struct _align_s)))) * ((sizeof(struct _align_s)))) : ((((sizeof(Seg_t))) + (((sizeof(struct _align_s)))-1)) & ~(((sizeof(struct _align_s)))-1)) ))), endb = ((Block_t*)((char*)(seg->baddr) - sizeof(Head_t)) ); b < endb;) {
	    if (((((b)->head.head.size.size)) & (04)) || !((((b)->head.head.size.size)) & (01)))
		continue;

	    data = ((Void_t*)((b)->body.data) );
	    if (vd->mode & 0001000) {
		data = ((Vmuchar_t*)(data) + 2*sizeof(Head_t));
		s = (((Head_t*)((Vmuchar_t*)(data) - 2*sizeof(Head_t)))->head.size.size );
