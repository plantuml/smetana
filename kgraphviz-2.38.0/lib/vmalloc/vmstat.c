#line 1 "graphviz-2.38.0\\lib\\vmalloc\\vmstat.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\vmalloc\\vmstat.c" 2
...

{
    ...
    if (vd->mode & 0000400)
	st->n_busy = 0;
    else if ((vd->mode & 0000200) && (s = vd->pool) > 0) {
	s = (((sizeof(struct _align_s))&((sizeof(struct _align_s))-1)) ? (((((s)) + (((sizeof(struct _align_s)))-1)) / ((sizeof(struct _align_s)))) * ((sizeof(struct _align_s)))) : ((((s)) + (((sizeof(struct _align_s)))-1)) & ~(((sizeof(struct _align_s)))-1)) );
	for (b = vd->free; b; b = ((b)->head.head.seg.link))
	    st->n_free += 1;
    }
	...
	endb = ((Block_t*)((char*)(seg->baddr) - sizeof(Head_t)) );
	...
	if (vd->mode & (0001000 | 0000100 | 0002000)) {
		...
		if (((((b)->head.head.size.size)) & (04)) || !((((b)->head.head.size.size)) & (01))) {
		    ...
		    if (vd->mode & 0001000)
			s = (((Head_t*)((Vmuchar_t*)(((Vmuchar_t*)(((Void_t*)((b)->body.data) )) + 2*sizeof(Head_t))) - 2*sizeof(Head_t)))->head.size.size );
		    else if (vd->mode & 0002000)
			s = (((Head_t*)((Vmuchar_t*)(((Void_t*)((b)->body.data)))+(((((Block_t*)((char*)(((Void_t*)((b)->body.data))) - sizeof(Head_t)) ))->head.head.size.size)&~(07))-sizeof(Head_t)) )->head.size.size);
		    if (s > st->m_busy)
			st->m_busy = s;
		    st->s_busy += s;
		    st->n_busy += 1;
		...
		b = (Block_t *) ((Vmuchar_t *) ((Void_t*)((b)->body.data) ) +
				 (((b)->head.head.size.size) & ~(07)));
	    }
	} else if (vd->mode & 0000400) {
	    if ((s =
		 seg->free ? (((seg->free)->head.head.size.size) + sizeof(Head_t)) : 0) > 0) {
		st->s_free += s;
		st->n_free += 1;
	    }
	...
	} else if ((vd->mode & 0000200) && s > 0) {
	    if (seg->free)
		st->n_free += (((seg->free)->head.head.size.size) + sizeof(Head_t)) / s;
	    st->n_busy +=
		((seg->baddr - (Vmuchar_t *) b) - sizeof(Head_t)) / s;
	}
    }

    if ((vd->mode & 0000200) && s > 0) {
	st->n_busy -= st->n_free;
	if (st->n_busy > 0)
