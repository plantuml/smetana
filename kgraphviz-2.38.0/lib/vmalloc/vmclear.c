#line 1 "graphviz-2.38.0\\lib\\vmalloc\\vmclear.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\vmalloc\\vmclear.c" 2
...

{
	...
	for (s = 0; s < 7; ++s)
	    ((vd)->tiny)[s] = ((Block_t *)0);
	for (s = 0; s <= 7; ++s)
	    ((vd)->cache)[s] = ((Block_t *)0);
    }

    for (seg = vd->seg; seg; seg = next) {
	next = seg->next;

	tp = ((Block_t*)(((Vmuchar_t*)(seg)) + (((sizeof(struct _align_s))&((sizeof(struct _align_s))-1)) ? (((((sizeof(Seg_t))) + (((sizeof(struct _align_s)))-1)) / ((sizeof(struct _align_s)))) * ((sizeof(struct _align_s)))) : ((((sizeof(Seg_t))) + (((sizeof(struct _align_s)))-1)) & ~(((sizeof(struct _align_s)))-1)) )));
	...
	tp = ((Block_t*)((char*)(seg->baddr) - sizeof(Head_t)) );
	((tp)->head.head.seg.seg) = seg;
