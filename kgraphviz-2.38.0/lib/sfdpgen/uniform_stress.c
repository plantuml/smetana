#line 1 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 2
...
#line 17 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 2
...
#line 18 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 2
...
#line 19 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 2
...
#line 20 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 2
#line 1 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 1
...
#line 22 "graphviz-2.38.0\\lib\\sfdpgen\\uniform_stress.c" 2
...
UniformStressSmoother UniformStressSmoother_new(int dim, SparseMatrix A, double *x, double alpha, double M, int *flag){
  ...
  sm->lambda = (void *)0;
  sm->data = gmalloc(sizeof(double)*2);
  ...
  for (i = 0; i < m; i++){
    ...
    for (j = ia[i]; j < ia[i+1]; j++){
      k = ja[j];
      if (k != i){
	dist = ((((a[j]) >= 0 ? (a[j]) : -(a[j])))>(epsilon)?(((a[j]) >= 0 ? (a[j]) : -(a[j]))):(epsilon));
	jd[nz] = jw[nz] = k;
	w[nz] = -1/(dist*dist);
	w[nz] = -1.;
	d[nz] = w[nz]*dist;
...
double UniformStressSmoother_smooth(UniformStressSmoother sm, int dim, double *x, int maxit_sm) {

  return StressMajorizationSmoother_smooth(sm, dim, x, maxit_sm, 0.001);

}

SparseMatrix get_distance_matrix(SparseMatrix A, double scaling){
...
void uniform_stress(int dim, SparseMatrix A, double *x, int *flag){
  ...
  int maxit = 300, samepoint = (!(0)), i, k, n = A->m;
  ...
  *flag = 0;
  ...
  for (i = 1; i < n; i++){
    for (k = 0; k < dim; k++) {
      if (((x[0*dim+k] - x[i*dim+k]) >= 0 ? (x[0*dim+k] - x[i*dim+k]) : -(x[0*dim+k] - x[i*dim+k])) > 1.0e-16){
	samepoint = (0);
	i = n;
	break;
      }
  ...
  assert(SparseMatrix_is_symmetric(B, (0)));
