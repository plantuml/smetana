#line 1 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 1
/* $Id$ $Revision$ */
...
#line 22 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
...
#line 23 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
...
#line 24 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2

#line 1 "graphviz-2.38.0\\lib\\sfdpgen\\sparse_solve.h" 1
/* $Id$ $Revision$ */
/* vim:set shiftwidth=4 ts=8: */
...
#line 18 "graphviz-2.38.0\\lib\\sfdpgen\\sparse_solve.h" 2
...
double cg(Operator Ax, Operator precond, int n, int dim, double *x0, double *rhs, double tol, int maxit, int *flag);
...
#line 26 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
...
#line 27 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
...
#line 28 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
#line 1 "graphviz-2.38.0\\lib\\sfdpgen\\spring_electrical.h" 1
/* $Id$ $Revision$ */
/* vim:set shiftwidth=4 ts=8: */
...
#line 29 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
#line 1 "graphviz-2.38.0\\lib\\neatogen\\call_tri.h" 1
/* $Id$ $Revision$ */
/* vim:set shiftwidth=4 ts=8: */
...
#line 30 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
#line 1 "graphviz-2.38.0\\lib\\sfdpgen\\sfdpinternal.h" 1
/* $Id$ $Revision$ */
/* vim:set shiftwidth=4 ts=8: */
...
#line 17 "graphviz-2.38.0\\lib\\sfdpgen\\sfdpinternal.h" 2


 
#line 31 "graphviz-2.38.0\\lib\\sfdpgen\\post_process.c" 2
...
SparseMatrix ideal_distance_matrix(SparseMatrix A, int dim, double *x){
  ...
  if (D->type != MATRIX_TYPE_REAL){
    ...
    D->a = (double*)gmalloc((D->nz)*sizeof(double));
  }
  d = (double*) D->a;

  mask = (int*)gmalloc((D->m)*sizeof(int));
...
StressMajorizationSmoother StressMajorizationSmoother2_new(SparseMatrix A, int dim, double lambda0, double *x, 
  ...
  sm = (struct StressMajorizationSmoother_struct*)gmalloc(sizeof(struct StressMajorizationSmoother_struct));
  sm->scaling = 1.;
  sm->data = (void *)0;
  ...
  lambda = sm->lambda = (double*)gmalloc((m)*sizeof(double));
  for (i = 0; i < m; i++) sm->lambda[i] = lambda0;
  mask = (int*)gmalloc((m)*sizeof(int));

  avg_dist = (double*)gmalloc((m)*sizeof(double));
...
StressMajorizationSmoother SparseStressMajorizationSmoother_new(SparseMatrix A, int dim, double lambda0, double *x,
  ...
  assert(SparseMatrix_is_symmetric(A, 0) && A->type == MATRIX_TYPE_REAL);
  ...
  sm = gmalloc(sizeof(struct StressMajorizationSmoother_struct));
  sm->scaling = 1.;
  sm->data = (void *)0;
  sm->scheme = SM_SCHEME_NORMAL;
  sm->D = A;
  ...
  lambda = sm->lambda = gmalloc(sizeof(double)*m);
  for (i = 0; i < m; i++) sm->lambda[i] = lambda0;
  ...
  if (edge_labeling_scheme == ELSCHEME_PENALTY || edge_labeling_scheme == ELSCHEME_STRAIGHTLINE_PENALTY){
    ...
    */
	...
	nz += (int)((k+1)*(k+1));
      ...
      irn = data->irn = gmalloc(sizeof(int)*nz);
      jcn = data->jcn = gmalloc(sizeof(int)*nz);
      val = data->val = gmalloc(sizeof(double)*nz);
    }
    nz = 0;
    for (i = 0; i < n_constr_nodes; i++){
      ii = constr_nodes[i];
    ...
    */
    ...
    x00 = gmalloc(sizeof(double)*m*dim);
...
static double uniform_stress_solve(SparseMatrix Lw, double alpha, int dim, double *x0, double *rhs, double tol, int maxit, int *flag){
  Operator Ax;
  Operator Precon;
...
double StressMajorizationSmoother_smooth(StressMajorizationSmoother sm, int dim, double *x, int maxit_sm, double tol) {
  SparseMatrix Lw = sm->Lw, Lwd = sm->Lwd, Lwdd = (void *)0;
  int i, j, k, m, *id, *jd, *iw, *jw, idiag, flag = 0, iter = 0;
  double *w, *dd, *d, *y = (void *)0, *x0 = (void *)0, *x00 = (void *)0, diag, diff = 1, *lambda = sm->lambda, res, alpha = 0., M = 0.;
  SparseMatrix Lc = (void *)0;
  double dij, dist;
  ...
  m = Lw->m;
  x0 = (double*)gmalloc((dim*m)*sizeof(double));
  if (!x0) goto RETURN;

  x0 = memcpy(x0, x, sizeof(double)*dim*m);
  y = (double*)gmalloc((dim*m)*sizeof(double));
  if (!y) goto RETURN;
  ...
  while (iter++ < maxit_sm && diff > tol){
	...
	dd[idiag] = -diag;
      ...
      SparseMatrix_multiply_dense(Lwdd, 0, x, 0, &y, 0, dim);
    } else {
      for (i = 0; i < m; i++){
	for (j = 0; j < dim; j++){
...
TriangleSmoother TriangleSmoother_new(SparseMatrix A, int dim, double lambda0, double *x, int use_triangularization){
  ...
  sm = (struct StressMajorizationSmoother_struct*)gmalloc((1)*sizeof(struct StressMajorizationSmoother_struct));
  sm->scaling = 1;
  sm->data = (void *)0;
  sm->scheme = SM_SCHEME_NORMAL;
  sm->tol_cg = 0.01;
...
SpringSmoother SpringSmoother_new(SparseMatrix A, int dim, spring_electrical_control ctrl, double *x){
  ...
  sm = (struct SpringSmoother_struct*)gmalloc((1)*sizeof(struct SpringSmoother_struct));
  mask = (int*)gmalloc((m)*sizeof(int));
  ...
  *flag = 0;
  ...
  case SMOOTHING_TRIANGLE:{
    ...
    if (ctrl->smoothing == SMOOTHING_RNG){
      sm = TriangleSmoother_new(A, dim, 0, x, 0);
    } else {
      sm = TriangleSmoother_new(A, dim, 0, x, (!(0)));
    }
