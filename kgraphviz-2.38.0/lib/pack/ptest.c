...
static void initPos(Agraph_t * g)
{
    ...
    point *sp;
    int pn;
    attrsym_t *N_pos = (agattr(g,AGNODE,"pos",NULL));
	...
	ND_coord_i(n).x = (ROUND((ND_pos(n)[0])*72));
	ND_coord_i(n).y = (ROUND((ND_pos(n)[1])*72));
    }
...
static void ptest_nodesize(node_t * n, boolean flip)
{
    ...
    w = ND_xsize(n) = (ROUND((ND_width(n))*72));
    ND_lw(n) = ND_rw(n) = w / 2;
    ND_ht(n) = ND_ysize(n) = (ROUND((ND_height(n))*72));
}
...
static void ptest_initNode(node_t * n)
{
    ...
    ND_label(n) = make_label(str,
			     ...
			     n->graph);
    ND_shape(n) = bind_shape(late_nnstring(n, N_shape, "ellipse"));
