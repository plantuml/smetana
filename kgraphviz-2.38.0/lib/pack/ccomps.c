#line 1 "graphviz-2.38.0\\lib\\pack\\ccomps.c" 1
/* $Id$ $Revision$ */
...
#line 19 "graphviz-2.38.0\\lib\\pack\\ccomps.c" 2
...
#line 20 "graphviz-2.38.0\\lib\\pack\\ccomps.c" 2
...
static void push(stk_t* sp, Agnode_t * np)
{
	...
	if (sp->curblk->next == NULL) {
	    blk_t *bp = (blk_t*)gmalloc(sizeof(blk_t));
	    if (bp == 0) {
	    ...
	    bp->prev = sp->curblk;
	    bp->next = NULL;
	    bp->data = (Agnode_t **)gmalloc((1000000)*sizeof(Agnode_t *));
	    if (bp->data == 0) {
		agerr(AGERR, "gc: Out of memory\n");
		longjmp(jbuf, 1);
	    }
	    bp->endp = bp->data + 1000000;
	    sp->curblk->next = bp;
	}
...
static int dfs(Agraph_t * g, Agnode_t * n, void *state, stk_t* stk)
{
	...
	if (stk->actionfn) stk->actionfn(n, state);
	    ...
	    if ((other = ((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node)) == n)
		other = ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node);
            if (!((stk)->markfn(other,-1)))
                push(stk, other);
        }
    }
    return cnt;
...
{
    int ret;
    if (v < 0) return (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->mark);
    ret = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->mark);
    (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->mark) = v;
    return ret;
}
...
Agraph_t **pccomps(Agraph_t * g, int *ncc, char *pfx, boolean * pinned)
{
	...
	*ncc = 0;
    ...
    name = setPrefix (pfx, &len, buffer, 128);

    ccs = (Agraph_t **)gmalloc((bnd)*sizeof(Agraph_t *));
    ...
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	if (((&stk)->markfn(n,-1)) || !((((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pinned) == 3))
	    continue;
	if (!out) {
	    sprintf(name + len, "%d", c_cnt);
	    out = agsubg(g, name,1);
	    agbindrec(out, "Agraphinfo_t", sizeof(Agraphinfo_t), (!(0)));	//node custom data
	    ccs[c_cnt] = out;
	    c_cnt++;
	    pin = (!(0));
	}
	...
	if (c_cnt == bnd) {
	    bnd *= 2;
	    ccs = ((Agraph_t **)grealloc(ccs,(bnd)*sizeof(Agraph_t *)));
	}
	ccs[c_cnt] = out;
	c_cnt++;
    }
packerror:
	...
	*ncc = 0;
	...
	ccs = ((Agraph_t **)grealloc(ccs,(c_cnt)*sizeof(Agraph_t *)));
	*ncc = c_cnt;
	*pinned = pin;
    }
    return ccs;
...
static void deriveClusters(Agraph_t* dg, Agraph_t * g)
{
	...
	if ((strncmp(agnameof(subg), "cluster", 7) == 0)) {
	    dn = agnode(dg, agnameof(subg), 1);
	    agbindrec (dn, "ccgnodeinfo", sizeof(ccgnodeinfo_t), (!(0)));
	    (((ccgnodeinfo_t*)((dn)->base.data))->ptr.g) = subg;
	    for (n = agfstnode(subg); n; n = agnxtnode(subg, n)) {
		if ((((ccgnodeinfo_t*)aggetrec(n, "ccgnodeinfo", 0))->ptr.n)) {
		   fprintf (stderr, "Error: node \"%s\" belongs to two non-nested clusters \"%s\" and \"%s\"\n",
			agnameof (n), agnameof(subg), agnameof((((ccgnodeinfo_t*)aggetrec(n, "ccgnodeinfo", 0))->ptr.n)));  
		}
		(((ccgnodeinfo_t*)aggetrec(n, "ccgnodeinfo", 0))->ptr.n=dn);
	    }
	}
	else {
...
static Agraph_t *deriveGraph(Agraph_t * g)
{
	...
	agbindrec (dn, "ccgnodeinfo", sizeof(ccgnodeinfo_t), (!(0)));
	(((ccgnodeinfo_t*)((dn)->base.data))->ptr.n) = n;
	(((ccgnodeinfo_t*)aggetrec(n, "ccgnodeinfo", 0))->ptr.n=dn);
    }
	...
	Agedge_t *e;
	Agnode_t *hd;
	Agnode_t *tl = (((ccgnodeinfo_t*)aggetrec(n, "ccgnodeinfo", 0))->ptr.n);
	for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
	    hd = ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node);
	    hd = (((ccgnodeinfo_t*)aggetrec(hd, "ccgnodeinfo", 0))->ptr.n);
	    if (hd == tl)
		continue;
	    if (hd > tl)
		agedge(dg, tl, hd, 0, 1);
...
static void unionNodes(Agraph_t * dg, Agraph_t * g)
{
	...
	if (((((Agobj_t*)((((ccgnodeinfo_t*)((dn)->base.data))->ptr.v)))->tag).objtype) == 1) {
	    agsubnode(g, (((ccgnodeinfo_t*)((dn)->base.data))->ptr.n), 1);
	} else {
	    clust = (((ccgnodeinfo_t*)((dn)->base.data))->ptr.g);
...
{
    int ret;
    if (v < 0) return (((ccgnodeinfo_t*)(n->base.data))->mark);
    ret = (((ccgnodeinfo_t*)(n->base.data))->mark);
    (((ccgnodeinfo_t*)(n->base.data))->mark) = v;
    return ret;
}
...
int node_induce(Agraph_t * g, Agraph_t* eg)
{
	...
	for (e = agfstout(eg, n); e; e = agnxtout(eg, e)) {
	    if (agsubnode(g, ((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node),0)) {
		agsubedge(g,e,1);
		e_cnt++;
	    }
	}
...
/* fprintf (stderr, "subgInduce %s inCluster %d\n", agnameof(root), inCluster); */
	...
	if ((((ccgraphinfo_t*)aggetrec(subg, "ccgraphinfo", 0))->cc_subg))
	    continue;
	if ((proj = projectG(subg, g, inCluster))) {
	    in_cluster = (inCluster || (strncmp(agnameof(subg), "cluster", 7) == 0));
...
Agraph_t **cccomps(Agraph_t * g, int *ncc, char *pfx)
{
	...
	*ncc = 0;
    ...
    aginit(g, 0, "ccgraphinfo", -sz, 0);

    /* Bind ccgraphinfo to graph and all subgraphs */
    aginit(g, 1, "ccgnodeinfo", sizeof(ccgnodeinfo_t), 0);

    name = setPrefix (pfx, &len, buffer, 128);

    dg = deriveGraph(g);

    ccs = (Agraph_t **)gmalloc((agnnodes(dg))*sizeof(Agraph_t *));
	...
	out = agsubg(g, name, 1);
	agbindrec(out, "ccgraphinfo", sizeof(ccgraphinfo_t), 0);
	(((ccgraphinfo_t*)aggetrec(out, "ccgraphinfo", 0))->cc_subg) = 1;
    ...
    agclose(dg);
    agclean (g, 0, "ccgraphinfo");
    agclean (g, 1, "ccgnodeinfo");
    freeStk (&stk);
    ccs = ((Agraph_t **)grealloc(ccs,(c_cnt)*sizeof(Agraph_t *)));
    if (name != buffer)
	free(name);
