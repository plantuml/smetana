...
Agraph_t **pccomps(Agraph_t * g, int *ncc, char *pfx, boolean * pinned)
{
	...
	*ncc = 0;
    ...
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	if (((&stk)->markfn(n,-1)) || !(ND_pinned(n) == 3))
	    continue;
	if (!out) {
	    sprintf(name + len, "%d", c_cnt);
	    out = agsubg(g, name,1);
	    agbindrec(out, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	//node custom data
	    ccs[c_cnt] = out;
	    c_cnt++;
	    pin = NOT(0);
	}
...
static void deriveClusters(Agraph_t* dg, Agraph_t * g)
{
	...
	if ((strncmp(agnameof(subg), "cluster", 7) == 0)) {
	    ...
	    agbindrec (dn, "ccgnodeinfo", sizeof(ccgnodeinfo_t), NOT(0));
...
static void unionNodes(Agraph_t * dg, Agraph_t * g)
{
	...
	if (AGTYPE((((ccgnodeinfo_t*)((dn)->base.data))->ptr.v)) == AGNODE) {
...
Agraph_t **cccomps(Agraph_t * g, int *ncc, char *pfx)
{
	...
	*ncc = 0;
    ...
    aginit(g, AGRAPH, "ccgraphinfo", -sz, 0);

    /* Bind ccgraphinfo to graph and all subgraphs */
    aginit(g, AGNODE, "ccgnodeinfo", sizeof(ccgnodeinfo_t), 0);
    ...
    agclose(dg);
    agclean (g, AGRAPH, "ccgraphinfo");
    agclean (g, AGNODE, "ccgnodeinfo");
    freeStk (&stk);
    ccs = RALLOC(c_cnt, ccs, Agraph_t *);
    if (name != buffer)
	free(name);
