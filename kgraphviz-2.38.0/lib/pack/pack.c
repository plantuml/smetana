	...
	int ssize, pack_info * pinfo, point center)
{
    ...
    info->cells = pointsOf(ps);
    info->nc = sizeOf(ps);
    W = ((int)ceil((GD_bb(g).UR.x - GD_bb(g).LL.x + 2 * margin)/(ssize)));
    H = ((int)ceil((GD_bb(g).UR.y - GD_bb(g).LL.y + 2 * margin)/(ssize)));
    info->perim = W + H;
...
static void shiftEdge(Agedge_t * e, int dx, int dy)
{
    ...
    if (ED_label(e))
	((ED_label(e)->pos).x += dx, (ED_label(e)->pos).y += dy);
    if (ED_xlabel(e))
	((ED_xlabel(e)->pos).x += dx, (ED_xlabel(e)->pos).y += dy);
    if (ED_head_label(e))
	((ED_head_label(e)->pos).x += dx, (ED_head_label(e)->pos).y += dy);
    if (ED_tail_label(e))
	((ED_tail_label(e)->pos).x += dx, (ED_tail_label(e)->pos).y += dy);
	...
	if (bz.sflag)
	    ((ED_spl(e)->list[j].sp).x += dx, (ED_spl(e)->list[j].sp).y += dy);
	if (bz.eflag)
	    ((ED_spl(e)->list[j].ep).x += dx, (ED_spl(e)->list[j].ep).y += dy);
    }
}
...
static void shiftGraph(Agraph_t * g, int dx, int dy)
{
    ...
    if (GD_label(g))
	((GD_label(g)->pos).x += dx, (GD_label(g)->pos).y += dy);
	    ...
	    int doSplines)
{
	...
	for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	    ND_pos(n)[0] += fx;
	    ND_pos(n)[1] += fy;
	    ((ND_coord(n)).x += dx, (ND_coord(n)).y += dy);
	    if (ND_xlabel(n)) {
		((ND_xlabel(n)->pos).x += dx, (ND_xlabel(n)->pos).y += dy);
            }
	    if (doSplines) {
		for (e = agfstout(eg, n); e; e = agnxtout(eg, e))
		    shiftEdge(e, dx, dy);
