...
{ int *sz = arg; agbindrec(node,"level node rec",sz[1],NOT(0)); }
static void my_init_edge(Agraph_t *g, Agobj_t *edge, void *arg)
{ int *sz = arg; agbindrec(edge,"level edge rec",sz[2],NOT(0)); }
static void my_init_graph(Agraph_t *g, Agobj_t *graph, void *arg)
{ int *sz = arg; agbindrec(graph,"level graph rec",sz[0],NOT(0)); }
...
static void compile_samerank(graph_t * ug, graph_t * parent_clust)
{
    ...
    case 3:
	GD_has_sourcerank(clust) = NOT(0);	/* fall through */
    case 2:
	leader = union_all(ug);
	GD_minrep(clust) = union_one(leader, GD_minrep(clust));
	break;
    case 5:
	GD_has_sinkrank(clust) = NOT(0);	/* fall through */
    case 4:
	leader = union_all(ug);
...
static void compile_nodes(graph_t * g, graph_t * Xg)
{
	...
	if (find(n) == n)
	    ND_rep(n) = agnode(Xg, agnameof(n), NOT(0));
    }
    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	if (ND_rep(n) == ((Agnode_t*)0))
...
static void weak(graph_t * g, node_t * t, node_t * h, edge_t * orig)
{
    ...
    if (!e) {
	v = agnode(g, 0, NOT(0));
	e = agedge(g, v, t, (char *) 0, NOT(0));
	f = agedge(g, v, h, (char *) 0, NOT(0));
    }
    ED_minlen(e) = MAX(ED_minlen(e), 0);	/* effectively a nop */
    ED_weight(e) += ED_weight(orig) * 1000;
...
static void compile_clusters(graph_t * g, graph_t * Xg)
{
	...
	for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
		...
		if (!top) {
		    ...
		    top = agnode(Xg, sname, NOT(0));
		    agedge(Xg, top, rep, (char *) 0, NOT(0));
		    agfree(g, sname);
		}
	    }
		...
		if (!bot) {
		    ...
		    bot = agnode(Xg, sname, NOT(0));
		    agedge(Xg, rep, bot, (char *) 0, NOT(0));
		    agfree(g, sname);
		}
	    }
	}
	if (top && bot) {
	    e = agedge(Xg, top, bot, (char *) 0, NOT(0));
	    merge(e, 0, 1000);
	}
    }
...
static void reverse_edge(graph_t * g, edge_t * e)
{
    ...
    rev = (agedge(g,aghead(e),agtail(e),NULL,0));
    if (!rev)
	rev = agedge(g, aghead(e), agtail(e), (char *) 0, NOT(0));
...
static void connect_components(graph_t * g)
{
	...
	if (ND_mark(n) == 0)
	    dfscc(g, n, cc++);
    if (cc > 1) {
	node_t *root = agnode(g, "\177root", NOT(0));
	int ncc = 1;
	for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	    if (ND_mark(n) == ncc) {
		(void) agedge(g, root, n, (char *) 0, NOT(0));
		ncc++;
	    }
	}
    }
