...
static Agraph_t *clone_graph(Agraph_t * ing, Agraph_t ** xg)
{
    ...
    agbindrec(clone, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	//node custom data
    sprintf(gname, "_clone_%d", id++);
    xclone = agopen(gname, ing->desc,((Agdisc_t *)0));
    for (n = agfstnode(ing); n; n = agnxtnode(ing, n)) {
	agsubnode(clone,n,1);
	xn = agnode(xclone, agnameof(n),1);
        agbindrec(xn, "Agnodeinfo_t", sizeof(Agnodeinfo_t), NOT(0));	//node custom data
	(((cdata*)(ND_alg(n)))->u.clone) = xn;
    }

    for (n = agfstnode(ing); n; n = agnxtnode(ing, n)) {
	xn = (((cdata*)(ND_alg(n)))->u.clone);
	for (e = agfstout(ing, n); e; e = agnxtout(ing, e)) {
	    agsubedge(clone,e,1);
	    xh = (((cdata*)(ND_alg(aghead(e))))->u.clone);
	    xe = agedge(xclone, xn, xh, NULL, 1);
	    agbindrec(xe, "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));	//node custom data
	    (ED_to_orig(xe)) = e;
	    (ND_order(xn)) += 1;
	    (ND_order(xh)) += 1;
	}
...
static void find_pair_edges(Agraph_t * g, Agnode_t * n, Agraph_t * outg)
{
	...
	for (ep = agfstedge(g, n); ep; ep = agnxtedge(g, ep, n)) {
		...
		if (n1 < n2) {	/* count edge only once */
		    edge_cnt++;
		    if ((ED_to_orig(ex))) {
			agdelete(outg, (ED_to_orig(ex)));
			(ED_to_orig(ex)) = 0;	/* delete only once */
		    }
		}
	    }
	}
	...
	if (diff < no_pair_count) {
		...
		agbindrec(agedge(g, tp, hp, NULL, 1), "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));   // edge custom data
		(ND_order(tp))++;
		(ND_order(hp))++;
		diff--;
	    }
		...
		hp = neighbors_without[mark];
		agbindrec(agedge(g, tp, hp, NULL, 1), "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));	//node custom data
		(ND_order(tp))++;
		(ND_order(hp))++;
	    }
	}
		 ...
		 Agnode_t * change)
{
    ...
    parent = (((cdata*)(ND_alg(ancestor)))->u.t.tparent);
    if (parent == NULL)
	return;
    ...
    if ((((cdata*)(ND_alg(parent)))->u.t.fdist) == 0) {
	(((cdata*)(ND_alg(parent)))->u.t.first) = n;
	(((cdata*)(ND_alg(parent)))->u.t.fdist) = dist;
    } else if (dist > (((cdata*)(ND_alg(parent)))->u.t.fdist)) {
	if ((((cdata*)(ND_alg(parent)))->u.t.first) != change) {
	    if (!(((cdata*)(ND_alg(parent)))->u.t.sdist) || ((((cdata*)(ND_alg(parent)))->u.t.second) != change))
		change = (((cdata*)(ND_alg(parent)))->u.t.first);
	    (((cdata*)(ND_alg(parent)))->u.t.second) = (((cdata*)(ND_alg(parent)))->u.t.first);
	    (((cdata*)(ND_alg(parent)))->u.t.sdist) = (((cdata*)(ND_alg(parent)))->u.t.fdist);
	}
	(((cdata*)(ND_alg(parent)))->u.t.first) = n;
	(((cdata*)(ND_alg(parent)))->u.t.fdist) = dist;
    } else if (dist > (((cdata*)(ND_alg(parent)))->u.t.sdist)) {
	(((cdata*)(ND_alg(parent)))->u.t.second) = n;
	(((cdata*)(ND_alg(parent)))->u.t.sdist) = dist;
	return;
    } else
	return;
...
static nodelist_t *find_longest_path(Agraph_t * tree)
{
    ...
    for (n = agfstnode(tree); n; n = agnxtnode(tree, n)) {
	length = (((cdata*)(ND_alg(n)))->u.t.fdist) + (((cdata*)(ND_alg(n)))->u.t.sdist);
	if (length > maxlength) {
	    common = n;
	    maxlength = length;
	}
    ...
    path = mkNodelist();
    for (n = (((cdata*)(ND_alg(common)))->u.t.first); n != common; n = (((cdata*)(ND_alg(n)))->u.t.tparent)) {
	appendNodelist(path, NULL, n);
	((((cdata*)(ND_alg(n)))->flags) |= (1 << 4));
    }
    appendNodelist(path, NULL, common);
    ((((cdata*)(ND_alg(common)))->flags) |= (1 << 4));

    if ((((cdata*)(ND_alg(common)))->u.t.sdist)) {	/* 2nd path might be empty */
	endPath = mkNodelist();
	for (n = (((cdata*)(ND_alg(common)))->u.t.second); n != common; n = (((cdata*)(ND_alg(n)))->u.t.tparent)) {
...
static void dfs(Agraph_t * g, Agnode_t * n, Agraph_t * tree)
{
    ...
    ((((cdata*)(ND_alg(n)))->flags) |= (1 << 0));
    for (e = agfstedge(g, n); e; e = agnxtedge(g, e, n)) {
	neighbor = aghead(e);
	if (neighbor == n)
	    neighbor = agtail(e);

	if (!((((cdata*)(ND_alg(neighbor)))->flags)&(1 << 0))) {
	    /* add the edge to the dfs tree */
	    agsubedge(tree,e,1);
	    (((cdata*)(ND_alg(neighbor)))->u.t.tparent) = n;
	    dfs(g, neighbor, tree);
	}
    }
}
...
static Agraph_t *spanning_tree(Agraph_t * g)
{
    ...
    agbindrec(tree, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	//node custom data

    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	agsubnode(tree,n,1);
	(((cdata*)(ND_alg(n)))->u.t.fdist) = 0;
	(((cdata*)(ND_alg(n)))->u.t.sdist) = 0;
	((((cdata*)(ND_alg(n)))->flags) &= ~(1 << 0));
    }

    for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	if (!((((cdata*)(ND_alg(n)))->flags)&(1 << 0))) {
	    (((cdata*)(ND_alg(n)))->u.t.tparent) = NULL;
	    dfs(g, n, tree);
	}
    }
...
static void block_graph(Agraph_t * g, block_t * sn)
{
	...
	for (e = agfstout(g, n); e; e = agnxtout(g, e)) {
	    if ((((cdata*)(ND_alg(aghead(e))))->block) == sn)
		agsubedge(subg,e,1);
	}
    }
}
...
static int count_all_crossings(nodelist_t * list, Agraph_t * subg)
{
	...
	for (e = agfstedge(subg, n); e; e = agnxtedge(subg, e, n)) {
		...
		for (eitem = (edgelistitem *) (*(((Dt_t*)(openEdgeList))->searchf))((openEdgeList),(void*)(0),0000200); eitem;
		    ...
		    ep = eitem->edge;
		    if ((((edata*)(ED_alg(ep)))->order) > (((edata*)(ED_alg(e)))->order)) {
...
static double largest_nodesize(nodelist_t * list)
{
    ...
    for (item = list->first; item; item = item->next) {
	n = (((cdata*)(ND_alg(item->curr)))->orig.np);
	if (ND_width(n) > size)
	    size = ND_width(n);
	if (ND_height(n) > size)
	    size = ND_height(n);
...
static void place_node(Agraph_t * g, Agnode_t * n, nodelist_t * list)
{
	...
	((((cdata*)(ND_alg(aghead(e))))->flags) |= (1 << 5));
    }
    for (e = agfstin(g, n); e; e = agnxtin(g, e)) {
	appendNodelist(neighbors, NULL, agtail(e));
	((((cdata*)(ND_alg(agtail(e))))->flags) |= (1 << 5));
    }
	...
	for (one = list->first; one; one = one->next) {
	    ...
	    if (((((cdata*)(ND_alg(one->curr)))->flags)&(1 << 5)) && ((((cdata*)(ND_alg(two->curr)))->flags)&(1 << 5))) {
	...
	for (one = list->first; one; one = one->next) {
	    if (((((cdata*)(ND_alg(one->curr)))->flags)&(1 << 5))) {
		appendNodelist(list, one, n);
		placed = 1;
		break;
	    }
	...
	((((cdata*)(ND_alg(one->curr)))->flags) &= ~(1 << 5));
...
nodelist_t *layout_block(Agraph_t * g, block_t * sn, double min_dist)
{
    ...
    for (item = longest_path->first; item; item = item->next) {
	n = item->curr;
	(((cdata*)(ND_alg(n)))->u.f.pos) = k;
	(((cdata*)(ND_alg(n)))->u.f.psi) = 0.0;
	theta = k * ((2.0 * M_PI) / N);
