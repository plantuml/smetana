...
static Agraph_t *makeBlockGraph(Agraph_t * g, circ_state * state)
{
    ...
    agbindrec(subg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));	//node custom data
    return subg;
}
...
static void dfs(Agraph_t * g, Agnode_t * u, circ_state * state, int isRoot, estack* stk)
{
    ...
    (((cdata*)(ND_alg(u)))->u.bc.low_val) = (((cdata*)(ND_alg(u)))->u.bc.val) = state->orderCount++;
    for (e = agfstedge(g, u); e; e = agnxtedge(g, e, u)) {
	v = aghead (e);
	if (v == u) {
            v = agtail(e);
	    if (!(((edata*)(ED_alg(e)))->order)) (((edata*)(ED_alg(e)))->order) = -1;
	}
	else {
	    if (!(((edata*)(ED_alg(e)))->order)) (((edata*)(ED_alg(e)))->order) = 1;
	}

        if ((((cdata*)(ND_alg(v)))->u.bc.val) == 0) {   /* Since VAL(root) == 0, it gets treated as artificial cut point */
	    (((cdata*)(ND_alg(v)))->parent) = u;
            push(stk, e);
            dfs(g, v, state, 0, stk);
            (((cdata*)(ND_alg(u)))->u.bc.low_val) = MIN((((cdata*)(ND_alg(u)))->u.bc.low_val), (((cdata*)(ND_alg(v)))->u.bc.low_val));
            if ((((cdata*)(ND_alg(v)))->u.bc.low_val) >= (((cdata*)(ND_alg(u)))->u.bc.val)) {       /* u is an articulation point */
		block_t *block = NULL;
		Agnode_t *np;
		Agedge_t *ep;
                do {
                    ep = pop(stk);
		    if ((((edata*)(ED_alg(ep)))->order) == 1)
			np = aghead (ep);
		    else
			np = agtail (ep);
		    if (!(((cdata*)(ND_alg(np)))->block)) {
			if (!block)
		...
		if (block) {	/* If block != NULL, it's not empty */
		    if (!(((cdata*)(ND_alg(u)))->block) && blockSize (block) > 1)
			addNode(block, u);
		    if (isRoot && ((((cdata*)(ND_alg(u)))->block) == block))
			insertBlock(&state->bl, block);
		    else
			appendBlock(&state->bl, block);
		}
            }
        } else if ((((cdata*)(ND_alg(u)))->parent) != v) {
            (((cdata*)(ND_alg(u)))->u.bc.low_val) = MIN((((cdata*)(ND_alg(u)))->u.bc.low_val), (((cdata*)(ND_alg(v)))->u.bc.val));
        }
    }
    if (isRoot && !(((cdata*)(ND_alg(u)))->block)) {
...
static void find_blocks(Agraph_t * g, circ_state * state)
{
	...
	for (n = agfstnode(g); n; n = agnxtnode(g, n)) {
	    if (late_bool((((cdata*)(ND_alg(n)))->orig.np), state->N_root, 0)) {
		root = n;
		break;
	    }
	}
...
block_t *createBlocktree(Agraph_t * g, circ_state * state)
{
	...
	min = (((cdata*)(ND_alg(n)))->u.bc.val);
	parent = (((cdata*)(ND_alg(n)))->parent);
	for (n = agnxtnode(subg, n); n; n = agnxtnode(subg, n)) {
	    if ((((cdata*)(ND_alg(n)))->u.bc.val) < min) {
		child = n;
		min = (((cdata*)(ND_alg(n)))->u.bc.val);
		parent = (((cdata*)(ND_alg(n)))->parent);
	    }
	}
	((((cdata*)(ND_alg(parent)))->flags) |= (1 << 3));
	((bp)->child) = child;
	next = bp->next;	/* save next since list insertion destroys it */
	appendBlock(&((((cdata*)(ND_alg(parent)))->block)->children), bp);
    }
