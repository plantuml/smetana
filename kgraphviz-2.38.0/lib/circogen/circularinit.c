...
{
    agbindrec(e, "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));	//node custom data
    common_init_edge(e);
...
static void circular_init_node_edge(graph_t * g)
{
    ...
    GD_neato_nlist(g) = (node_t **)zmalloc((agnnodes(g) + 1)*sizeof(node_t *));
			       ...
			       void *orig)
{
    ...
    agbindrec(n, "Agnodeinfo_t", sizeof(Agnodeinfo_t), NOT(0));	//node custom data
    ND_alg(n) = (void *) (cdata*)zmalloc(sizeof(cdata));
    if (isNode) {
	ND_pos(n) = (double*)zmalloc((Ndim)*sizeof(double));
	ND_lw(n) = ND_lw((node_t *) orig);
	ND_rw(n) = ND_rw((node_t *) orig);
	ND_ht(n) = ND_ht((node_t *) orig);
	(((cdata*)(ND_alg(n)))->orig.np) = (node_t *) orig;
    } else
	(((cdata*)(ND_alg(n)))->orig.g) = (graph_t *) orig;
    return n;
}
...
Agraph_t **circomps(Agraph_t * g, int *cnt)
{
    ...
    agbindrec (dg, "info", sizeof(Agraphinfo_t), NOT(0));
    GD_alg(g) = dg;  /* store derived graph for closing later */

    for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
	if ((((ndata*)(ND_alg(v)))->dnode))
	    continue;
	n = makeDerivedNode(dg, agnameof(v), 1, v);
	(((ndata*)(ND_alg(v)))->dnode) = n;
    }

    for (v = agfstnode(g); v; v = agnxtnode(g, v)) {
	for (e = agfstout(g, v); e; e = agnxtout(g, e)) {
	    dt = (((ndata*)(ND_alg(agtail(e))))->dnode);
	    dh = (((ndata*)(ND_alg(aghead(e))))->dnode);
	    if (dt != dh) {
		agbindrec(agedge(dg, dt, dh, NULL, 1), "Agedgeinfo_t", sizeof(Agedgeinfo_t), NOT(0));	//node custom data
	    }
	}
    }
	...
	for (e = agfstout(dg, n); e; e = agnxtout(dg, e)) {
	    ED_alg(e) = (edata*)zmalloc(sizeof(edata));
	}
    }
