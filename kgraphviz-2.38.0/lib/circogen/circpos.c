#line 1 "graphviz-2.38.0\\lib\\circogen\\circpos.c" 1
/* $Id$ $Revision$ */
...
#line 24 "graphviz-2.38.0\\lib\\circogen\\circpos.c" 2
...
getRotation(block_t * sn, Agraph_t * g, double x, double y, double theta)
{
    ...
    if (sn->parent_pos >= 0) {
	theta += 3.14159265358979323846 - sn->parent_pos;
	if (theta < 0)
    ...
    if (count == 2) {
	return (theta - 3.14159265358979323846 / 2.0);
    }
    ...
    newX = (((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->pos)[0] + x;
    newY = (((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->pos)[1] + y;
    mindist2 = (((newX) * (newX)) + ((newY) * (newY)));    /* save sqrts by using sqr of dist to find min */
	...
	newX = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[0] + x;
	newY = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[1] + y;
	...
	double n_x = (((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->pos)[0];
	if ((((sn)->flags)&(1 << 0)) && (-r < n_x)) {
	    double R = (sqrt(((((x)) * ((x))) + (((y)) * ((y))))));
	    double n_y = (((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->pos)[1];
	    double phi = atan2(n_y, n_x + r);
	    double l = r - rho / (cos(phi));

	    theta += 3.14159265358979323846 / 2.0 - phi - asin((l / R) * (cos(phi)));
	} else {		/* Origin still at center of this block */
	    double phi = atan2((((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->pos)[1], (((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->pos)[0]);
	    theta += 3.14159265358979323846 - phi - (((cdata*)((((Agnodeinfo_t*)(((Agobj_t*)(neighbor))->data))->alg)))->u.f.psi);
	    if (theta > 2 * 3.14159265358979323846)
		theta -= 2 * 3.14159265358979323846;
	}
    } else
	theta = 0;
...
static void applyDelta(block_t * sn, double x, double y, double rotate)
{
	...
	if (rotate != 0) {
	    ...
	    tmpX = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[0];
	    tmpY = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[1];
	    cosR = cos(rotate);
	    sinR = sin(rotate);
	    ...
	    X = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[0];
	    Y = (((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[1];
	}

	/* translate */
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[0] = X + x;
	(((Agnodeinfo_t*)(((Agobj_t*)(n))->data))->pos)[1] = Y + y;
    }
...
getInfo (posinfo_t* pi, posstate * stp, double min_dist)
{
	...
	if ((((child)->child)? (((cdata*)((((Agnodeinfo_t*)(((Agobj_t*)(((child)->child)))->data))->alg)))->parent) : 0) == pi->n) {
	    childCount++;
...
positionChildren (Agraph_t* g, posinfo_t* pi, posstate * stp, int length, double min_dist)
{
    ...
    if (length == 1) {
	childAngle = 0;
	d = pi->diameter/(2*3.14159265358979323846);
	childRadius = ((childRadius)>(d)?(childRadius):(d));
	d = 2*3.14159265358979323846*childRadius - pi->diameter;
	if (d > 0)
	...
	if (length == 1) {
	    if (childAngle != 0) {
		if (pi->childCount == 2)
		    childAngle = 3.14159265358979323846;
		else
	...
	(((cdata*)((((Agnodeinfo_t*)(((Agobj_t*)(pi->n))->data))->alg)))->u.f.psi) = midAngle;
    }
	 ...
	 block_t * sn, double min_dist)
{
    ...
    double angle;
    double theta = 0.0;
    posinfo_t* parents = (posinfo_t*)zmalloc((childCount)*sizeof(posinfo_t));
    int num_parents = 0;
    posinfo_t* next;
    posinfo_t* curr;
    double delta;
    ...
    state.nodeAngle = 2 * 3.14159265358979323846 / length;
    state.firstAngle = -1;
    state.lastAngle = -1;
	...
	delta = next->theta - curr->theta;
	    ...
	    delta = 2*3.14159265358979323846 - delta;
	...
	for (i = 0; i < num_parents; i++) {
	    if (i+1 == num_parents) {
		next = parents;
		delta = next->theta - curr->theta + 2*3.14159265358979323846; 
	    }
	    else {
		next = curr+1;
		delta = next->theta - curr->theta; 
    ...
    angle = (state.firstAngle + state.lastAngle) / 2.0 - 3.14159265358979323846;
    return angle;
}
...
static void doBlock(Agraph_t * g, block_t * sn, double min_dist)
{
    ...
    double centerAngle = 3.14159265358979323846;
    ...
    if ((length == 1) && ((((sn)->child)? (((cdata*)((((Agnodeinfo_t*)(((Agobj_t*)(((sn)->child)))->data))->alg)))->parent) : 0))) {
	sn->parent_pos = centerAngle;
	if (sn->parent_pos < 0)
	    sn->parent_pos += 2 * 3.14159265358979323846;
    }
}
