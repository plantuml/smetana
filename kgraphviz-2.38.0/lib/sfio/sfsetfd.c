#line 1 "graphviz-2.38.0\\lib\\sfio\\sfsetfd.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfsetfd.c" 2
...
static int _sfdup(register int fd, register int newfd)
...

{
    ...
    { while(close(dupfd) < 0 && errno == 4) errno = 0; };
...
int sfsetfd(register Sfio_t * f, register int newfd)
...

{
    ...
    if ((f->mode & 0000004) && f->file < 0) {	/* restoring file descriptor after a previous freeze */
	if (newfd < 0)
	    { return(-1); };
	...
	if (oldfd >= 0) {
		...
		{ while(close(oldfd) < 0 && errno == 4) errno = 0; };
		...
		f->extent = f->here = 0;
		f->mode = (f->mode & (0000001|0000002)) | 0000004;
		f->bits &= ~00000010;	/* off /dev/null handling */
	    }
	}
