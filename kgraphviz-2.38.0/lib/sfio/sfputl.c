#line 1 "graphviz-2.38.0\\lib\\sfio\\sfputl.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfputl.c" 2
...
int _sfputl(register Sfio_t * f, _ast_intmax_t v)
...

{
    ...
    register unsigned char *s, *ps;
    register ssize_t n, p;
    unsigned char c[(2*sizeof(_ast_intmax_t))];
    ...
    s = ps = &(c[(2*sizeof(_ast_intmax_t)) - 1]);
    if (v < 0) {		/* add 1 to avoid 2-complement problems with -SF_MAXINT */
	v = -(v + 1);
	*s = (unsigned char) (((( long)(v))&((1 << 6)-1)) | (1 << 6));
    } else
	*s = (unsigned char) (((( long)(v))&((1 << 6)-1)));
    v = (unsigned _ast_intmax_t) v >> 6;

    while (v > 0) {
	*--s = (unsigned char) ((((unsigned long)(v))&((1 << 7)-1)) | (1 << 7));
	v = (unsigned _ast_intmax_t) v >> 7;
    }
    n = (ps - s) + 1;

    if (n > 8 || (((p) = (f)->endb - ((ps) = (f)->next) ) > 0 ? (p) : ((p) = (((f)->mode |= 00100000),_sfflsbuf(f,-1)), (ps) = (f)->next, (p)) ) < n)
	n = (((f)->mode |= 00100000),sfwrite(f,(Void_t *) s,n));	/* write the hard way */
    else {
	switch (n) {
	case 8:
	    *ps++ = *s++;
