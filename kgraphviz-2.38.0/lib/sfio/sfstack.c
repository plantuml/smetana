#line 1 "graphviz-2.38.0\\lib\\sfio\\sfstack.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfstack.c" 2
...

{
    register int n;
    register Sfio_t *rf;
    register Sfrsrv_t *rsrv;
    register Vtmutex_t *mtx;

    { if(f1) (void)(0); if(f2) (void)(0); };

    if (f1 && (f1->mode & (0000001|0000002)) != f1->mode && _sfmode(f1, 0, 0) < 0)
	{ if(f1) (void)(0); if(f2) (void)(0); return(((Sfio_t *)0)); };
    if (f2 && (f2->mode & (0000001|0000002)) != f2->mode && _sfmode(f2, 0, 0) < 0)
	{ if(f1) (void)(0); if(f2) (void)(0); return(((Sfio_t *)0)); };
    if (!f1)
	{ if(f1) (void)(0); if(f2) (void)(0); return(f2); };

    /* give access to other internal functions */
    (_Sfextern.sf_stack) = sfstack;
	...
	if (f2->push)
	    { if(f1) (void)(0); if(f2) (void)(0); return(((Sfio_t *)0)); };
	if (f1->pool && f1->pool != &(_Sfextern.sf_pool) && f1->pool != f2->pool && f1 == f1->pool->sf[0]) {	/* get something else to pool front since f1 will be locked */
	    for (n = 1; n < f1->pool->n_sf; ++n) {
		if (((f1->pool->sf[n])->mode&(00000100|00000040|00000400) ? 1 : ((f1->pool->sf[n])->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f1->pool->sf[n]) : 0))
		    continue;
		(*(_Sfextern.sf_pmove)) (f1->pool->sf[n], 0);
		break;
	    }
	}
    }

    if (f2->pool && f2->pool != &(_Sfextern.sf_pool) && f2 != f2->pool->sf[0])
	(*(_Sfextern.sf_pmove)) (f2, 0);
    ...
    { if(f1) (void)(0); if(f2) (void)(0); return(rf); };
}
