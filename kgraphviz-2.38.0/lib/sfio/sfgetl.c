#line 1 "graphviz-2.38.0\\lib\\sfio\\sfgetl.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfgetl.c" 2
...
_ast_intmax_t sfgetl(register Sfio_t * f)
...

{
	...
	for (ends = s + p; s < ends;) {
	    c = *s++;
	    if (c & (1 << 7))
		v = ((unsigned _ast_intmax_t) v << 7) | (((unsigned long)(c))&((1 << 7)-1));
	    else {		/* special translation for this byte */
		v = ((unsigned _ast_intmax_t) v << 6) | ((( long)(c))&((1 << 6)-1));
		f->next = s;
		v = (c & (1 << 6)) ? -v - 1 : v;
		goto done;
	    }
