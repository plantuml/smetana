#line 1 "graphviz-2.38.0\\lib\\sfio\\sfnew.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfnew.c" 2
...

{
	...
	if (flags & 0000200) {
		...
		f->mutex = ((Vtmutex_t *)0);
	    ( (f)->next = (unsigned char*)0, (f)->endw = (unsigned char*)0, (f)->endr = (unsigned char*)0, (f)->endb = (unsigned char*)0, (f)->push = (Sfio_t*)0, (f)->flags = (unsigned short)0, (f)->file = -1, (f)->data = (unsigned char*)0, (f)->size = (ssize_t)(-1), (f)->val = (ssize_t)(-1), (f)->extent = (_ast_intmax_t)(-1), (f)->here = (_ast_intmax_t)0, (f)->getr = 0, (f)->tiny[0] = 0, (f)->bits = 0, (f)->mode = 0, (f)->disc = (struct _sfdisc_s*)0, (f)->pool = (struct _sfpool_s*)0, (f)->rsrv = (struct _sfrsrv_s*)0, (f)->proc = (struct _sfproc_s*)0, (f)->mutex = (f->mutex), (f)->stdio = (Void_t*)0, (f)->lpos = (_ast_intmax_t)0, (f)->iosz = (size_t)0 );
	    oldf = ((Sfio_t *)0);
	} else if (f->mode & 00020000) {	/* only allow SF_STATIC to be already closed */
	    if (!(f->flags & 0001000))
		return ((Sfio_t *)0);
	    sflags = f->flags;
	    oldf = ((Sfio_t *)0);
		...
		(((f)->mode |= 00100000),sfclose(f)) < 0)
		return ((Sfio_t *)0);

	    if (f->data
		&& ((flags & 0000004) || size != (size_t) (-1))) {
		if (sflags & 0000020)
		    free((Void_t *) f->data);
	...
	if (!(flags & 0000004) && file >= 0 && file <= 2) {
	...
	if (!f) {
		...
		return ((Sfio_t *)0);
	    ( (f)->next = (unsigned char*)0, (f)->endw = (unsigned char*)0, (f)->endr = (unsigned char*)0, (f)->endb = (unsigned char*)0, (f)->push = (Sfio_t*)0, (f)->flags = (unsigned short)0, (f)->file = -1, (f)->data = (unsigned char*)0, (f)->size = (ssize_t)(-1), (f)->val = (ssize_t)(-1), (f)->extent = (_ast_intmax_t)(-1), (f)->here = (_ast_intmax_t)0, (f)->getr = 0, (f)->tiny[0] = 0, (f)->bits = 0, (f)->mode = 0, (f)->disc = (struct _sfdisc_s*)0, (f)->pool = (struct _sfpool_s*)0, (f)->rsrv = (struct _sfrsrv_s*)0, (f)->proc = (struct _sfproc_s*)0, (f)->mutex = (((Vtmutex_t *)0)), (f)->stdio = (Void_t*)0, (f)->lpos = (_ast_intmax_t)0, (f)->iosz = (size_t)0 );
	}
    }
    ...
    /* stream type */
    f->mode = (flags & 0000001) ? 0000001 : 0000002;
    f->flags = (flags & 0077177) | (sflags & (0000020 | 0001000));
    f->bits = (flags & (0000001|0000002)) == (0000001|0000002) ? 00000002 : 0;
    f->file = file;
    f->here = f->extent = 0;
    f->getr = f->tiny[0] = 0;
    ...
    if (size != (size_t) (-1)) {
	f->size = size;
	f->data = size <= 0 ? ((unsigned char *)0) : (unsigned char *) buf;
    }
    f->endb = f->endr = f->endw = f->next = f->data;

    if ((_Sfextern.sf_notify))
	(*(_Sfextern.sf_notify)) (f, 0, f->file);

    if (f->flags & 0000004)
	(void) _sfmode(f, f->mode & (0000001|0000002), 0);
