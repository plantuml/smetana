#line 1 "graphviz-2.38.0\\lib\\sfio\\sfrd.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfrd.c" 2
...

{
    ...
    /* sync all pool heads */
    for (p = (_Sfextern.sf_pool).next; p; p = p->next) {
	if (p->n_sf <= 0)
	    continue;
	f = p->sf[0];
	if (!((f)->mode&(00000100|00000040|00000400) ? 1 : ((f)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f) : 0) && f->next > f->data &&
	    (f->mode & 0000002) && f->extent < 0)
	    (void) _sfflsbuf(f, -1);
    }

    /* and all the ones in the discrete pool */
    for (n = 0; n < (_Sfextern.sf_pool).n_sf; ++n) {
	f = (_Sfextern.sf_pool).sf[n];
...
ssize_t sfrd(register Sfio_t * f, register Void_t * buf, register size_t n,
	     Sfdisc_t * disc)
...

{
    ...
    register int local, rcrv, dosync, oerrno;

    { if(!f) return(-1); };

    ((local) = ((f)->mode&00100000), (f)->mode &= ~00100000, (local));
    if ((rcrv = f->mode & (00000010 | 00000020)))
	f->mode &= ~(00000010 | 00000020);
    f->bits &= ~00000040;
	...
	{ Sfdisc_t* d; if(!(dc)) d = (dc) = (f)->disc; else d = (f->bits&00001000) ? ((dc) = (dc)->disc) : (dc); while(d && !(d->readf)) d = d->disc; if(d) (dc) = d; };
	if (dc && dc->exceptf && (f->flags & 0002000)) {
	    register int rv;
	    if (local)
		((f)->mode |= 00100000);
	    if ((rv = _sfexcept(f, 0000001, n, dc)) > 0)
		n = rv;
	    else if (rv < 0) {
		f->flags |= 0000400;
		{ return((ssize_t) rv); };
	...
	if (dc && dc->readf) {
	    ...
	    { int dcdown = f->bits&00001000; f->bits |= 00001000; r = (*dc->readf)(f,buf,n,dc); if(!dcdown) f->bits &= ~00001000; };
	    ...
	    r = 0;
	else if (f->extent < 0 && (f->flags & 0000100) && rcrv) {	/* try peek read */
	    r = sfpkrd(f->file, (char *) buf, n,
		       (rcrv & 00000010) ? (int) f->getr : -1,
		       -1L, (rcrv & 00000020) ? 1 : 0);
	    if (r > 0) {
		if (rcrv & 00000020)
		    f->mode |= 00001000;
		else
	...
	if (r > 0) {
	    if (!(f->bits & 00001000)) {	/* not a continuation call */
		if (!(f->mode & 00001000)) {
		    f->here += r;
		    if (f->extent >= 0 && f->extent < f->here)
			f->extent = f->here;
		}
		if ((unsigned char *) buf >= f->data &&
		    (unsigned char *) buf < f->data + f->size)
		    f->endb = f->endr = ((unsigned char *) buf) + r;
	    }
	...
	if (local)
	    ((f)->mode |= 00100000);
	switch (_sfexcept(f, 0000001, (ssize_t) r, dc)) {
	case 3:
	    goto do_continue;
	case 0:
	    n = local ? 0 : (ssize_t) r;
