#line 1 "graphviz-2.38.0\\lib\\sfio\\sfungetc.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfungetc.c" 2
...
static int _uexcept(register Sfio_t * f, register int type, Void_t * val,
...
int sfungetc(register Sfio_t * f, register int c)
...

{
	...
	if (c < 0 || (f->mode != 0000001 && _sfmode(f, 0000001, 0) < 0))
	{ return(-1); };
    (void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);

    /* fast handling of the typical unget */
    if (f->next > f->data && f->next[-1] == (unsigned char) c) {
	f->next -= 1;
	goto done;
    }
	...
	if (!(uf = sfnew(((Sfio_t *)0), ((char *)0), (size_t) (-1),
			 -1, 0000004 | 0000001))) {
	    c = -1;
	    goto done;
	}
	(&(_Sfextern.sf_udisc))->exceptf = _uexcept;
	sfdisc(uf, (&(_Sfextern.sf_udisc)));
	...
	if (f->size < 0)
	    f->size = 0;
	if (!(data = (unsigned char *) malloc(f->size + 16))) {
	    c = -1;
	    goto done;
	}
	f->flags |= 0000020;
