#line 1 "graphviz-2.38.0\\lib\\sfio\\sfswap.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfswap.c" 2
...
Sfio_t *sfswap(register Sfio_t * f1, register Sfio_t * f2)
...

{
    ...
    if (!f1 || (f1->mode & 00020000)
	|| (((f1)->mode&(00000100|00000040|00000400) ? 1 : ((f1)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f1) : 0) && (f1->mode & 00000100)))
	return ((Sfio_t *)0);
    if (f2 && ((f2)->mode&(00000100|00000040|00000400) ? 1 : ((f2)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f2) : 0) && (f2->mode & 00000100))
	return ((Sfio_t *)0);
    if (f1 == f2)
	return f2;

    f1mode = f1->mode;
    (void)((f1)->mode |= 00000040, (f1)->endr = (f1)->endw = (f1)->data);
    f1->mode |= 00000100;	/* make sure there is no recursion on f1 */

    if (f2) {
	f2mode = f2->mode;
	(void)((f2)->mode |= 00000040, (f2)->endr = (f2)->endw = (f2)->data);
	f2->mode |= 00000100;	/* make sure there is no recursion on f2 */
    } else {
	f2 = f1->file == 0 ? sfstdin :
	    f1->file == 1 ? sfstdout :
	    f1->file == 2 ? sfstderr : ((Sfio_t *)0);
	if ((!f2 || !(f2->mode & 00020000))) {
	    if (!(f2 = (Sfio_t *) malloc(sizeof(Sfio_t)))) {
		f1->mode = f1mode;
		(void)((0) ? 0 : ((f1)->mode &= ~(00000040|00000010|00000020), ((f1)->mode == 0000001 ? ((f1)->endr = ((f1)->flags&0) ? (f1)->data : (f1)->endb) : (f1)->mode == 0000002 ? ((f1)->endw = ((f1)->flags&(0|0000040)) ? (f1)->data : (f1)->endb) : ((f1)->endw = (f1)->endr = (f1)->data) ), 0) );
		return ((Sfio_t *)0);
	    }

	    ( (f2)->next = (unsigned char*)0, (f2)->endw = (unsigned char*)0, (f2)->endr = (unsigned char*)0, (f2)->endb = (unsigned char*)0, (f2)->push = (Sfio_t*)0, (f2)->flags = (unsigned short)0, (f2)->file = -1, (f2)->data = (unsigned char*)0, (f2)->size = (ssize_t)(-1), (f2)->val = (ssize_t)(-1), (f2)->extent = (_ast_intmax_t)(-1), (f2)->here = (_ast_intmax_t)0, (f2)->getr = 0, (f2)->tiny[0] = 0, (f2)->bits = 0, (f2)->mode = 0, (f2)->disc = (struct _sfdisc_s*)0, (f2)->pool = (struct _sfpool_s*)0, (f2)->rsrv = (struct _sfrsrv_s*)0, (f2)->proc = (struct _sfproc_s*)0, (f2)->mutex = (((Vtmutex_t *)0)), (f2)->stdio = (Void_t*)0, (f2)->lpos = (_ast_intmax_t)0, (f2)->iosz = (size_t)0 );
	}
	f2->mode = 00020000 | 00000040;
	f2mode = 00020000;
    }
    ...
    if (f2mode & 00020000) {	/* swapping to a closed stream */
	if (!(f1->flags & 0001000))
	    free(f1);
    } else {
	f1->mode = f2mode;
    ...
    f2->mode = f1mode;
    (void)((0) ? 0 : ((f2)->mode &= ~(00000040|00000010|00000020), ((f2)->mode == 0000001 ? ((f2)->endr = ((f2)->flags&0) ? (f2)->data : (f2)->endb) : (f2)->mode == 0000002 ? ((f2)->endw = ((f2)->flags&(0|0000040)) ? (f2)->data : (f2)->endb) : ((f2)->endw = (f2)->endr = (f2)->data) ), 0) );
    return f2;
}
