#line 1 "graphviz-2.38.0\\lib\\sfio\\sfgetm.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfgetm.c" 2
...
unsigned _ast_intmax_t sfgetm(register Sfio_t * f, unsigned _ast_intmax_t m)
...
{
    unsigned _ast_intmax_t v;
    register unsigned char *s, *ends, c;
    register int p;

    { if(!f) return((unsigned _ast_intmax_t)(-1)); };

    if (f->mode != 0000001 && _sfmode(f, 0000001, 0) < 0)
	{ return((unsigned _ast_intmax_t)(-1)); };

    (void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);

    for (v = 0;;) {
	if ((((p) = (f)->endb - ((s) = (f)->next) ) > 0 ? (p) : ((p) = (((f)->mode |= 00100000),_sffilbuf(f,-1)), (s) = (f)->next, (p)) ) <= 0) {
	    f->flags |= 0000400;
	    v = (unsigned _ast_intmax_t) (-1);
	    goto done;
	}
	for (ends = s + p; s < ends;) {
	    c = *s++;
	    v = (v << 8) | (((unsigned long)(c))&((1 << 8)-1));
	    if ((m >>= 8) <= 0) {
		f->next = s;
		goto done;
	    }
