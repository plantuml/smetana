#line 1 "graphviz-2.38.0\\lib\\sfio\\sfpopen.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfpopen.c" 2
...
{
    register char *s, *cmd, **argv, **p, *interp;
    register int n;
  ...
  do_interp:
	...
	if (*s == '/')
	    break;
    execl(interp, s + 1, "-c", argcmd, ((char *)0));
    _exit(127);
}
...

{
    ...
    register int pid, fd, pkeep, ckeep, sflags;
	...
	|| !(sflags = _sftype(mode, ((int *)0), ((int *)0))))
	 return ((Sfio_t *)0);
    ...
    if (pipe(parent) < 0)
	goto error;
    if ((sflags & (0000001|0000002)) == (0000001|0000002) && pipe(child) < 0)
	goto error;
	...
	if (!
	    (f =
	     sfnew(f, ((Void_t *)0), (size_t) (-1), parent[pkeep],
		   sflags)))
	    goto error;
	{ while(close(parent[!pkeep]) < 0 && errno == 4) errno = 0; };
	;

	if ((sflags & (0000001|0000002)) == (0000001|0000002)) {
	    { while(close(child[!ckeep]) < 0 && errno == 4) errno = 0; };
	    ;
	}

	/* save process info */
	fd = (sflags & (0000001|0000002)) == (0000001|0000002) ? child[ckeep] : -1;
	...
	( (&sf)->next = (unsigned char*)0, (&sf)->endw = (unsigned char*)0, (&sf)->endr = (unsigned char*)0, (&sf)->endb = (unsigned char*)0, (&sf)->push = (Sfio_t*)0, (&sf)->flags = (unsigned short)0, (&sf)->file = -1, (&sf)->data = (unsigned char*)0, (&sf)->size = (ssize_t)(-1), (&sf)->val = (ssize_t)(-1), (&sf)->extent = (_ast_intmax_t)(-1), (&sf)->here = (_ast_intmax_t)0, (&sf)->getr = 0, (&sf)->tiny[0] = 0, (&sf)->bits = 0, (&sf)->mode = 0, (&sf)->disc = (struct _sfdisc_s*)0, (&sf)->pool = (struct _sfpool_s*)0, (&sf)->rsrv = (struct _sfrsrv_s*)0, (&sf)->proc = (struct _sfproc_s*)0, (&sf)->mutex = (((Vtmutex_t *)0)), (&sf)->stdio = (Void_t*)0, (&sf)->lpos = (_ast_intmax_t)0, (&sf)->iosz = (size_t)0 );

	/* must be careful so not to close something useful */
	if ((sflags & (0000001|0000002)) == (0000001|0000002) && pkeep == child[ckeep])
	    if ((child[ckeep] = dup(pkeep)) < 0)
		_exit(127);

	if (parent[pkeep] != pkeep) {
	    sf.file = parent[pkeep];
	    { while(close(pkeep) < 0 && errno == 4) errno = 0; };
	    if (sfsetfd(&sf, pkeep) != pkeep)
		_exit(127);
	}

	if ((sflags & (0000001|0000002)) == (0000001|0000002) && child[ckeep] != ckeep) {
	    sf.file = child[ckeep];
	    { while(close(ckeep) < 0 && errno == 4) errno = 0; };
    ...
    case -1:			/* error */
      error:
	if (parent[0] >= 0) {
	    { while(close(parent[0]) < 0 && errno == 4) errno = 0; };
	    { while(close(parent[1]) < 0 && errno == 4) errno = 0; };
	}
	if (child[0] >= 0) {
	    { while(close(child[0]) < 0 && errno == 4) errno = 0; };
	    { while(close(child[1]) < 0 && errno == 4) errno = 0; };
	}
	return ((Sfio_t *)0);
    }
