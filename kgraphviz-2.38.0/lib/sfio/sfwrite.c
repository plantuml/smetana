#line 1 "graphviz-2.38.0\\lib\\sfio\\sfwrite.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfwrite.c" 2
...
ssize_t sfwrite(register Sfio_t * f, const Void_t * buf, register size_t n)
...
{
    register unsigned char *s, *begs, *next;
    register ssize_t w;
    register int local;
    ...
    /* release peek lock */
    if (f->mode & 00000400) {
	if (!(f->mode & 0000002) && (f->flags & (0000001|0000002)) != (0000001|0000002))
	    { return((ssize_t)(-1)); };

	if ((unsigned char *) buf != f->next &&
	    (!f->rsrv || f->rsrv->data != (unsigned char *) buf))
	    { return((ssize_t)(-1)); };

	f->mode &= ~00000400;

	if (f->mode & 00001000) {	/* read past peeked data */
	    char buf[16];
	    register ssize_t r;
	...
	if ((f->mode & 0000001) && f->proc)
	    f->next += n;
    }

    s = begs = (unsigned char *) buf;
    for (;; f->mode &= ~00000040) {	/* check stream mode */
	if (((f)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) ) != 0000002
	    && _sfmode(f, 0000002, local) < 0) {
	    w = s > begs ? s - begs : -1;
	    { return(w); };
	}
	...
	if (w == 0 || ((f->flags & 0020000) && w < (ssize_t) n)) {
	    if (f->flags & 0000004) {	/* extend buffer */
		(void) (((f)->mode |= 00100000),sfwr(f,(Void_t*)s,n - w,f->disc));
		if ((w = f->endb - f->next) < (ssize_t) n)
		    break;
	    } else if (f->next > f->data) {
		(void) (((f)->mode |= 00100000),_sfflsbuf(f,-1));
		if ((w = f->endb - f->next) < (ssize_t) n &&
		    (f->flags & 0020000) && f->next > f->data)
		    break;
	    }
	}

	if (!(f->flags & 0000004) && f->next == f->data && (((ssize_t)(n) >= (f)->size) || ((n) >= 1024 && (ssize_t)(n) >= (f)->size/16 ) )) {	/* bypass buffering */
	    if ((w = (((f)->mode |= 00100000),sfwr(f,(Void_t*)s,n,f->disc))) <= 0)
		break;
	} else {
	    if (w > (ssize_t) n)
		w = (ssize_t) n;
    ...
    if (f->extent < 0 && (f->flags & 0000100) && !(f->flags & 0004000))
	(void) (((f)->mode |= 00100000),_sfflsbuf(f,-1));

    /* check to see if buffer should be flushed */
    else if (n == 0 && (f->flags & 0000040) && !(f->flags & 0000004)) {
