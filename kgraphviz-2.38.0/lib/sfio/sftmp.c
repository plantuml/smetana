#line 1 "graphviz-2.38.0\\lib\\sfio\\sftmp.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sftmp.c" 2
...

{
	...
	static unsigned long Key, A;
	if (A == 0 || t > 0) {	/* get a quasi-random coefficient */
	    register int r;
	    A = (unsigned long) time(((time_t *)0)) ^ (((unsigned long) (&t)) >> 3);
	    if (Key == 0)
	...
	if ((fd = open(file, 000)) >= 0) {	/* file already exists */
	    { while(close(fd) < 0 && errno == 4) errno = 0; };
	    fd = -1;
	} else if ((fd = creat(file, 0666)) >= 0) {	/* reopen for read and write */
	    { while(close(fd) < 0 && errno == 4) errno = 0; };
	    if ((fd = open(file, 002)) >= 0)
		break;
...

{
    ...
    if (type != 0000002 && type != 3 &&
	type != 5 && type != 6 && type != 8)
	return 0;

    /* notify function */
    notifyf = (_Sfextern.sf_notify);

    /* try to create the temp file */
    ( (&newf)->next = (unsigned char*)0, (&newf)->endw = (unsigned char*)0, (&newf)->endr = (unsigned char*)0, (&newf)->endb = (unsigned char*)0, (&newf)->push = (Sfio_t*)0, (&newf)->flags = (unsigned short)0, (&newf)->file = -1, (&newf)->data = (unsigned char*)0, (&newf)->size = (ssize_t)(-1), (&newf)->val = (ssize_t)(-1), (&newf)->extent = (_ast_intmax_t)(-1), (&newf)->here = (_ast_intmax_t)0, (&newf)->getr = 0, (&newf)->tiny[0] = 0, (&newf)->bits = 0, (&newf)->mode = 0, (&newf)->disc = (struct _sfdisc_s*)0, (&newf)->pool = (struct _sfpool_s*)0, (&newf)->rsrv = (struct _sfrsrv_s*)0, (&newf)->proc = (struct _sfproc_s*)0, (&newf)->mutex = (((Vtmutex_t *)0)), (&newf)->stdio = (Void_t*)0, (&newf)->lpos = (_ast_intmax_t)0, (&newf)->iosz = (size_t)0 );
    newf.flags = 0001000;
    newf.mode = 00020000;
    ...
    (_Sfextern.sf_notify) = 0;
    sf = sfnew(&newf, ((Void_t *)0), (size_t) (-1), fd,
	       0000001 | 0000002);
    (_Sfextern.sf_notify) = notifyf;
    if (!sf)
	return -1;
	...
	(void)(-1);
	(void)(-1);
	newf.mutex = ((Vtmutex_t *)0);
    }

    /* make sure that new stream has the same mode */
    if ((m = f->flags & (0000001 | 0000002)) != (0000001 | 0000002))
	sfset(sf, ((~m) & (0000001 | 0000002)), 0);
    sfset(sf, (f->mode & (0000001 | 0000002)), 1);
    ...
    if (savf.data) {
	{ _ast_intmax_t s = (&savf)->next - (&savf)->data; if(s > (&savf)->here) { (&savf)->here = s; if(s > (&savf)->extent) (&savf)->extent = s; } };
	if (!(savf.flags & 0000020))
	    (void) sfsetbuf(f, (Void_t *) savf.data, savf.size);
	if (savf.extent > 0)
	    (void) sfwrite(f, (Void_t *) savf.data, (size_t) savf.extent);
	(void) sfseek(f, (_ast_intmax_t) (savf.next - savf.data), 0);
	if ((savf.flags & 0000020))
    ...
    newf.data = newf.endb = newf.endr = newf.endw = ((unsigned char *)0);
    newf.file = -1;
    sfclose(&newf);
...
Sfio_t *sftmp(register size_t s)
...

{
	...
	{ ((Sfread_f)0), ((Sfwrite_f)0), ((Sfseek_f)0), _tmpexcept,
    ...
    if (!
	(f =
	 sfnew(((Sfio_t *)0), ((char *)0), s, -1,
	       0000004 | 0000001 | 0000002)))
	 return ((Sfio_t *)0);

    if (s != (size_t) (-1))	/* set up a discipline for out-of-bound, etc. */
	f->disc = &Tmpdisc;

    /* make the file now */
    if (s == 0 && _tmpexcept(f, 6, ((Void_t *)0), f->disc) < 0) {
	sfclose(f);
	return ((Sfio_t *)0);
    }
