#line 1 "graphviz-2.38.0\\lib\\sfio\\sfwr.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfwr.c" 2
...
static ssize_t sfoutput(Sfio_t * f, register char *buf, register size_t n)
...
{
    register char *sp, *wbuf, *endbuf;
    register ssize_t s, w, wr;

    s = w = 0;
    wbuf = buf;
    endbuf = buf + n;
    while (n > 0) {
	if ((ssize_t) n < (_Sfextern.sf_page)) {	/* no hole possible */
	    buf += n;
	    s = n = 0;
	} else
	    while ((ssize_t) n >= (_Sfextern.sf_page)) {	/* see if a hole of 0's starts here */
		sp = buf + 1;
		if (buf[0] == 0 && buf[(_Sfextern.sf_page) - 1] == 0) {	/* check byte at a time until int-aligned */
		    while (((unsigned long) sp) % sizeof(int)) {
			if (*sp != 0)
			    goto chk_hole;
			sp += 1;
		    }
	      ...
	      chk_hole:
		if ((s = sp - buf) >= (_Sfextern.sf_page))	/* found a hole */
		    break;

		/* skip a dirty page */
		n -= (_Sfextern.sf_page);
		buf += (_Sfextern.sf_page);
	    }

	/* write out current dirty pages */
	if (buf > wbuf) {
	    if ((ssize_t) n < (_Sfextern.sf_page)) {
		buf = endbuf;
		n = s = 0;
	    }
	...
	if (s >= (_Sfextern.sf_page)) {
	    ...
	    if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) s,1,((Sfdisc_t *)0))) < 0)
		break;
	    w += s;
	    n -= s;
	    wbuf = (buf += s);
		...
		s = (ssize_t) n <= (_Sfextern.sf_page) ? 1 : (_Sfextern.sf_page);
		buf += s;
		n -= s;
	    }
	}
...
ssize_t sfwr(register Sfio_t * f, register const Void_t * buf, register size_t n,
...
{
    register ssize_t w;
    register Sfdisc_t *dc;
    register int local, oerrno;

    { if(!f) return((ssize_t)(-1)); };

    ((local) = ((f)->mode&00100000), (f)->mode &= ~00100000, (local));
    if (!local && !(f->bits & 00001000)) {	/* an external user's call */
	if (f->mode != 0000002 && _sfmode(f, 0000002, 0) < 0)
	    { return((ssize_t)(-1)); };
	if (f->next > f->data && (((f)->mode |= 00100000),sfsync(f)) < 0)
	    { return((ssize_t)(-1)); };
    }

    for (;;) {			/* stream locked by sfsetfd() */
	if (!(f->flags & 0000004) && f->file < 0)
	    { return((ssize_t) 0); };

	/* clear current error states */
	f->flags &= ~(0000200 | 0000400);

	dc = disc;
	if (f->flags & 0000004)	/* total required buffer */
	    w = n + (f->next - f->data);
	else {			/* warn that a write is about to happen */
	    { Sfdisc_t* d; if(!(dc)) d = (dc) = (f)->disc; else d = (f->bits&00001000) ? ((dc) = (dc)->disc) : (dc); while(d && !(d->writef)) d = d->disc; if(d) (dc) = d; };
	    if (dc && dc->exceptf && (f->flags & 0002000)) {
		register int rv;
		if (local)
		    ((f)->mode |= 00100000);
		if ((rv = _sfexcept(f, 0000002, n, dc)) > 0)
		    n = rv;
		else if (rv < 0) {
		    f->flags |= 0000400;
		    { return(rv); };
		...
		if (f->flags & 0000010) {
		    if (f->here != f->extent || (f->flags & 0000100)) {
			f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) 0,2,dc));
			f->extent = f->here;
		    }
		} else if ((f->flags & 0000100)
			   && !(f->flags & 0004000))
		    f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)f->here,0,dc));
	    }
	    ...
	    if (dc && dc->writef) {
		{ int dcdown = f->bits&00001000; f->bits |= 00001000; w = (*dc->writef)(f,buf,n,dc); if(!dcdown) f->bits &= ~00001000; };
	    } else if (((f)->extent < 0 && ((f)->bits&00000010) ))
		w = n;
	    else if (f->flags & 0020000)
		goto do_write;
	    else if ((ssize_t) n >= (_Sfextern.sf_page) &&
		     !(f->flags & (0000100 | 0000010)) &&
		     f->here == f->extent && (f->here % (_Sfextern.sf_page)) == 0) {
	    ...
	    if (w > 0) {
		if (!(f->bits & 00001000)) {
		    if (f->flags & (0000010 | 0004000))
			f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) 0,1,dc));
		    else
			f->here += w;
	...
	if (local)
	    ((f)->mode |= 00100000);
	switch (_sfexcept(f, 0000002, w, dc)) {
	case 3:
	    goto do_continue;
	case 0:
	    w = local ? 0 : w;
