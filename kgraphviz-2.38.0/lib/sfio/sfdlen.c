#line 1 "graphviz-2.38.0\\lib\\sfio\\sfdlen.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfdlen.c" 2
...

{
    ...
    for (w = 1; w <= (16*sizeof(_ast_fltmax_t)); ++w) {	/* get 2^SF_PRECIS precision at a time */
	n = (int) (x = ldexp(v, (7-1)));
	v = x - n;
	if (v <= 0.)
	    break;
    ...
    return 1 + ( ((unsigned _ast_intmax_t)(exp) < (1 << 7) ? 1 : (unsigned _ast_intmax_t)(exp) < ((1 << 7)*(1 << 7)) ? 2 : (unsigned _ast_intmax_t)(exp) < (((1 << 7)*(1 << 7))*(1 << 7)) ? 3 : (unsigned _ast_intmax_t)(exp) < ((((1 << 7)*(1 << 7))*(1 << 7))*(1 << 7)) ? 4 : 5) ) + w;
}
