#line 1 "graphviz-2.38.0\\lib\\sfio\\sfresize.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfresize.c" 2
...
int sfresize(Sfio_t * f, _ast_intmax_t size)
...

{
	...
	(f->mode != 0000002 && _sfmode(f, 0000002, 0) < 0))
	{ return(-1); };
	...
	if (f->extent >= size) {
	    if ((f->flags & 0000020) && (f->next - f->data) <= size) {
	    ...
	    memset(((char *)(f->data + size)),'\0',((int)(f->extent - size)));
	} else {
	    if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)size,0,f->disc)) != size)
		{ return(-1); };
