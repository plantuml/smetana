#line 1 "graphviz-2.38.0\\lib\\sfio\\sfnputc.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfnputc.c" 2
...
ssize_t sfnputc(register Sfio_t * f, register int c, register size_t n)
...

{
    ...
    if (((f)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) ) != 0000002 && _sfmode(f, 0000002, local) < 0)
	{ return(-1); };
    ...
    if ((size_t) p > n)
	p = n;
    switch(p) { default : memset((Void_t*)ps,(int)c,p); ps += p; break; case 7 : *ps++ = c; case 6 : *ps++ = c; case 5 : *ps++ = c; case 4 : *ps++ = c; case 3 : *ps++ = c; case 2 : *ps++ = c; case 1 : *ps++ = c; };
    ps -= p;
	...
	if ((p = (((f)->mode |= 00100000),sfwrite(f,(Void_t *) ps,p))) <= 0 || (n -= p) <= 0) {
	    w -= n;
	    goto done;
	}
	if ((size_t) p > n)
