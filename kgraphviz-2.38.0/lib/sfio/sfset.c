#line 1 "graphviz-2.38.0\\lib\\sfio\\sfset.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfset.c" 2
...
int sfset(register Sfio_t * f, register int flags, register int set)
...

{
    ...
    if (flags == 0 && set == 0)
	{ return((f->flags & 0077177)); };

    if ((oflags = (f->mode & (0000001|0000002))) != (int) f->mode
    ...
    oflags = f->flags;
    if (!(f->bits & 00000002) || (flags & (0000001|0000002)) == (0000001|0000002))
	flags &= ~(0000001|0000002);

    /* set the flag */
    if (set)
	f->flags |= (flags & 0027163);
    else
	f->flags &= ~(flags & 0027163);

    /* must have at least one of read/write */
    if (!(f->flags & (0000001|0000002)))
	f->flags |= (oflags & (0000001|0000002));
	...
	if ((flags == 0000002 && !(f->mode & 0000002)) ||
	    (flags == 0000001 && !(f->mode & (0000001 | 00004000))))
	    (void) _sfmode(f, flags, 1);
    }

    /* if not shared or unseekable, public means nothing */
    if (!(f->flags & 0000100) || f->extent < 0)
	f->flags &= ~0004000;
