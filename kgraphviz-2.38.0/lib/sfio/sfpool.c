#line 1 "graphviz-2.38.0\\lib\\sfio\\sfpool.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfpool.c" 2
...
static int delpool(register Sfpool_t * p)
...
static Sfpool_t *newpool(register int mode)




{
    register Sfpool_t *p, *last = &(_Sfextern.sf_pool);

    /* look to see if there is a free pool */
    for (last = &(_Sfextern.sf_pool), p = last->next; p; last = p, p = p->next) {
	if (p->mode == 00020000) {
	    p->mode = 0;
	    break;
	}
	...
	(void)((Vtmutex_t*)0);	/* initialize mutex */
...
{
    register Sfio_t *head;
    register ssize_t k, w, v;
    register int rv;
    ...
    head = p->sf[0];
    if (((head)->mode&(00000100|00000040|00000400) ? 1 : ((head)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(head) : 0))
	{ return(-1); };

    (void)((head)->mode |= 00000040, (head)->endr = (head)->endw = (head)->data);
    rv = -1;

    if (!(p->mode & 0000100)) {
	if ((((head)->mode |= 00100000),sfsync(head)) < 0)
	    goto done;
    } else {			/* shared pool, data can be moved among streams */
	if (((head)->mode & ~(00000020|00000010|((1) ? 00000040 : 0)) ) != 0000002 && _sfmode(head, 0000002, 1) < 0)
	    goto done;
	 /**/ ;
	 /**/ ;
	...
	if ((k = v - (f->endb - f->data)) <= 0)
	    ...
	    if ((w = (((head)->mode |= 00100000),sfwr(head,(Void_t*)head->data,k,head->disc))) == k)
		v -= k;
    ...
    head->mode &= ~00000040;	/* partially unlock because it's no longer head */
...

{
    ...
    if (p->n_sf == 0 || p == &(_Sfextern.sf_pool)) {
	if (p != &(_Sfextern.sf_pool))
	    delpool(p);
	goto done;
    }
    ...
    for (n = 0; n < p->n_sf; ++n)
	if (!((p->sf[n])->mode&(00000100|00000040|00000400) ? 1 : ((p->sf[n])->mode&00010000) ? (*(_Sfextern.sf_stdsync))(p->sf[n]) : 0))
	    break;
    if (n < p->n_sf && n > 0) {
	f = p->sf[n];
	p->sf[n] = p->sf[0];
    ...
    f = p->sf[0];
    f->mode &= ~00000200;
    if (!((f)->mode&(00000100|00000040|00000400) ? 1 : ((f)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f) : 0))
	((f)->mode == 0000001 ? ((f)->endr = ((f)->flags&0) ? (f)->data : (f)->endb) : (f)->mode == 0000002 ? ((f)->endw = ((f)->flags&(0|0000040)) ? (f)->data : (f)->endb) : ((f)->endw = (f)->endr = (f)->data) );
...
static int _sfpmove(register Sfio_t * f, register int type)
...
Sfio_t *sfpool(register Sfio_t * f, register Sfio_t * pf, register int mode)
...

{
    ...
    (_Sfextern.sf_pmove) = _sfpmove;

    if (!f) {			/* return head of pool of pf regardless of lock states */
	if (!pf)
	    return ((Sfio_t *)0);
	else if (!pf->pool || pf->pool == &(_Sfextern.sf_pool))
	    return pf;
	else
	    return pf->pool->sf[0];
    }
	...
	(void)(0);
	if ((f->mode & (0000001|0000002)) != f->mode && _sfmode(f, 0, 0) < 0) {
	    (void)(0);
	    return ((Sfio_t *)0);
	}
	if (f->disc == (&(_Sfextern.sf_udisc)))
	    (void) sfclose((*(_Sfextern.sf_stack)) (f, ((Sfio_t *)0)));
    }
    if (pf) {
	(void)(0);
	if ((pf->mode & (0000001|0000002)) != pf->mode && _sfmode(pf, 0, 0) < 0) {
	    if (f)
		(void)(0);
	    (void)(0);
	    return ((Sfio_t *)0);
	}
	if (pf->disc == (&(_Sfextern.sf_udisc)))
	    (void) sfclose((*(_Sfextern.sf_stack)) (pf, ((Sfio_t *)0)));
    }

    /* f already in the same pool with pf */
    if (f == pf || (pf && f->pool == pf->pool && f->pool != &(_Sfextern.sf_pool))) {
	if (f)
	    (void)(0);
	if (pf)
	    (void)(0);
    ...
    if (pf)
	(void)((pf)->mode |= 00000040, (pf)->endr = (pf)->endw = (pf)->data);

    if (!pf) {			/* deleting f from its current pool */
	if (!(p = f->pool) || p == &(_Sfextern.sf_pool) ||
	    _sfpmove(f, -1) < 0 || _sfsetpool(f) < 0)
	    goto done;

	if ((p = f->pool) == &(_Sfextern.sf_pool) || p->n_sf <= 0)
	    rv = f;
	else
    ...
    if (pf->pool && pf->pool != &(_Sfextern.sf_pool))	/* always use current mode */
	mode = pf->pool->mode;

    if (mode & 0000100) {	/* can only have write streams */
	if (((f)->mode & ~(00000020|00000010|((1) ? 00000040 : 0)) ) != 0000002 && _sfmode(f, 0000002, 1) < 0)
	    goto done;
	if (((pf)->mode & ~(00000020|00000010|((1) ? 00000040 : 0)) ) != 0000002 && _sfmode(pf, 0000002, 1) < 0)
	    goto done;
	if (f->next > f->data && (((f)->mode |= 00100000),sfsync(f)) < 0)	/* start f clean */
	    goto done;
    }
    ...
    if (!(p = pf->pool) || p == &(_Sfextern.sf_pool)) {	/* making a new pool */
	if (!(p = newpool(mode)))
	    goto done;
	...
	goto done;
     ...
     /**/ ;
    (void)((0) ? 0 : ((pf)->mode &= ~(00000040|00000010|00000020), ((pf)->mode == 0000001 ? ((pf)->endr = ((pf)->flags&0) ? (pf)->data : (pf)->endb) : (pf)->mode == 0000002 ? ((pf)->endw = ((pf)->flags&(0|0000040)) ? (pf)->data : (pf)->endb) : ((pf)->endw = (pf)->endr = (pf)->data) ), 0) );
