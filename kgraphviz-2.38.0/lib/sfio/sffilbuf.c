#line 1 "graphviz-2.38.0\\lib\\sfio\\sffilbuf.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sffilbuf.c" 2
...
int _sffilbuf(Sfio_t * f, register int n)
...
{
    register ssize_t r;
    register int first, local, rcrv, rc, justseek;
    ...
    rcrv = f->mode & (00000010 | 00000020 | 00000040);
    rc = f->getr;

    justseek = f->bits & 00000040;
    f->bits &= ~00000040;

    for (first = 1;; first = 0, (f->mode &= ~00000040)) {	/* check mode */
	...
	if ((r = f->endb - f->next) > 0) {	/* on first iteration, n is amount beyond current buffer;
	    ...
	    if (!(f->bits & 00000001) && f->next > f->data &&
		n > (f->size - (f->endb - f->data))) {
		memcpy(f->data, f->next, r);
		f->next = f->data;
		f->endb = f->data + r;
	    }
	} else if (!(f->flags & 0000004) && !(f->bits & 00000001))
	...
	if (f->bits & 00000001)
	    ...
	    if (n > 0) {
		if (r > n && f->extent < 0 && (f->flags & 0000100))
	...
	f->mode |= rcrv;
	f->getr = rc;
	if ((r = (((f)->mode |= 00100000),sfrd(f,(Void_t*)f->endb,r,f->disc))) >= 0) {
	    r = f->endb - f->next;
	    break;
	}
    }
    ...
    rcrv = (n == 0) ? (r > 0 ? (int) (*f->next++) : (-1)) : (int) r;
