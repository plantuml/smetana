#line 1 "graphviz-2.38.0\\lib\\sfio\\sfseek.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfseek.c" 2
...
static void newpos(Sfio_t * f, _ast_intmax_t p)
...

{
    ...
    f->endb = (f->mode & 0000002) ? f->data + f->size : f->data;
    if ((f->here = p) < 0) {
	f->extent = -1;
	f->here = 0;
    }
...
_ast_intmax_t sfseek(Sfio_t * f, _ast_intmax_t p, int type)
...

{
    ...
    { if(!f) return((_ast_intmax_t)(-1)); };

    ((local) = ((f)->mode&00100000), (f)->mode &= ~00100000, (local));

    hardseek = (type | f->flags) & (0000100 | 0004000);

    if (hardseek && f->mode == (0000001 | 00004000)) {
	newpos(f, f->here);
	f->mode = 0000001;
    }

    /* set and initialize the stream to a definite mode */
    if ((int) ((f)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) ) != (mode = f->mode & (0000001|0000002))) {
	int flags = f->flags;

	if (hardseek & 0004000)	/* seek ptr must follow file descriptor */
	...
	if (mode < 0)
	    { return((_ast_intmax_t)(-1)); };
    }

    mustsync = (type & 0000100) && !(type & 0004000) &&
	(f->mode & 0000001) && !(f->flags & 0000004);
	...
	(void) (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) 0,1,f->disc));
	{ return((_ast_intmax_t)(-1)); };
    }
	...
	if (r >= 0 && r <= f->size) {
	    ...
	    f->here = p;
	    if (p > f->extent)
		memset(((char *)(f->data + f->extent)),'\0',(
(int)(p - f->extent)));
	    goto done;
	}

	/* check exception handler, note that this may pop stream */
	if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)r,0,f->disc)) != r) {
	    p = -1;
	    goto done;
    ...
    if (f->mode & 0000002) {	/* see if we can avoid flushing buffer */
	if (!hardseek && type < 2 && !(f->flags & 0000010)) {
    ...
    if (type == 2 || (f->mode & 0000002)) {
	if ((hardseek & 0004000) || type == 2)
	    p = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)p,type,f->disc));
	else {
	    r = p + (type == 1 ? f->here : 0);
	    p = (hardseek
		 || r != f->here) ? (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)r,0,f->disc)) : r;
	}
	if (p >= 0)
	    newpos(f, p);
	...
	if ((hardseek || (type == 1 && p == 0))) {
	    if ((s = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) 0,1,f->disc))) == f->here ||
		(s >= 0 && !(hardseek & 0004000) &&
		 (s = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)f->here,0,f->disc))) == f->here))
		goto near_done;
	    else if (s < 0) {
		p = -1;
		goto done;
	...
	if (b <= 1024)
	    ...
	    c *= 2;
	    a = c + 3 * (b - c) / 4;
	    a = ((a + 1024 - 1) / 1024) * 1024;

	    b = ((b + 1024 - 1) / 1024) * 1024;
    ...
    if (p < f->lpos && f->size > 1024 && (p + 1024) > s) {
	if ((r = s - f->size) < 0)
	    r = 0;
    } else {
	r = p;
	...
	&& (f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)r,0,f->disc))) != r) {
	if (r < p)		/* now try to just get to p */
	    f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)p,0,f->disc));
	if (f->here != p)
	    p = -1;
	goto done;
    }
	...
	(void) (((f)->mode |= 00100000),sfrd(f,(Void_t*)f->data,f->size,f->disc));
	if (p <= f->here && p >= (f->here - (f->endb - f->data)))
	    ...
	    if ((f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)p,0,f->disc))) != p)
		p = -1;
	}
    }
