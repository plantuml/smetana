#line 1 "graphviz-2.38.0\\lib\\sfio\\sftell.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sftell.c" 2
...
_ast_intmax_t sftell(register Sfio_t * f)
...

{
    ...
    if (f->flags & 0000004)
	{ return((_ast_intmax_t)(f->next - f->data)); };

    /* let sfseek() handle the hard case */
    if (f->extent >= 0 && (f->flags & (0000100 | 0000010)))
	p = sfseek(f, (_ast_intmax_t) 0, 1);
    else
	p = f->here + ((f->mode & 0000002) ? f->next - f->data : f->next -
		       f->endb);
