#line 1 "graphviz-2.38.0\\lib\\sfio\\sfpoll.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfpoll.c" 2
...
int sfpoll(Sfio_t ** fa, register int n, int tm)
...

{
  ...
  retry:for (r = c = 0; r < n; ++r) {
	...
	for (;;) {		/* check accessibility */
	    ...
	    if (!(f->flags & 0000004) &&
		((f->mode & 0000002) || f->here < f->extent))
		goto do_ready;

	    /* at an apparent eof, pop stack if ok, then recheck */
	    ((f)->mode |= 00100000);
	    switch (_sfexcept(f, f->mode & (0000001|0000002), 0, f->disc)) {
	    case 0:
		if (f->flags & 0000004)
		    goto do_never;
		else
	...
	if ((d = fa[c]->disc) && d->exceptf) {
	    if ((r = (*d->exceptf) (fa[c], 12, (Void_t *) 0, d)) < 0)
		goto done;
	    else if (r > 0)
		goto retry;
	}
