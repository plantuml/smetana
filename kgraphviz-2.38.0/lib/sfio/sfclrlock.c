#line 1 "graphviz-2.38.0\\lib\\sfio\\sfclrlock.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfclrlock.c" 2
...
int sfclrlock(register Sfio_t * f)
...

{
    ...
    /* already closed */
    if (f && (f->mode & 00020000))
	return 0;

    { if(!f) return(0); };

    /* clear error bits */
    f->flags &= ~(0000400 | 0000200);
    ...
    ((!((f)->bits&00002000) ? 0 : (((f)->bits &= ~00002000), ((f)->size /= 8)) ), ((f)->bits &= ~(00001000|00002000)) );

    /* throw away all lock bits except for stacking state SF_PUSH */
    f->mode &=
	((0000001|0000002) | 0000004 | 00000200 | 00000100 | 00004000 | 00010000);

    rv = (f->mode & 00000100) ? 0 : (f->flags & 0077177);
