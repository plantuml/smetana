#line 1 "graphviz-2.38.0\\lib\\sfio\\sfsize.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfsize.c" 2
...
_ast_intmax_t sfsize(register Sfio_t * f)
...

{
    ...
    if (f->extent >= 0) {
	if (f->flags & (0000100 | 0000010)) {
	    for (disc = f->disc; disc; disc = disc->disc)
		if (disc->seekf)
		    break;
	    if (!0 || disc) {
		_ast_intmax_t e;
		if ((e = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)0,2,disc))) >= 0)
		    f->extent = e;
		if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)f->here,0,disc)) != f->here)
		    f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) 0,1,disc));
	    }
	...
	if ((f->flags & (0000100 | 0004000)) == (0000100 | 0004000))
	    f->here = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t) 0,1,f->disc));
    }

    if (f->here != s && (f->mode & 0000001)) {	/* buffered data is known to be invalid */
