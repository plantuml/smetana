#line 1 "graphviz-2.38.0\\lib\\sfio\\sfopen.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfopen.c" 2
...
Sfio_t *sfopen(register Sfio_t * f, const char *file, const char *mode)
...

{
    ...
    if ((sflags = _sftype(mode, &oflags, ((int *)0))) == 0)
	return ((Sfio_t *)0);

    /* usually used on the standard streams to change control flags */


    if (f && !file && (f->mode & 0000004)) {
	{ if(!f) return(((Sfio_t *)0)); };

	if (f->mode & 0000004) {	/* paranoia in case another thread snuck in */
	    if (f->file >= 0 && !(f->flags & 0000004) && (oflags &= (000 | 000 | 020)) != 0) {	/* set the wanted file access control flags */
		int ctl = fcntl(f->file, F_GETFL, 0);
		ctl = (ctl & ~(000 | 000 | 020)) | oflags;
		fcntl(f->file, F_SETFL, ctl);
	    }
	    /* set all non read-write flags */
	    f->flags |= (sflags & (0077177 & ~(0000001|0000002)));

	    /* reset read/write modes */
	    if ((sflags &= (0000001|0000002)) != 0) {
		f->flags = (f->flags & ~(0000001|0000002)) | sflags;

		if ((f->flags & (0000001|0000002)) == (0000001|0000002))
		    f->bits |= 00000002;
		else
		    f->bits &= ~00000002;

		if (f->flags & 0000001)
		    f->mode = (f->mode & ~0000002) | 0000001;
		else
		    f->mode = (f->mode & ~0000001) | 0000002;
	    }
	...
	while ((fd = open(file, oflags & (001 | 002))) < 0
	       && errno == 4)
	    errno = 0;
	if (fd >= 0) {
	    if ((oflags & (004 | 040)) == (004 | 040)) {
		{ while(close(fd) < 0 && errno == 4) errno = 0; };	/* error: file already exists */
		return ((Sfio_t *)0);
	    }
	    if (oflags & 010) {	/* truncate file */
		register int tf;
		while ((tf = creat(file, 0666)) < 0 &&
		       errno == 4)
		    errno = 0;
		{ while(close(tf) < 0 && errno == 4) errno = 0; };
	    }
	} else if (oflags & 004) {
	    while ((fd = creat(file, 0666)) < 0 && errno == 4)
		errno = 0;
	    if (!(oflags & 001)) {	/* the file now exists, reopen it for read/write */
	...
	oldfd = f ? f->file : -1;
	if ((f = sfnew(f, ((char *)0), (size_t) (-1), fd, sflags))
	    && oldfd >= 0)
...
int _sftype(register const char *mode, int *oflagsp, int *uflagp)
...
{
    register int sflags, oflags, uflag;
	...
	default:
		...
		oflags = (oflags & ~(000 | 001)) | 002;
	    if (oflagsp)
		*oflagsp = oflags;
	    if (uflagp)
		*uflagp = uflag;
	    if ((sflags & (0000004 | (0000001|0000002))) == 0000004)
		sflags |= 0000001;
	    return sflags;
	}
}
