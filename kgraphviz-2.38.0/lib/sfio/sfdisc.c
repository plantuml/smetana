#line 1 "graphviz-2.38.0\\lib\\sfio\\sfdisc.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfdisc.c" 2
...
Sfdisc_t *sfdisc(register Sfio_t * f, register Sfdisc_t * disc)
...
{
    register Sfdisc_t *d, *rdisc;
    register Sfread_f oreadf;
    register Sfwrite_f owritef;
    register Sfseek_f oseekf;
    ssize_t n;

    { if(!f) return(((Sfdisc_t *)0)); };

    if ((f->flags & 0000001) && f->proc && (f->mode & 0000002)) {	/* make sure in read mode to check for read-ahead data */
	if (_sfmode(f, 0000001, 0) < 0)
	    { return(((Sfdisc_t *)0)); };
    } else if ((f->mode & (0000001|0000002)) != f->mode && _sfmode(f, 0, 0) < 0)
	{ return(((Sfdisc_t *)0)); };
    ...
    if (!(f->flags & 0000004)) {
	if (((f->mode & 0000002) && f->next > f->data) ||
	    (f->mode & 0000001) || f->disc == (&(_Sfextern.sf_udisc)))
	    (void) (((f)->mode |= 00100000),sfsync(f));

	if (((f->mode & 0000002) && (n = f->next - f->data) > 0) ||
	    ((f->mode & 0000001) && f->extent < 0
	     && (n = f->endb - f->next) > 0)) {
	    register Sfexcept_f exceptf;
	    register int rv = 0;

	    exceptf = disc ? disc->exceptf :
		f->disc ? f->disc->exceptf : ((Sfexcept_f)0);
		...
		rv = (*exceptf) (f, 8, &n, disc ? disc : f->disc);
		(void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);
	    }
    ...
    { for(d = f->disc; d && !d->readf; d = d->disc) ; oreadf = d ? d->readf : ((Sfread_f)0); };
    { for(d = f->disc; d && !d->writef; d = d->disc) ; owritef = d ? d->writef : ((Sfwrite_f)0); };
    { for(d = f->disc; d && !d->seekf; d = d->disc) ; oseekf = d ? d->seekf : ((Sfseek_f)0); };

    if (disc == ((Sfdisc_t*)0)) {	/* popping, warn the being popped discipline */
	if (!(d = f->disc))
	    goto done;
	disc = d->disc;
	if (d->exceptf) {
	    (void)((0) ? 0 : ((f)->mode &= ~(00000040|00000010|00000020), ((f)->mode == 0000001 ? ((f)->endr = ((f)->flags&0) ? (f)->data : (f)->endb) : (f)->mode == 0000002 ? ((f)->endw = ((f)->flags&(0|0000040)) ? (f)->data : (f)->endb) : ((f)->endw = (f)->endr = (f)->data) ), 0) );
	    if ((*(d->exceptf)) (f, 6, (Void_t *) disc, d) < 0)
		goto done;
	    (void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);
	}
	f->disc = disc;
	...
	do {			/* loop to handle the case where d may pop itself */
	    ...
	    if (d && d->exceptf) {
		(void)((0) ? 0 : ((f)->mode &= ~(00000040|00000010|00000020), ((f)->mode == 0000001 ? ((f)->endr = ((f)->flags&0) ? (f)->data : (f)->endb) : (f)->mode == 0000002 ? ((f)->endw = ((f)->flags&(0|0000040)) ? (f)->data : (f)->endb) : ((f)->endw = (f)->endr = (f)->data) ), 0) );
		if ((*(d->exceptf)) (f, 5, (Void_t *) disc, d) < 0)
		    goto done;
		(void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);
	    }
	} while (d != f->disc);
    ...
    if (!(f->flags & 0000004)) {	/* this stream may have to be reinitialized */
	...
	if(!reinit) { for(d = f->disc; d && !d->readf; d = d->disc) ; if((d ? d->readf : ((Sfread_f)0)) != oreadf) reinit = 1; };
	if(!reinit) { for(d = f->disc; d && !d->writef; d = d->disc) ; if((d ? d->writef : ((Sfwrite_f)0)) != owritef) reinit = 1; };
	if(!reinit) { for(d = f->disc; d && !d->seekf; d = d->disc) ; if((d ? d->seekf : ((Sfseek_f)0)) != oseekf) reinit = 1; };

	if (reinit) {
	    ((f)->mode |= 00100000);
	    f->bits &= ~00000010;	/* turn off /dev/null handling */
	    if ((f->bits & 00000001) || (f->mode & 0000004))
		sfsetbuf(f, ((Void_t *)0), (size_t) (-1));
	    else if (f->data == f->tiny)
		sfsetbuf(f, ((Void_t *)0), 0);
	    else {
		int flags = f->flags;
		sfsetbuf(f, (Void_t *) f->data, f->size);
		f->flags |= (flags & 0000020);
	    }
	}
    }
