#line 1 "graphviz-2.38.0\\lib\\sfio\\sfclose.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfclose.c" 2
...
int sfclose(register Sfio_t * f)




{
    register int local, ex, rv;
    Void_t *data = ((Void_t *)0);
    ...
    if (!(f->mode & 0000004) &&
	((f)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) ) != (f->mode & (0000001|0000002)) &&
	((f)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) ) != (f->mode & (0000001 | 00004000)) &&
	_sfmode(f, 0, local) < 0)
	{ return(-1); };
	...
	if (!(pop = (*(_Sfextern.sf_stack)) (f, ((Sfio_t *)0))))
	    { return(-1); };
	if (sfclose(pop) < 0) {
	    (*(_Sfextern.sf_stack)) (f, pop);
	    { return(-1); };
	}
    }

    rv = 0;
    if (f->disc == (&(_Sfextern.sf_udisc)))	/* closing the ungetc stream */
	f->disc = ((Sfdisc_t *)0);
    ...
    if (f->disc
	&& (ex =
	    (((f)->mode |= 00100000),sfraise(f,local ? 0 : 4,((Void_t *)0)))) != 0)
	{ return(ex); };

    if (!local && f->pool) {	/* remove from pool */
	if (f->pool == &(_Sfextern.sf_pool)) {
	    register int n;

	    ;
	    for (n = 0; n < (_Sfextern.sf_pool).n_sf; ++n) {
		if ((_Sfextern.sf_pool).sf[n] != f)
		    continue;
		/* found it */
		(_Sfextern.sf_pool).n_sf -= 1;
		for (; n < (_Sfextern.sf_pool).n_sf; ++n)
		    (_Sfextern.sf_pool).sf[n] = (_Sfextern.sf_pool).sf[n + 1];
		break;
	    }
	    ;
	} else {
    ...
    if (f->data && (!local || (f->flags & 0000004) || (f->bits & 00000001))) {	/* free buffer */
	...
	(*(_Sfextern.sf_notify)) (f, 4, f->file);
    if (f->file >= 0 && !(f->flags & 0000004))
	{ while(close(f->file) < 0 && errno == 4) errno = 0; };
    f->file = -1;

    ((f)->mode = (00020000|00000040) );
    f->flags &= 0001000;
    f->here = 0;
    f->extent = -1;
    ...
    if (!local) {
	if (f->disc && (ex = (((f)->mode |= 00100000),sfraise(f,11,((Void_t *)0)))) != 0) {
	    rv = ex;
	    goto done;
	}
