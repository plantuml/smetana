#line 1 "graphviz-2.38.0\\lib\\sfio\\sfreserve.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfreserve.c" 2
...
Void_t *sfreserve(register Sfio_t * f, ssize_t size, int type)
...

{
	...
	if ((rsrv = f->rsrv) && (n = -rsrv->slen) > 0) {
	    rsrv->slen = 0;
	    _Sfi = f->val = n;
	    { return((Void_t *) rsrv->data); };
	} else
	    { return(((Void_t *)0)); };
    }

    if (type > 0 && !(type == 0000010 || type == 1))
	{ return(((Void_t *)0)); };
	...
	else if (type > 0 && f->extent < 0 && (f->flags & 0000100)) {
	    if (n == 0) {	/* peek-read only if there is no buffered data */
		f->mode |= 00000020;
		(void) (((f)->mode |= 00100000),_sffilbuf(f,sz == 0 ? -1 :(sz - n)));
	    }
	...
	if (f->flags & 0000004) {
	    if ((f->mode & 0000002) && (f->flags & 0000020)) {
		(void) (((f)->mode |= 00100000),sfwr(f,(Void_t*)f->next,sz,f->disc));
		n = f->endb - f->next;
	    }
	    ...
	    if (type <= 0 && (rsrv = _sfrsrv(f, sz)) &&
		(n = (((f)->mode |= 00100000),sfread(f,(Void_t *) rsrv->data,sz))) < sz)
		rsrv->slen = -n;
	}
    }
