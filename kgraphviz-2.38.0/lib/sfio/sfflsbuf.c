#line 1 "graphviz-2.38.0\\lib\\sfio\\sfflsbuf.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfflsbuf.c" 2
...
int _sfflsbuf(register Sfio_t * f, register int c)
...
{
    register ssize_t n, w;
    register unsigned char *data;
    unsigned char outc;
    register int local, isall;
    int inpc = c;
	...
	if (n == (f->endb - data) && (f->flags & 0000004)) {	/* extend string stream buffer */
	    (void) (((f)->mode |= 00100000),sfwr(f,(Void_t*)data,1,f->disc));

	    /* !(f->flags&SF_STRING) is required because exception
	       handlers may turn a string stream to a file stream */
	    if (f->next < f->endb || !(f->flags & 0000004))
		n = f->next - (data = f->data);
	    else {
	...
	if (c >= 0) {		/* write into buffer */
		...
		*f->next++ = c;
		if (c == '\n' &&
		    (f->flags & 0000040) && !(f->flags & 0000004)) {
		    c = -1;
		    n += 1;
		} else
		    break;
	...
	if (n == 0 || (f->flags & 0000004))
	    break;

	isall = ((((isall) = (f)->mode&00000020) ? ((f)->mode &= ~00000020) : 0), ((isall) || (f)->extent < 0 || ((f)->flags&(0000100|0000010|0020000)) ) );
	if ((w = (((f)->mode |= 00100000),sfwr(f,(Void_t*)data,n,f->disc))) > 0) {
