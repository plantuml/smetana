#line 1 "graphviz-2.38.0\\lib\\sfio\\sfraise.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfraise.c" 2
...
{
    register Sfdisc_t *disc, *next, *d;
    register int local, rv;

    { if(!f) return(-1); };

    ((local) = ((f)->mode&00100000), (f)->mode &= ~00100000, (local));
    if (!(((f)->mode&(00020000|00000040)) == (00020000|00000040) ) &&
	!(local &&
	  (type == 0 || type == 4 ||
	   type == 11 || type == 14)) &&
