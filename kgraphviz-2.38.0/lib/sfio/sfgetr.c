#line 1 "graphviz-2.38.0\\lib\\sfio\\sfgetr.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfgetr.c" 2
...
char *sfgetr(register Sfio_t * f, register int rc, int type)
...
{
    register ssize_t n;
    register unsigned char *s, *ends, *us;
    register ssize_t un;
    register int found;
    ...
    if (rc < 0 || (f->mode != 0000001 && _sfmode(f, 0000001, 0) < 0))
	{ return(((char *)0)); };
    ...
    /* compatibility mode */
    type = type < 0 ? 0000020 : type == 1 ? 0000004 : type;

    if (type & 0000020) {	/* return the broken record */
	...
	if ((n = (ends = f->endb) - (s = f->next)) <= 0) {	/* for unseekable devices, peek-read 1 record */
	    ...
	    if ((((n) = (f)->endb - ((s) = (f)->next) ) > 0 ? (n) : ((n) = (((f)->mode |= 00100000),_sffilbuf(f,-1)), (s) = (f)->next, (n)) ) <= 0) {
		us = ((unsigned char *)0);
		goto done;
	    } else {
		ends = s + n;
	...
	if (s < ends) {
	    ...
	    if (!us && (!(type & 0000004) || !(f->flags & 0000004) || ((f->flags & 0000004) && (f->bits & 00000002)))) {	/* returning data in buffer */
		us = f->next;
		un = s - f->next;
		f->next = s;
		goto done;
	...
	if (!rsrv || rsrv->size < un + n + 1) {
	    if (rsrv)
		rsrv->slen = un;
	    if ((rsrv = _sfrsrv(f, un + n + 1)) != ((Sfrsrv_t *)0))
		us = rsrv->data;
	    else {
		us = ((unsigned char *)0);
		goto done;
	...
	ends = f->next;
	f->next += n;
	switch(n) { default : memcpy((Void_t*)s,(Void_t*)ends,n); s += n; ends += n; break; case 7 : *s++ = *ends++; case 6 : *s++ = *ends++; case 5 : *s++ = *ends++; case 4 : *s++ = *ends++; case 3 : *s++ = *ends++; case 2 : *s++ = *ends++; case 1 : *s++ = *ends++; };
    }

  done:
    _Sfi = f->val = un;
    f->getr = 0;
    if (found && rc != 0 && (type & 0000004)) {
	us[un - 1] = '\0';
