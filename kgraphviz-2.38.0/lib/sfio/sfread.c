#line 1 "graphviz-2.38.0\\lib\\sfio\\sfread.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfread.c" 2
...
ssize_t sfread(register Sfio_t * f, Void_t * buf, register size_t n)
...
{
    register unsigned char *s, *begs;
    register ssize_t r;
    register int local, justseek;

    { if(!f) return((ssize_t)(-1)); };

    ((local) = ((f)->mode&00100000), (f)->mode &= ~00100000, (local));
    justseek = f->bits & 00000040;
    f->bits &= ~00000040;
	...
	if (f->mode & 00002000) {
	    if (((unsigned char *) buf + f->val) != f->next &&
		...
		{ return((ssize_t)(-1)); };
	    f->mode &= ~00000400;
	    if (f->mode & 00001000) {	/* actually read the data now */
		f->mode &= ~00001000;
		if (n > 0)
	...
	if (((f)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) ) != 0000001 && _sfmode(f, 0000001, local) < 0) {
	...
	if (!(f->flags & 0000004) && !(f->bits & 00000001)) {
	    f->next = f->endb = f->data;

	    /* exact IO is desirable for these cases */
	    if ((((ssize_t)(n) >= (f)->size) || ((n) >= 1024 && (ssize_t)(n) >= (f)->size/16 ) ) || ((f->flags & 0000100) && f->extent < 0))
		r = (ssize_t) n;
	    ...
	    if (r == (ssize_t) n && (r = (((f)->mode |= 00100000),sfrd(f,(Void_t*)s,r,f->disc))) >= 0) {
		s += r;
		n -= r;
