#line 1 "graphviz-2.38.0\\lib\\sfio\\sfsk.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfsk.c" 2
...
_ast_intmax_t sfsk(register Sfio_t * f, _ast_intmax_t addr, register int type, Sfdisc_t * disc)
...

{
    ...
    register ssize_t s;
    register int local, mode;

    { if(!f) return((_ast_intmax_t)(-1)); };

    ((local) = ((f)->mode&00100000), (f)->mode &= ~00100000, (local));
    if (!local && !(f->bits & 00001000)) {
	if ((mode = f->mode & (0000001|0000002)) != (int) f->mode
	    && _sfmode(f, mode, 0) < 0)
	...
	if (f->flags & 0000004) {
	    ...
	    { Sfdisc_t* d; if(!(dc)) d = (dc) = (f)->disc; else d = (f->bits&00001000) ? ((dc) = (dc)->disc) : (dc); while(d && !(d->seekf)) d = d->disc; if(d) (dc) = d; };
	    if (dc && dc->seekf) {
		{ int dcdown = f->bits&00001000; f->bits |= 00001000; p = (*dc->seekf)(f,addr,type,dc); if(!dcdown) f->bits &= ~00001000; };
	    } else {
	...
	if (local)
	    ((f)->mode |= 00100000);
	switch (_sfexcept(f, 3, s, dc)) {
	case 1:
	case 3:
	    if (f->flags & 0000004)
		{ return((_ast_intmax_t) s); };
	    goto do_continue;
	default:
	    { return((_ast_intmax_t)(-1)); };
	}
