#line 1 "graphviz-2.38.0\\lib\\sfio\\sfputm.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfputm.c" 2
...
int _sfputm(register Sfio_t * f, unsigned _ast_intmax_t v, unsigned _ast_intmax_t max)
...

{
    ...
    unsigned char c[(2*sizeof(unsigned _ast_intmax_t))];

    { if(!f) return(-1); };

    if (v > max|| (f->mode != 0000002 && _sfmode(f, 0000002, 0) < 0))
	{ return(-1); };
    (void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);

    /* code v as integers in base SF_UBASE */
    s = ps = &(c[(2*sizeof(unsigned _ast_intmax_t)) - 1]);
    *s = (unsigned char) (((unsigned long)(v))&((1 << 8)-1));
    while ((max>>= 8) > 0) {
	v >>= 8;
	*--s = (unsigned char) (((unsigned long)(v))&((1 << 8)-1));
    }
