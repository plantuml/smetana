#line 1 "graphviz-2.38.0\\lib\\sfio\\sfstrtod.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfstrtod.c" 2
...
static _ast_fltmax_t sfpow10(register int n)
...
_ast_fltmax_t _sfstrtod(register const char *s, char **retp)
...
{
    register int n, c, m;
    register int mode, fexp, sign, expsign;
    _ast_fltmax_t dval;
	...
	for (m = (2*sizeof(int)), n = 0; m > 0; --m, ++s) {	/* get and process a char */
	    c = *s;
	    if (isdigit(c))
		n = 10 * n + (c - '0');
	    else
	...
	if (mode == 0) {	/* doing the integer part */
	    if (dval == 0.)
		dval = (_ast_fltmax_t) n;
	    else
		dval = dval * sfpow10(m) + (_ast_fltmax_t) n;
	} else if (mode == 1) {	/* doing the fractional part */
	    fexp -= m;
	    if (n > 0)
		dval += n * sfpow10(fexp);
	...
	if (m < (2*sizeof(int))) {	/* detected a non-digit */
	    if (c == '.') {	/* start the fractional part or no match */
		if (mode != 0)
		    break;
		mode = 1;
		s += 1;
