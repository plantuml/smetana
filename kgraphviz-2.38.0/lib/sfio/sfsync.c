#line 1 "graphviz-2.38.0\\lib\\sfio\\sfsync.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfsync.c" 2
...
{
    register Sfpool_t *p, *next;
    register Sfio_t *f;
    register int n, rv;
    register int nsync, count, loop;


    for (loop = 0; loop < 3; ++loop) {
	rv = nsync = count = 0;
	for (p = &(_Sfextern.sf_pool); p; p = next) {	/* find the next legitimate pool */
	    for (next = p->next; next; next = next->next)
		if (next->n_sf > 0)
		    break;

	    /* walk the streams for _Sfpool only */
	    for (n = 0; n < ((p == &(_Sfextern.sf_pool)) ? p->n_sf : 1); ++n) {
		count += 1;
		f = p->sf[n];
		...
		if (((f)->mode&(00000100|00000040|00000400) ? 1 : ((f)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f) : 0))
		    continue;
		if ((f->mode & 0000001) && (f->mode & 00004000))
		    goto did_sync;
		if ((f->mode & 0000001) && !(f->bits & 00000001) &&
		    f->next == f->endb)
		    goto did_sync;
		if ((f->mode & 0000002) && !(f->bits & 00000004) &&
		    f->next == f->data)
		    goto did_sync;
...

{
    ...
    ((local) = ((origf)->mode&00100000), (origf)->mode &= ~00100000, (local));

    if (origf->disc == (&(_Sfextern.sf_udisc)))	/* throw away ungetc */
	(void) sfclose((*(_Sfextern.sf_stack)) (origf, ((Sfio_t *)0)));

    rv = 0;

    if ((origf->mode & (0000001|0000002)) != ((origf)->mode & ~(00000020|00000010|((local) ? 00000040 : 0)) )
    ...
    for (; f; f = f->push) {
	if ((f->flags & 0002000) && f->disc && f->disc->exceptf)
	    (void) (*f->disc->exceptf) (f, 9, (Void_t *) ((int) 1),
					f->disc);

	(void)((f)->mode |= 00000040, (f)->endr = (f)->endw = (f)->data);

	/* pretend that this stream is not on a stack */
	mode = f->mode & 00000100;
	f->mode &= ~00000100;

	/* these streams do not need synchronization */
	if ((f->flags & 0000004) || (f->mode & 00004000))
	    goto next;

	if ((f->mode & 0000002) && (f->next > f->data || (f->bits & 00000004))) {	/* sync the buffer, make sure pool don't move */
	    register int pool = f->mode & 00000200;
	    f->mode &= ~00000200;
	    if (f->next > f->data && (((f)->mode |= 00000020), (((f)->mode |= 00100000),_sfflsbuf(f,-1))) < 0)
		rv = -1;
	    if (!((f)->extent < 0 && ((f)->bits&00000010) ) && (f->bits & 00000004)) {	/* realize a previously created hole of 0's */
		if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)(_ast_intmax_t)(-1),1,f->disc)) >= 0)
		    (void) (((f)->mode |= 00100000),sfwr(f,(Void_t*)"",1,f->disc));
		f->bits &= ~00000004;
	    }
	    f->mode |= pool;
	}

	if ((f->mode & 0000001) && f->extent >= 0 && ((f->bits & 00000001) || f->next < f->endb)) {	/* make sure the file pointer is at the right place */
	    f->here -= (f->endb - f->next);
	    f->endr = f->endw = f->data;
	    f->mode = 0000001 | 00004000 | 00000040;
	    (void) (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)f->here,0,f->disc));

	    if ((f->flags & 0000100) && !(f->flags & 0004000) &&
		!(f->bits & 00000001)) {
	...
	if ((f->flags & 0002000) && f->disc && f->disc->exceptf)
	    (void) (*f->disc->exceptf) (f, 9, (Void_t *) ((int) 0),
					f->disc);
    }

  done:
    if (!local && f && (f->mode & 00000200) && f->pool
	&& f != f->pool->sf[0])
	(((f->pool->sf[0])->mode |= 00100000),sfsync(f->pool->sf[0]));
