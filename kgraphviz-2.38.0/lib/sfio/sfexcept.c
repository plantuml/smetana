#line 1 "graphviz-2.38.0\\lib\\sfio\\sfexcept.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfexcept.c" 2
...

{
    ...
    lock = f->mode & 00000040;

    if (local && io <= 0)
	f->flags |= io < 0 ? 0000400 : 0000200;
	...
	if (type == 0000001)
	    goto chk_stack;
	else if (type != 0000002 && type != 3)
	    { return(0); };
	if (local && io >= 0) {
	    if (f->size >= 0 && !(f->flags & 0000020))
		goto chk_stack;
	    /* extend buffer */
	    if ((size = f->size) < 0)
		size = 0;
	    if ((io -= size) <= 0)
		io = 1024;
	    size = ((size + io + 1024 - 1) / 1024) * 1024;
	    if (f->size > 0)
		data = (unsigned char *) realloc((char *) f->data, size);
	    else
    ...
    if (errno == 4) {
	if ((_Sfextern.sf_exiting) || (f->bits & 00000400))	/* stop being a hero */
	    { return(0); };
  ...
  chk_stack:
    if (local && f->push && ((type == 0000001 && f->next >= f->endb) || (type == 0000002 && f->next <= f->data))) {	/* pop the stack */
	register Sfio_t *pf;
	...
	/* pop and close */
	pf = (*(_Sfextern.sf_stack)) (f, ((Sfio_t *)0));
	if ((ev = sfclose(pf)) < 0)	/* can't close, restack */
	    (*(_Sfextern.sf_stack)) (f, pf);
