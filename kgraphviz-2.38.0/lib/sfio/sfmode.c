#line 1 "graphviz-2.38.0\\lib\\sfio\\sfmode.c" 1
/* $Id$ $Revision$ */
...
#line 16 "graphviz-2.38.0\\lib\\sfio\\sfmode.c" 2
...

{
    ...
    (_Sfextern.sf_exiting) = 1001;

    sfsync(((Sfio_t *)0));

    for (p = &(_Sfextern.sf_pool); p; p = p->next) {
	for (n = 0; n < p->n_sf; ++n) {
	    if (!(f = p->sf[n]) || ((f)->mode&(00000100|00000040|00000400) ? 1 : ((f)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f) : 0))
		continue;
	    ...
	    (void) (((f)->mode |= 00100000),sfraise(f,14,((Void_t *)0)));

	    if (f->flags & 0000004)
		continue;

	    /* from now on, write streams are unbuffered */
	    pool = f->mode & 00000200;
	    f->mode &= ~00000200;
	    if ((f->flags & 0000002) && !(f->mode & 0000002))
		(void) _sfmode(f, 0000002, 1);
	    if (((f->bits & 00000001) && f->data) ||
		((f->mode & 0000002) && f->next == f->data))
		(void) (((f)->mode |= 00100000),sfsetbuf(f,((Void_t *)0),0));
	    f->mode |= pool;
...
{
    register Sfpool_t *p;
    register Sfio_t **array;
    register int n, rv;

    if (!(_Sfextern.sf_cleanup)) {
	(_Sfextern.sf_cleanup) = _sfcleanup;
	(void) atexit(_sfcleanup);
    }

    if (!(p = f->pool))
	p = f->pool = &(_Sfextern.sf_pool);
...
Sfrsrv_t *_sfrsrv(register Sfio_t * f, register ssize_t size)
...
int _sfpopen(register Sfio_t * f, int fd, int pid, int stdio)
...

{
    ...
    p->pid = pid;
    p->size = p->ndata = 0;
    p->rdata = ((unsigned char *)0);
    p->file = fd;
    p->sigp = (!stdio && pid >= 0 && (f->flags & 0000002)) ? 1 : 0;
...
int _sfpclose(register Sfio_t * f)
...

{
	...
	if (p->file >= 0)
	    { while(close(p->file) < 0 && errno == 4) errno = 0; };
...

{
    ...
    if (type == 0000002) {	/* save unread data */
	p->ndata = f->endb - f->next;
	if (p->ndata > p->size) {
	    if (p->rdata)
		free((char *) p->rdata);
	    if ((p->rdata = (unsigned char *) malloc(p->ndata)))
		p->size = p->ndata;
	    else {
		p->size = 0;
		return -1;
...
int _sfmode(register Sfio_t * f, register int wanted, register int local)
...

{
    ...
    (void)(0);			/* initialize mutexes */

    if ((!local && ((f)->mode&(00000100|00000040|00000400) ? 1 : ((f)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f) : 0))
	|| (!(f->flags & 0000004) && f->file < 0)) {
	...
	for (;;) {
	    if ((rv = (*f->disc->exceptf) (f, 13, 0, f->disc)) < 0)
		return rv;
	    if ((!local && ((f)->mode&(00000100|00000040|00000400) ? 1 : ((f)->mode&00010000) ? (*(_Sfextern.sf_stdsync))(f) : 0)) ||
		(!(f->flags & 0000004) && f->file < 0)) {
		if (rv == 0) {
    ...
    if (f->mode & 00010000)	/* synchronizing with stdio pointers */
	(*(_Sfextern.sf_stdsync)) (f);

    if (f->disc == (&(_Sfextern.sf_udisc)) && wanted == 0000002 &&
	sfclose((*(_Sfextern.sf_stack)) (f, ((Sfio_t *)0))) < 0) {
	local = 1;
	goto err_notify;
    }

    if (f->mode & 00000200) {	/* move to head of pool */
	if (f == f->pool->sf[0] || (*(_Sfextern.sf_pmove)) (f, 0) < 0) {
	    local = 1;
	    goto err_notify;
	}
	f->mode &= ~00000200;
	...
	if (wanted != (int) (f->mode & (0000001|0000002)) && !(f->flags & wanted))
	    goto err_notify;

	if ((f->flags & 0000004) && f->size >= 0 && f->data) {
	    f->mode &= ~0000004;
	    f->extent = ((f->flags & 0000001) || (f->bits & 00000002)) ?
		f->size : 0;
	    f->here = 0;
	    ...
	    n = f->flags;
	    (void) (((f)->mode |= 00100000),sfsetbuf(f,f->data,f->size));
	    f->flags |= (n & 0000020);
	}
    }

    if (wanted == (int) ((f)->mode & ~(00000020|00000010|((1) ? 00000040 : 0)) ))
	goto done;

    switch (((f)->mode & ~(00000020|00000010|((1) ? 00000040 : 0)) )) {
    case 0000002:		/* switching to SF_READ */
	if (wanted == 0 || wanted == 0000002)
	    break;
	if (!(f->flags & 0000001))
	    goto err_notify;
	else if (f->flags & 0000004) {
	    { _ast_intmax_t s = (f)->next - (f)->data; if(s > (f)->here) { (f)->here = s; if(s > (f)->extent) (f)->extent = s; } };
	...
	/* reset buffer */
	if (f->next > f->data && (((f)->mode |= 00100000),_sfflsbuf(f,-1)) < 0)
	    goto err_notify;
    ...
    case (0000001 | 00004000):	/* a previously sync-ed read stream */
	if (wanted != 0000002) {	/* just reset the pointers */
	    f->mode = 0000001 | 00000040;

	    /* see if must go with new physical location */
	    if ((f->flags & (0000100 | 0004000)) ==
		(0000100 | 0004000)
		&& (addr = (((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)0,1,f->disc))) != f->here) {
		...
		if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)addr,0,f->disc)) < 0)
		    goto err_notify;
		f->here = addr;
	    }
    ...
    case 0000001:		/* switching to SF_WRITE */
	if (wanted != 0000002)
	    break;
	else if (!(f->flags & 0000002))
	    goto err_notify;
	else if (f->flags & 0000004) {
	    f->endb = f->data + f->size;
	    f->mode = 0000002 | 00000040;
	    break;
	}
	...
	if (!(f->mode & 00004000)) {
	    n = f->endb - f->next;
	    if (f->extent >= 0 && (n > 0 || (f->data && (f->bits & 00000001)))) {	/* reset file pointer */
		addr = f->here - n;
		if ((((f)->mode |= 00100000),sfsk(f,(_ast_intmax_t)addr,0,f->disc)) < 0)
		    goto err_notify;
		f->here = addr;
	...
	if (f->data == f->tiny) {
	    f->endb = f->data = f->next = ((unsigned char *)0);
	    f->size = 0;
	} else
    ...
    default:			/* unknown case */
      err_notify:
	if ((wanted &= (0000001|0000002)) == 0
	    && (wanted = f->flags & (0000001|0000002)) == (0000001|0000002))
	    wanted = 0000001;

	/* set errno for operations that access wrong stream type */
	if (wanted != (f->mode & (0000001|0000002)) && f->file >= 0)
	    errno = 9;

	if ((_Sfextern.sf_notify))		/* notify application of the error */
	    (*(_Sfextern.sf_notify)) (f, wanted, f->file);
