...
static void computeNodeGroups(graph_t * g)
{
	...
	} else			/* group same ranked nodes */
	{
		...
		nodeGroups[nNodeGroups].nodes =
		    (node_t **)zmalloc((ND_UF_size(l))*sizeof(node_t *));
...
static void computeLayerWidths(graph_t * g)
{
	...
	layerWidthInfo[ND_rank(v)].width +=
	    nodeGroups[i].width * 72 + (layerWidthInfo[ND_rank(v)].width >
					 0) * GD_nodesep(g);
	if (layerWidthInfo[ND_rank(v)].height < nodeGroups[i].height * 72)
	    layerWidthInfo[ND_rank(v)].height = nodeGroups[i].height * 72;
...
static void reduceMaxWidth2(graph_t * g)
{
	...
	if ((w +
	     ...
	     72 + (w > 0) * GD_nodesep(g) <= targetWidth)
	    || !fst) {
	    w += (layerWidthInfo[maxLayerIndex].nodeGroupsInLayer[i])->
		width * 72 + (w > 0) * GD_nodesep(g);
	    if (!fst) {
		fstNdGrp =
