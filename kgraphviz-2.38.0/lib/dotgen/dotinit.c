...
{
    Agsym_t* rk = agattr(g,AGNODE,"rank","");
    Agsym_t* order = agattr(g,AGNODE,"order","");
    Agnode_t* n;
    char buf[BUFSIZ];
...
{
    agbindrec(sg, "Agraphinfo_t", sizeof(Agraphinfo_t), NOT(0));
    GD_drawing(sg) = (layout_t*)zmalloc(sizeof(layout_t));
...
attachPos (Agraph_t* g)
{
	...
	ND_pos(np) = ps;
	ps[0] = ((ND_coord(np).x)/(double)72);
	ps[1] = ((ND_coord(np).y)/(double)72);
	ps += 2;
    }
}
...
resetCoord (Agraph_t* g)
{
    ...
    for (np = agfstnode(g); np; np = agnxtnode(g, np)) {
	ND_pos(np) = 0;
	ND_coord(np).x = ((ps[0])*(double)72);
	ND_coord(np).y = ((ps[1])*(double)72);
	ps += 2;
    }
    free (sp);
}
