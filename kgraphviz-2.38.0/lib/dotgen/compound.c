#line 1 "graphviz-2.38.0\\lib\\dotgen\\compound.c" 1
/* $Id$ $Revision$ */
...
#line 20 "graphviz-2.38.0\\lib\\dotgen\\compound.c" 2
...
static int countVertCross(pointf * pts, double xcoord)
{
    ...
    sign = (((pts[0].x)<(xcoord)) ? -1 : (((pts[0].x)>(xcoord)) ? 1 : 0));
    if (sign == 0)
	num_crossings++;
    for (i = 1; i <= 3; i++) {
	old_sign = sign;
	sign = (((pts[i].x)<(xcoord)) ? -1 : (((pts[i].x)>(xcoord)) ? 1 : 0));
...
static int countHorzCross(pointf * pts, double ycoord)
{
    ...
    sign = (((pts[0].y)<(ycoord)) ? -1 : (((pts[0].y)>(ycoord)) ? 1 : 0));
    if (sign == 0)
	num_crossings++;
    for (i = 1; i <= 3; i++) {
	old_sign = sign;
	sign = (((pts[i].y)<(ycoord)) ? -1 : (((pts[i].y)>(ycoord)) ? 1 : 0));
	if ((sign != old_sign) && (old_sign != 0))
	    num_crossings++;
    }
    return num_crossings;
	     ...
	     double xcoord, double ymin, double ymax)
{
    ...
    if ((no_cross == 1) && (((pts[3].x>=0)?(int)(pts[3].x + .5):(int)(pts[3].x - .5)) == ((xcoord>=0)?(int)(xcoord + .5):(int)(xcoord - .5)))) {
	       ...
	       double ycoord, double xmin, double xmax)
{
    ...
    if ((no_cross == 1) && (((pts[3].y>=0)?(int)(pts[3].y + .5):(int)(pts[3].y - .5)) == ((ycoord>=0)?(int)(ycoord + .5):(int)(ycoord - .5)))) {
...
static int splineIntersectf(pointf * pts, boxf * bb)
{
    ...
    t = findVertical(pts, 0.0, ((1.0)<(tmin)?(1.0):(tmin)), bb->UR.x, bb->LL.y,
		     bb->UR.y);
    if ((t >= 0) && (t < tmin)) {
	Bezier(origpts, 3, t, pts, NULL);
	tmin = t;
    }
    t = findHorizontal(pts, 0.0, ((1.0)<(tmin)?(1.0):(tmin)), bb->LL.y, bb->LL.x,
		       bb->UR.x);
    if ((t >= 0) && (t < tmin)) {
	Bezier(origpts, 3, t, pts, NULL);
	tmin = t;
    }
    t = findHorizontal(pts, 0.0, ((1.0)<(tmin)?(1.0):(tmin)), bb->UR.y, bb->LL.x,
		       bb->UR.x);
    if ((t >= 0) && (t < tmin)) {
	Bezier(origpts, 3, t, pts, NULL);
	tmin = t;
...
static void makeCompoundEdge(graph_t * g, edge_t * e, Dt_t* clustMap)
{
    ...
    if (!lt && !lh)
	return;
    if (!(((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)) return;

    /* at present, we only handle single spline case */
    if ((((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->size > 1) {
    ...
    /* allocate new Bezier */
    nbez = (bezier*)gmalloc(sizeof(bezier));
    ...
    fixed = 0;
    if (lh) {
	bb = &((((Agraphinfo_t*)(((Agobj_t*)(lh))->data))->bb));
	if (!inBoxf((((Agnodeinfo_t*)(((Agobj_t*)(head))->data))->coord), bb)) {
	    agerr(AGWARN, "%s -> %s: head not inside head cluster %s\n",
		  agnameof(((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node)), agnameof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node)), agget(e, "lhead"));
	} else {
		...
		if (inBoxf((((Agnodeinfo_t*)(((Agobj_t*)(tail))->data))->coord), bb)) {
		    agerr(AGWARN,
    ...
    fixed = 0;
    if (lt) {
	bb = &((((Agraphinfo_t*)(((Agobj_t*)(lt))->data))->bb));
	if (!inBoxf((((Agnodeinfo_t*)(((Agobj_t*)(tail))->data))->coord), bb)) {
	    agerr(AGWARN, "%s -> %s: tail not inside tail cluster %s\n",
		agnameof(((((((Agobj_t*)(e))->tag).objtype) == 3? (e): ((e)+1))->node)), agnameof(((((((Agobj_t*)(e))->tag).objtype) == 2? (e): ((e)-1))->node)), agget(e, "ltail"));
	} else {
    ...
    nbez->list = (pointf*)gmalloc((nbez->size)*sizeof(pointf));
    for (i = 0, j = starti; i < nbez->size; i++, j++)
	nbez->list[i] = bez->list[j];
    free(bez->list);
    free(bez);
    (((Agedgeinfo_t*)(((Agobj_t*)(e))->data))->spl)->list = nbez;
}
