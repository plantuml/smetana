...
static boolean samedir(edge_t * e, edge_t * f)
{
    ...
    for (e0 = e; ED_edge_type(e0) != 0; e0 = ED_to_orig(e0));
    for (f0 = f; ED_edge_type(f0) != 0; f0 = ED_to_orig(f0));
...
{
    return ((ND_node_type(v) == 1) && (ND_in(v).size == 1)
...
{
    return ((ND_node_type(v) == 1) && (ND_out(v).size == 1)
