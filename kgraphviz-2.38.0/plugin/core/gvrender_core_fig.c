#line 1 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 1
/* $Id$ $Revision$ */
...
#line 25 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
#line 26 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
#line 28 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
#line 29 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
#line 30 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
#line 31 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
#line 32 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
#line 1 "graphviz-2.38.0\\lib\\common\\color.h" 1
/* $Id$ $Revision$ */
/* vim:set shiftwidth=4 ts=8: */
...
#line 33 "graphviz-2.38.0\\plugin\\core\\gvrender_core_fig.c" 2
...
static void figptarray(GVJ_t *job, pointf * A, int n, int close)
{
    ...
    for (i = 0; i < n; i++) {
	((p).x = (((A[i]).x>=0)?(int)((A[i]).x + .5):(int)((A[i]).x - .5)),(p).y = (((A[i]).y>=0)?(int)((A[i]).y + .5):(int)((A[i]).y - .5)));
        gvprintf(job, " %d %d", p.x, p.y);
    }
    if (close) {
	((p).x = (((A[0]).x>=0)?(int)((A[0]).x + .5):(int)((A[0]).x - .5)),(p).y = (((A[0]).y>=0)?(int)((A[0]).y + .5):(int)((A[0]).y - .5)));
        gvprintf(job, " %d %d", p.x, p.y);
    }
    gvputs(job, "\n");
}
...
static void fig_resolve_color(GVJ_t *job, gvcolor_t * color)
{
	...
	case COLOR_STRING:
		...
		if ((*(figcolor[i])==*(color->u.string)&&!strcmp(figcolor[i],color->u.string))) {
		    color->u.index = i;
		    break;
		}
	    }
...
static void fig_textspan(GVJ_t * job, pointf p, textspan_t * span)
{
    ...
    double angle = job->rotation ? (3.14159265358979323846 / 2.0) : 0.0;
 ...
 */
    ...
    gvprintf(job,
            ...
            font_size, angle, font_flags, height, length, ((p.x>=0)?(int)(p.x + .5):(int)(p.x - .5)), ((p.y>=0)?(int)(p.y + .5):(int)(p.y - .5)),
            fig_string(span->str));
}
...
static void fig_ellipse(GVJ_t * job, pointf * A, int filled)
{
    ...
    start_x = center_x = ((A[0].x>=0)?(int)(A[0].x + .5):(int)(A[0].x - .5));
    start_y = center_y = ((A[0].y>=0)?(int)(A[0].y + .5):(int)(A[0].y - .5));
    radius_x = ((A[1].x - A[0].x>=0)?(int)(A[1].x - A[0].x + .5):(int)(A[1].x - A[0].x - .5));
    radius_y = ((A[1].y - A[0].y>=0)?(int)(A[1].y - A[0].y + .5):(int)(A[1].y - A[0].y - .5));
    end_x = ((A[1].x>=0)?(int)(A[1].x + .5):(int)(A[1].x - .5));
    end_y = ((A[1].y>=0)?(int)(A[1].y + .5):(int)(A[1].y - .5));
	      ...
	      int arrow_at_end, int filled)
{
    ...
    for (i = 0; i + 3 < n; i += 3) {
        ...
        for (step = 1; step <= 6; step++) {
            count++;
            pf = Bezier (V, 3, (double) step / 6, NULL, NULL);
