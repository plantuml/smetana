...
static void svg_textspan(GVJ_t * job, pointf p, textspan_t * span)
{
    ...
    if (pA) {
	switch ((((Agraphinfo_t*)(((Agobj_t*)(job->gvc->g))->data))->fontnames)) {
	case PSFONTS:
	    family = pA->name;
	    weight = pA->weight;
	    style = pA->style;
    ...
    gvputs(job, ">");
    gvputs(job, xml_string0(span->str, (!(0))));
    gvputs(job, "</text>\n");
}
...
static int svg_gradstyle(GVJ_t * job, pointf * A, int n)
{
    ...
    obj_state_t *obj = job->obj;
    angle = obj->gradient_angle * 3.14159265358979323846 / 180;	//angle of gradient line
