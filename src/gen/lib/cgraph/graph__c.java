...
try {
		...
		memdisc = (Agmemdisc_s) ((proto != null && proto.getPtr("mem") != null) ? proto.getPtr("mem") : AgMemDisc.amp());
		...
		rv.getStruct("disc").setPtr("id", ((proto != null && proto.getPtr("id") != null) ? proto.getPtr("id") : AgIdDisc.amp()));
		rv.getStruct("disc").setPtr("io", ((proto != null && proto.getPtr("io") != null) ? proto.getPtr("io") : AgIdDisc.amp()));
...
try {
    Agraph_s par;
    g.setPtr("n_seq", agdtopen(g, Ag_subnode_seq_disc.amp(), Dttree));
    g.setPtr("n_id", agdtopen(g, Ag_subnode_id_disc.amp(), Dttree));
    g.setPtr("e_seq", agdtopen(g, EQ(g, agroot(g))? Ag_mainedge_seq_disc.amp() : Ag_subedge_seq_disc.amp(), Dttree));
    g.setPtr("e_id", agdtopen(g, EQ(g, agroot(g))? Ag_mainedge_id_disc.amp() : Ag_subedge_id_disc.amp(), Dttree));
    g.setPtr("g_dict", agdtopen(g, Ag_subgraph_id_disc.amp(), Dttree));
    par = agparent(g);
    if (par!=null) {
	AGSEQ(g, agnextseq(par, AGRAPH));
  	par.getPtr("g_dict").call("searchf", par.getPtr("g_dict"),g,0000001);
    }				/* else AGSEQ=0 */
    if (N(par) || par.getPtr("desc").getBoolean("has_attrs"))
	agraphattr_init(g);
    agmethod_init(g, g);
    return g;
} finally {
...
try {
	int tmp = g.getPtr("clos").getArray("seq").plus(objtype).getInt();
	tmp++;
	g.getPtr("clos").getArray("seq").plus(objtype).setInt(tmp);
	return tmp;
} finally {
