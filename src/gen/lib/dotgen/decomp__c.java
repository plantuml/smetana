...
try {
    int i;
    i = GD_comp(G).getInt("size");
    GD_comp(G).setInt("size", 1+GD_comp(G).getInt("size"));
    GD_comp(G).setPtr("list", ALLOC_empty(GD_comp(G).getInt("size"), GD_comp(G).getPtr("list"), Agnode_s.class));
    GD_comp(G).getArray("list").plus(i).setPtr(GD_nlist(G));
} finally {
...
try {
    int c, i;
    __array__ vec = __array__.malloc(elist.class, 4);
    Agnode_s other;
    Agedge_s e;
    add_to_component(n);
	...
	if (vec.plus(c).getStruct().getPtr("list")!=null)
	    for (i = 0; (e = (Agedge_s) vec.plus(c).getStruct().getArray("list").plus(i).getPtr())!=null; i++) {
