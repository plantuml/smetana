...
public static Object checkFlatAdjacent(Object... arg) {
UNSUPPORTED("e2z2o5ybnr5tgpkt8ty7hwan1"); // static void
UNSUPPORTED("4q3clmc1u2r4xyu91lch84i7w"); // checkFlatAdjacent (edge_t* e)
UNSUPPORTED("erg9i1970wdri39osu8hx2a6e"); // {
UNSUPPORTED("dvftpyjsvkvw9h6kpv580gy4e"); //     node_t* tn = agtail(e);
UNSUPPORTED("dtkvzdaeftn9590oo7ft2rb4y"); //     node_t* hn = aghead(e);
UNSUPPORTED("5p1cejq1x5h0qmiz808hskln9"); //     int i, lo, hi;
UNSUPPORTED("7ma9kdgag30w5ofv1niitxbro"); //     node_t* n;
UNSUPPORTED("9oaiogwids948gqg6fyce2gkx"); //     rank_t *rank;
UNSUPPORTED("3dx8ty2ob1s28899lzcjozxir"); //     if (ND_order(tn) < ND_order(hn)) {
UNSUPPORTED("1leaxsmd4sqcbe90bsg61ap1e"); // 	lo = ND_order(tn);
UNSUPPORTED("dalv30psd393wkz866523jrz2"); // 	hi = ND_order(hn);
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("1nyzbeonram6636b1w955bypn"); //     else {
UNSUPPORTED("bg1khmbuhznkrdqmpnm6tr62b"); // 	lo = ND_order(hn);
UNSUPPORTED("74n8s9rimqjpbv74hmpfhy95p"); // 	hi = ND_order(tn);
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("1f14bf4glg46s19vpmuoi22ld"); //     rank = &(GD_rank(dot_root(tn))[ND_rank(tn)]);
UNSUPPORTED("c8kybl9wqg96bv3fc25zznn7b"); //     for (i = lo + 1; i < hi; i++) {
UNSUPPORTED("3pp7ewsbsse2hgk26f8gwnh7t"); // 	n = rank->v[i];
UNSUPPORTED("2ud6mjdfegpzhemgp9urbzgos"); // 	if ((ND_node_type(n) == 1 && ND_label(n)) || 
UNSUPPORTED("c4876jzgzro0adzw484xuff5v"); //              ND_node_type(n) == 0)
UNSUPPORTED("ai3czg6gaaxspsmndknpyvuiu"); // 	    break;
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("6zykauzxn1c2wvvufk17ast6y"); //     if (i == hi) {  /* adjacent edge */
UNSUPPORTED("8vxyvy38lzpbd83cu26nejaan"); // 	do {
UNSUPPORTED("dh2miz9aisyp9vl9jpn4tf03t"); // 	    ED_adjacent(e) = 1;
UNSUPPORTED("4rygtyzntschpjvx0mxk1oa3b"); // 	    e = ED_to_virt(e);
UNSUPPORTED("airw6eo1sq0tnc18f6jdr32ne"); // 	} while (e); 
UNSUPPORTED("dvgyxsnyeqqnyzq696k3vskib"); //     }
UNSUPPORTED("c24nfmv9i7o5eoqaymbibp7m7"); // }
...
try {
	...
	if (ND_flat_out(n).getPtr("list")!=null) {
	    for (j = 0; (e = (Agedge_s) ND_flat_out(n).getArray("list").plus(j).getPtr())!=null; j++) {
		checkFlatAdjacent (e);
	    }
	}
	for (j = 0; j < ND_other(n).getInt("size"); j++) {
	    e = (Agedge_s) ND_other(n).getArray("list").plus(j).getPtr();
	    if (ND_rank(aghead(e)) == ND_rank(agtail(e)))
		checkFlatAdjacent (e);
	}
    }
    if ((GD_rank(g).plus(0).getPtr("flat")!=null) || (GD_n_cluster(g) > 0)) {
	for (i = 0; (n = (Agnode_s) GD_rank(g).plus(0).getArray("v").plus(i).getPtr())!=null; i++) {
	    for (j = 0; (e = (Agedge_s) ND_flat_in(n).getArray("list").plus(j).getPtr())!=null; j++) {
	...
	if (ND_flat_out(n).getPtr("list")!=null) {
	    for (i = 0; (e = (Agedge_s) ND_flat_out(n).getArray("list").plus(i).getPtr())!=null; i++) {
		if (ED_label(e)!=null) {
		    if (ED_adjacent(e)!=0) {
