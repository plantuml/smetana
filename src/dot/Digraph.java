...
public class Digraph {
	...
	private Map<String, Agnode_s> nodes = new LinkedHashMap<String, Agnode_s>();
	private List<Agedge_s> edges = new ArrayList<Agedge_s>();
	private Collection<Agraph_s> clusters = new ArrayList<Agraph_s>();
	private final Agraph_s graph;
	private final Deque<Agraph_s> currents = new LinkedList<Agraph_s>();

	public Digraph() {
		Z.open();
		graph = agopen(new CString("g"), Z._().Agdirected, null);
	...
	private void openSubgraph(String s) {
		...
		final Agraph_s subcluster = agsubg(currents.getLast(), new CString(name), true);
	...
	private void addNodeSimple(String s) {
		...
		Agnode_s result = agnode(currents.getLast(), new CString(name), true);
	...
	private void addNodeOption(Agnode_s node, String key, String value) {
		agsafeset(node, new CString(key), new CString(value), new CString(""));
	}

	private void addEdge(String s) {
		String val[] = s.split("->|;|\\[|\\]|,");
		Agnode_s n = nodes.get(val[0].trim());
		Agnode_s m = nodes.get(val[1].trim());
		if (n == null || m == null) {
			throw new IllegalArgumentException(s);
		}
		// Agedge_s result = agedge(currents.getLast(), n, m, null, true);
		Agedge_s result = agedge(graph, n, m, null, true);
	...
	private void addEdgeOption(Agedge_s edge, String key, String value) {
